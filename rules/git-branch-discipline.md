...
The deny applies to the whole `Bash` call if **any** segment is a deny-case. The guard segments on `;`, `&&`, `||`, `|`, `&` and newlines, splices backslash line continuations before segmenting, strips `(…)` subshell parentheses, and inspects `$(…)` / backtick subshells. Ahead of the split it blanks what bash does not execute — a single-quoted string, and a heredoc body under either delimiter form — so that text is never segmented into candidate commands. An unquoted delimiter is the one carve-out: every `$(…)` and backtick region of such a body survives, because bash really does run it there, and reaches the same substitution inspection. So `cat <<EOF … EOF` with a body line reading `git switch main` writes that line to a file and is allowed, while `$(git switch main)` in that same body is denied.

Every segment the split yields is then classified, and one deny-case segment denies the whole call: a compound command hides no segment from the classifier, and dilutes no verdict the classifier reaches. That is a claim about segmentation and nothing else. Whether a classified segment is *read* correctly is a separate question, and `## Why` carries it.
