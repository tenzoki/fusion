...
The deny applies to the whole `Bash` call if **any** segment is a deny-case. The guard segments on `;`, `&&`, `||`, `|`, `&` and newlines, splices backslash line continuations before segmenting, strips `(…)` subshell parentheses, and inspects `$(…)` / backtick subshells. Ahead of the split it blanks two regions and only these two: the body of a single-quoted string, and a heredoc body under either delimiter form. Both are data a command reads rather than a command bash runs, so their text never becomes a candidate command. An unquoted delimiter is the one carve-out: every `$(…)` and backtick region of such a body survives, because bash really does run it there, and reaches the same substitution inspection. So `cat <<EOF … EOF` with a body line reading `git switch main` writes that line to a file and is allowed, while `$(git switch main)` in that same body is denied.

Read that blanking as the two-item list it is, not as "the guard blanks what bash does not execute". A `#` comment is text bash does not execute, and it is deliberately **not** blanked: it is emitted where it stood, and its own words reach the classifier, which takes the `#` for the command word and allows the segment (`# git switch main` allows, measured). Blanking is this lexer's one way of removing text, and its one measured deny→allow came from removing text that should have stayed — a `<<WORD` standing in a comment or in an arithmetic expression was read as a heredoc opener, and the blanking that followed erased the real commands underneath it (`shared/issues/260809-2044_*_a-false-heredoc-opener-blanks-real-commands-….md`, closed).

Quoting is therefore not the only axis the lexer models. Bash suspends its tokenizer inside a family of spans and recognises no operator in them, and six of those spans are read as one unit each and emitted verbatim: a `#` comment to the end of its line, `$((…))` arithmetic expansion, `((…))` arithmetic command, the deprecated `$[…]`, `${…}` parameter expansion, and the `name[…]=` subscript of an array assignment. Emitting rather than removing is what bounds a wrong guess here: a span boundary the lexer gets wrong hands the classifier more text, never less, so it costs a false deny and never an allow on a line the shell runs.

Every segment the split yields is then classified, and one deny-case segment denies the whole call: a compound command hides no segment from the classifier, and dilutes no verdict the classifier reaches. That is a claim about segmentation and nothing else, and it covers the text that reaches the split, which is everything outside the two blanked regions named above. It therefore rests on that list being exactly two items long, which is why the paragraph above names them instead of characterising them. Whether a classified segment is *read* correctly is a separate question, and `## Why` carries it.

Inside a segment, an extra word in front of the verb does not hide it either. The classifier resolves the command word before it reads it, so a leading `VAR=value` assignment, a compound-command head or body introducer (`if`, `elif`, `while`, `until`, `then`, `else`, `do`), a wrapper program (`sudo`, `env`, `exec`, `xargs`, `nohup`, `timeout`, `command`, `nice`, `time`, …), a path (`/usr/bin/git`), quoting (`"git"`) and a backslash escape (`\git`) all resolve to the same `git` the bare form does. `if git switch main; then :; fi`, `sudo git switch main`, `exec git switch main` and `\git switch main` are denied exactly as `git switch main` is. The resolved name is **case-folded** as well, unconditionally and at the point of resolution, so `GIT switch main`, `/usr/bin/GIT switch main` and `SUDO git switch main` are denied too — on a case-insensitive volume, which is every stock macOS install, those spellings reach the same binary (`shared/issues/260809-1110_*_the-command-word-comparison-is-case-sensitive-….md`, closed).

Read that as covering the words it names, not the class they suggest. The wrappers are a table, so a program that runs another program and has no row still hides the verb beneath it — `parallel git switch main` is allowed, checked — and a command word that is itself an expansion (`$CMD switch main`) names no program the classifier can read. Both are bounds, not invitations — see point 1 under `## What to do instead`.
...
**It is a choke-point on the tool call, not a proof of impossibility.** The classifier reads the command text, so a command that hides the verb from its own text is not seen — `eval 'git switch main'` and `bash -c 'git switch main'` are both allowed today, as is a branch switch inside a script the agent invokes, inside a `case` arm (`main) git switch main;;`) or inside a function body. Nor is a hidden verb the only way through. Three defects of the other kind have been measured — the verb in plain sight, the classifier reading it, and the call allowed anyway — and all three are now closed: a trailing `--` behind a branch-creating flag (`shared/issues/260809-1105_*_a-trailing-separator-lifts-the-branch-deny-….md`), an unrecognised global option whose value came to stand in subcommand position (`shared/issues/260809-1106_*_the-unknown-global-option-fix-was-deleted-….md`), and a capitalised command word on a filesystem that does not distinguish case (`shared/issues/260809-1110_*_the-command-word-comparison-is-case-sensitive-….md`). So the classifier can err inside the command form it does classify, not only outside it.

No measured defect of that kind stands open today, which is a weaker statement than it looks. All three were found by sweeping the classifier, not by reading it, so what the sentence really reports is the age of the last sweep. Three bounds are stated rather than measured, and they stand where they always did: the wrapper table and the unnameable command word, both named under `## The rule`, and the global-option walk, which closes the invocations where each unrecognised option takes at most one separated value and claims nothing beyond that — `git --namespace foo bar -C d switch main`, with a second bare word between the value and the verb, resolves to allow here. That one is not a live bypass, because git itself reads `bar` as the subcommand and refuses the command (checked against git 2.49.0), but it is unproven rather than closed.

Reaching for any of this to get past a deny is exactly the behaviour this rule forbids, whatever the guard happened to allow.
