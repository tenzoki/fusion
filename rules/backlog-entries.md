# Backlog Entries — maintenance

**Provenance:** Partitioned out of `rules/fusion-workbench-conventions.md` `## Backlog entries` per decision `shared/decisions/260827-1056_*_which-parts-of-the-language-and-backlog-rules-does-every-dispatch-still-carry.md`; the section's own history is that file's.

What a backlog entry is, where it lives, its marker reading, its minimum content and the two bounds (no agent files; the backlog is not the work queue) stay always-on at `rules/fusion-workbench-conventions.md` `## Backlog entries`, and this file does not restate them. This is the maintenance mandate, emitted to the `playmaker` — the store's one maintainer.

## Backlog entries

**Maintenance is four operations and a ranking rename.** Splitting one entry's ideas across several files, merging several statements of one idea into one, closing an entry whose idea is no longer live, and deferring one to a named later moment: four, each performed only with a user confirmation the run holds for that operation. Renaming between `_o_` and `_p_` states the playmaker's own ranking of a live idea and is autonomous. None of the five adds an idea to the store, which is why the no-agent-files bound survives them: the text a merge writes consolidates statements already filed.
