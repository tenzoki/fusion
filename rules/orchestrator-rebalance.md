...
|---|---|---|
| *Revise Artefact* re-entries count against the Max-Turns exit | the Turn counter and the resolved Turn budget | Neither exists. There is no counter to increment and no ceiling to reach, so nothing could ever force the Bounded Closure this bound promised. |
...
| `paused_at_task` in the state file, resuming *Revise Evidence base* where it left off | `fusion-workbench/agentstate.yaml` | The file is not written any more. The resume position now lives in the session's own reading of what it was doing, and nothing persists it across an interruption. |
...
**Approval 1 — does the brief stand?** Three options, always all three:

- **Revise Brief** — re-shape: dispatch `requirements-designer` with the current spec + the drift evidence. The destination we set was wrong; the next move is to re-state what we want. Under `state brief` there is no spec and nothing to drift from: this is the option that states one, and `requirements-designer` is dispatched in user-direct mode with the session's evidence. Emits `rebalance_directive` event. Re-enters the shaping step of `agents/orchestrator.md` `### Shaping and planning, when the task needs them`. **No new mechanism sits at this bullet**: the re-entry runs the ordinary shaping and planning steps and produces an ordinary spec. Forecloses: the artefact and evidence base are not touched until the new brief exists. (Bounding: once-per-session — see Rebalance bounding below.)
- **Accept Bounded Closure** — the brief is not reachable as stated; what was learned along the way is the artefact, and the session ends acknowledging that. Emits `bounded_closure_proposed` event. Names the reason in the closure note the work package takes. Forecloses: no further execution pass, no decision filed, no re-shape in this session. Terminal — see Rebalance bounding below.

  **It is the one move a mechanism can still force**, and only through the *Revise Brief* cap below. The three bounds that used to force it from a Turn count are gone (`## What the cut took out of this file`); nothing counts, so nothing else here reaches this option except the user choosing it.
- **Keep it** — the brief stands; what drifted is on the path to it. Forecloses: no re-shape and no closure at this approval. Opens Approval 2 immediately; no event of its own.

**Approval 2 — reached only on Keep it: what to revise?** Two options, always both:

- **Revise Artefact** — the artefact is not where it should be; the next move is another execution pass. The orchestrator names the next task itself from the state-auditor's three-edge summary, and re-enters the dispatch loop at `agents/orchestrator.md` `### Step 1 — read the task` with one task, not a queue. Emits `rebalance_artifact` event. Forecloses: the evidence base is not questioned on this pass. (Bounding: see Rebalance bounding below.)
- **Revise Evidence base** — file a new `_o_` decision record, or supersede an existing `_i_` decision (rename `_i_`→`_s_` and create a new `_o_`, per `fusion-workbench-conventions.md`). The basis we built on was wrong; the next move is to record a new question. Emits `rebalance_grounding` event. Forecloses: nothing is dispatched until the decision is filed. (Resume mechanics: see Rebalance bounding below.)

When the state-auditor's recommendation names a move, say which approval and which option it maps to when you put Approval 1, and put every option regardless: a recommendation is an input to the question, never a reason to hide a branch (`rules/critical-stance.md` §4).

The approval is reachable from the hand-run reconciliation's audit result and from nowhere else. It had a second trigger — the per-Turn Coherence check, decision `260827-1310_*_does-the-coherence-gate-ask-when-its-own-verdict-is-ok.md` — and that check was removed on 2026-09-10. The surviving procedure below is written against `agents/orchestrator.md`'s dispatch loop; the only Phases and Turns still named in this file are named in the table above, as things that are gone.

#### Rebalance bounding

**Every re-entry opens at Approval 1.** The task a *Revise Artefact* names, the resume after *Revise Evidence base*, a re-run reconciliation's audit result and a *Revise Brief* re-entry through the shaping step each reach the approval afresh: a retry that failed again is evidence about the brief, so Accept Bounded Closure and Revise Brief stay on offer, and Approval 2 is reached from Approval 1's **Keep it** only, on a re-entry as on the first pass. That the user said the brief stands last time is an input to the question, never a reason to hide the branch (`rules/critical-stance.md` §4).

**One of the four options carries a mechanical cap, and it is the only one.** *Revise Brief* is capped below. *Revise Artefact* and *Revise Evidence base* carry none, and *Accept Bounded Closure* is terminal and needs none. The three caps that used to bound *Revise Artefact* all counted Turns and went with the Turn loop (`## What the cut took out of this file`). What bounds a retry now is the user, who is asked after every commit whether to go on; say that plainly rather than calling the retries bounded.

- **Revise Artefact is not counted.** Each choice sends the orchestrator back into its dispatch loop with one named task. The approval reopens whenever the user runs another reconciliation and its result is not `coherent`, so a retry that drifted again is visible — but only if a reconciliation is run, and running one is the user's act. **Nothing forces Bounded Closure on repeated retries.** That is a real residual and it is stated rather than described away: a session can revise the artefact any number of times without any mechanism objecting.

- **Revise Brief is limited to once per session.** Count the choices in your own session context, starting at zero. The first Revise Brief choice re-enters the shaping step, regenerating spec and plan. A second Revise Brief in the same session is rejected; the approval instead forces Bounded Closure with reason `"Brief revised twice without convergence."`. Re-shaping twice in a session means the project, not this piece of work, needs to step back. Stating a brief under `state brief` is not a revision: it leaves the count at zero, so a later real Revise Brief is still the first.

  **The count is no longer persisted, and that costs something.** It rode `control.directive_revisions_this_session` in the session state file, which is not written any more, so the cap holds for as long as the session's own context does and no further. A session that is interrupted and restarted starts the count at zero and may revise the brief a second time without the approval objecting. Nothing detects that. It is not repaired here because a new persisted counter is precisely the class of hand-written state the cut removed — a number written at a boundary a session can pass without writing it — and re-adding one to hold a once-per-session cap would trade a measured failure mode for an unmeasured one. Say so if it comes up rather than claiming the cap survives a restart.

  **On the re-entry:** tell the user what triggered it — the reconciliation's audit result and their own Rebalance choice — and carry that into the commit message of whatever lands next, which is where this project keeps its per-change record. The requirements-designer produces a new spec with the prior commits as evidence base; then the planning step, and back into the dispatch loop.

- **Revise Evidence base files a record and nothing else.** The orchestrator notes where it was, then asks the user in chat to choose between:
  (a) **File a new `_o_` decision record** — orchestrator asks the user for the question text and any options/constraints (or for the full decision body if the user prefers to type it directly), then writes the file at `$OUT_DECISION/YYMMDD-HHMM_o_<topic>.md` per the decision-record template in `fusion-workbench-conventions.md`. A record that cites a work package names it by its bare basename, which carries no marker and never changes; OR
...
Brief** below dispatches the requirements-designer in its ordinary mode, which produces an ordinary spec, and
...
a permission: **can you quote the user's own words choosing it?** A stale evidence base, a state-auditor
audit result or your own reading that the brief no longer fits are inputs to the question you ask,
