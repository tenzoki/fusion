...
Whether the workbench is under version control at all is the project's decision — fusion ships no `.gitignore` rule for it, so a consuming project's workbench may be tracked, ignored, or neither. Where it *is* tracked, the split below ranges over **every root entry outside the artifact and legacy stores** (`circles/`, `shared/`, `archive/`, `stilwerk/` and the two above, all simply tracked), by whether a past version answers anything:
