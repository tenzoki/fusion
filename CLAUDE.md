...
- **Bus protocol (v3.4+)** — projects may opt in to workbench-mediated A2A messaging by populating `fusion-workbench/bus/`. Empty or absent `bus/` preserves pre-v3.4 single-terminal behaviour. The protocol is spec'd in `rules/fusion-workbench-conventions.md` `## Bus protocol`. Helper: `./.fusion/fusion-bus list|show|mark-read`. See `agents/orchestrator.md` `## User-Initiated Consultation` for the trigger/confirmation/file-and-tell flow; the orchestrator no longer offers bus filings at gate points (removed in v3.7.0).
