...
| `bin/fusion-bus-daemon` | Path D daemon (Python 3 stdlib, ~840 lines) that watches `bus/<agent>/inbox/` and routes messages via `tmux send-keys` with human-in-the-loop approval gating. Modes: strict (DEFAULT, every message waits for approve/deny) / observe (auto-inject + log) / silent (auto-inject silent). IPC via `bus/.daemon-state.json`, `bus/.daemon-cmd.json`, `bus/.pending/`, `bus/.approved/`, `bus/.denied/`. Started via `fusion-bus daemon start`; opt-in. See `rules/fusion-workbench-conventions.md` `### Path D — daemon and control IPC`. |
...
| Bus daemon never picks up new messages | Is the daemon running? `./.fusion/fusion-bus daemon status`. Is the target session registered with a `tmux_pane` in `bus/.sessions/`? Is `last_heartbeat` fresh (<600s)? Check `bus/.daemon.log` for recent action entries. |
