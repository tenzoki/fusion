...
- **Date-time filename prefix (v3.17.0+)** — tracking filenames use `YYYY-MM-DD_HH-MM` (e.g. `2026-06-03_14-30`), from `date +%Y-%m-%d_%H-%M`. Changed from the pre-v3.17.0 `YYMMDD-HHMM` form. The two coexist in a workbench (the change is forward-only; existing files are not renamed). Parsers that read a filename's date prefix tolerate both: `skills/archive` (aging), `skills/circle-stash` (slug extraction via `sed -E 's/^.*\[t\]-(.+)\.md$/\1/'`, format-agnostic), `skills/log-activity`. Caveat: mixed prefixes do not interleave under a plain lexical sort (new `2026-…` sorts before old `26…`) — cosmetic only. Canonical definition in `rules/fusion-workbench-conventions.md` `## Timestamps`.
