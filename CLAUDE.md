...
| `bin/fusion-bus` | User-facing helper for the workbench A2A bus (Python). Subcommands: `list` (per-agent inbox counts) / `show <agent> <id>` (read a message) / `mark-read <agent> <id>` (move to `.processed/`). See `rules/fusion-workbench-conventions.md` `## Bus protocol`. |
| `bin/fusion-bus-session` | Session registry helper for the bus (shell). Subcommands: `register` / `heartbeat` / `clear`. Maintains `fusion-workbench/bus/.sessions/<pid>.json` so filers can target the right inbox when multiple sessions are live. |
