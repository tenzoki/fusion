...
A consuming project may ship `fusion.json` at its project root — `/fusion:setup` seeds it from `templates/fusion.json`, declaring nothing but its documentation notes — and the hooks read it on every guarded tool call. **What it configures is `orchestrator.dispatchMinutes` and `citations.extraPaths`:** the stopping time the orchestrator requests of a bound agent's dispatch, resolved once at Setup, and the project's own list of citation-bearing non-Markdown paths, which the two citation helpers resolve when they are run. No hook reads either. `orchestrator.maxTurns` stood beside them until 2026-09-10 and went with the Phase-2 Turn loop; a project still declaring it gets one advisory naming the leaf. Everything else the file used to carry configured the guard, which decides nothing.
