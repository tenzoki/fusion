...
| `hooks/lib/__tests__/workbench-citation-lint.test.ts` | a live workbench record carries a citation that resolves to nothing. The corpus is the Circle records, `portfolio.md`, the open issues and the live decisions — `portfolio.md` where the checkout has one, being class L of `rules/workbench-tracking.md` and so regenerated per checkout rather than carried by git. | the record, the line, the token, and the two remedies its message carries: correct the path, or restate the token as a statement rather than a pointer. |
