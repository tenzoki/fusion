...
| `lib/shell-reach.ts` | The reachability layer between the lexer and the mutation classifier: reads the shell's grammar — compound heads, body words, terminators, pipeline membership — and reports, per segment, the **edge** by which it is reached rather than the one operator standing before it. A body reached through `then` proves its condition returned zero; a body reached through `until`'s `do` proves the opposite; every element of a multi-element pipeline runs in a subshell, head included. It reads the segment list `parseCommand` returns and annotates a copy, so the lexer is untouched and its segmentation cannot drift. Its one permitted consumer is `bash-mutation-guard.ts`; the git classifier never comes near it | Yes |
