...
Three of the git rows only mutate under a flag, which is what keeps the read and revert forms allowed: `git clean -n rules` is a dry run and `git clean -fdx rules` is not; `git restore rules/x.md` and `git restore --staged rules/x.md` are fusion's own revert strategy and `git restore --source=HEAD~1 rules/x.md` overwrites from anywhere in history. `git apply` and `git am` name their targets inside the patch file rather than on the command line and are **not** reached — they sit in the residual list with `patch`.
