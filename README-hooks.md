...
**Three armings.** A bound that used to report starts blocking on a corpus already past the head-room it would enforce, so the baseline is set at the arming rather than at the old figure. It has happened three times: on 2026-08-14 for the always-on rule core (`hooks/lib/__tests__/rules-emission-golden.test.ts`), on 2026-08-15 for the three surfaces in `hooks/lib/__tests__/surface-growth-bound.test.ts`, and on 2026-09-09 for the per-dispatch-path total, back in the first of those files. The third is the one deliberately taken *before* the cut it bounds rather than at it, because an arming taken at a cut absolves the cut. Each arming reproduces, as text in the file it armed, what its re-baseline let through.
...
**The cut-only refusal, and the measurement it rests on.** Whether a piece of work that only cuts may re-baseline the surfaces it cut was open from 2026-08-22 and was ruled by the user on 2026-09-11: it never does (`260822-1154_*_does-a-cut-only-circle-re-baseline-the-surfaces-it-cuts.md`, option 1). The reason is a measurement rather than a caution. Re-baselining is not one move with one sign, and at `e6703d51` the four surfaces would have gone four different ways:
