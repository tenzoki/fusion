...
Surfaced under *Misfiled — should be a decision* in `260819-1400-reconciliation-circles.md`. **Not moved by this pass**, per the reconciliation rule that a relocation between stores is the user's `mv`: the marker vocabulary changes with the store (`_o_/_p_/_c_/_d_` → `_o_/_a_/_i_/_d_/_s_`), and this Circle's decision store is where it would land under the Origin Rule, since the question arose from this Directive's own commit range.

**One correction to this record's own bookkeeping, and it is the reason a reader misses this item entirely.** Two records in this store carry the stamp `260817-1417`. This one is open; `260817-1417_*_the-release-went-out-over-a-turn-whose-six-shipped-file-commits-no-review-opened.md` is closed. The Circle record's `## Turn log` writes *"Filed as `260817-1417`, closed in Turn 4"*, naming the stamp alone — so a reader resolving that citation lands on the closed record and concludes the stamp is disposed of. The `_b_` record is terminal and is not edited; the ambiguity is recorded here instead, and in the reconciliation log.

---
Resolved: moot — the commit is published and not rewritten, the language rule already states the convention and no gate is added; `rules/fusion-workbench-conventions.md` `## Project language`.
