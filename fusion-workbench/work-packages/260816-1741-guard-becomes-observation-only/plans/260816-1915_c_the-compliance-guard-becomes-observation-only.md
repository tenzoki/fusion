...
**Spec:** none as a separate file. The specification is the Circle record, `260816-1741-guard-becomes-observation-only`, whose `## Directive` states the target state and whose `## Grounding snapshot` carries the three decisions this Circle executes.
...
Three decisions drive it, all answered by the user on 2026-08-16 and all recorded in `260816-1500-orchestrator-session.md`:

| Record | Answer |
|---|---|
| `260809-1224_*_is-the-decision-governed-escalation-check-3-a-live-feature.md` | option 1, retired |
| `260812-1232_*_does-the-escalation-counter-survive-a-block-source-that-ships-inert.md` | option 3, remove escalation, the halt and `clear-halt.js` |
| `260812-1232_*_does-the-write-guards-fusion-repo-stand-down-survive-the-loss-of-its-subject.md` | option 3, dissolution |

A fourth record belongs to this Circle and bounds the removal rather than driving it: `260816-1742_*_where-does-the-orchestrators-turn-budget-live-once-the-guard-configuration-file-is-gone.md`, answered as option 1. A renamed project-root file keeps the Turn budget, and `hooks/lib/config.ts`, `hooks/turn-budget.ts` and `bin/fusion-turn-budget` survive in reduced form instead of being deleted.
...
11. [DONE] **The shipped text says what the guard now is**
    - Executor: `coder`
    ...
    - Executed 2026-08-17. `cd hooks && npm test` exits 1 at **2 files / 3 cases**, down from 3 / 5. `derivable-enumerations-lint` is green — the `hooks/lib` table lost its `escalation.ts` and `project-relative.ts` rows and is back in set equality with the tree. `surface-growth-bound` (2 cases) is step 10's and untouched. **`reference-resolution-lint` is green except its whole-surface case, and the four citations keeping it red are all in `CLAUDE.md`** (`:29` and `:129` naming `hooks/lib/project-relative.ts`, `:30` naming `templates/fusion-guard.json` and `hooks/config.json`), which the Directive puts on the curator's gated path at step 16. Every one of the other 25 is gone. **Step 16 is therefore sufficient** to finish this gate; nothing outside `CLAUDE.md` is left for it to fix. The pinned counts were re-approved in the same change, `paths` 1122 → 1103, with the note stating that the pin had already fallen to 1093 through steps 2 to 7b before this step put 10 back.
    - **What the step's Files list did not carry, and where each landed.** Six defect records amend it and all six are closed here. `bin/fusion-turn-budget`'s header (`260816-2124_*_bin-fusion-turn-budgets-header-documents-the-configuration-file-7a-renames-and-no-step-owns-it.md`) now names `fusion.json` and states the two-layer merge instead of the plugin layer. `hooks/lib/guard-state-file.ts` (`260816-2125_*_guard-state-file-names-the-deleted-escalation-module-as-one-of-its-three-live-callers.md`) drops `escalation.ts` from its live-caller sentence, reads its `## What escalation.ts added` section as history, and re-justifies the optional `root` on what is true now — every caller in the tree passes one, and the parameter is kept the way `isFusionPluginRoot` is kept, explicitly without a current subject. `docs/upgrading-to-v9.md` (`260816-2126_*_the-v9-upgrade-note-tells-a-project-a-halt-still-blocks-and-still-clears-and-no-step-owns-it.md`) keeps its v9 history and corrects the two present-tense bullets in `## What needs no action`, pointing forward to the v10 note in prose rather than by path, because that file does not exist until step 12 and a citation of it would be the very dangling reference this step is closing. `skills/setup/SKILL.md` (`260816-2127_*_the-setup-deletion-offer-names-one-of-the-two-mechanisms-that-could-have-raised-the-flag.md`) attributes the legacy flag to no mechanism at all rather than to one of two, which is the formulation that cannot go stale again. `hooks-wiring.test.ts` (`260816-2315_*_hooks-wiring-test-was-in-step-9s-list-was-not-edited-and-justifies-bash-by-a-mechanism-removed-on-260812.md`) replaces the protected-path fingerprint with the reason that holds — the diagnostic loop, and that since `fab8a4b` the diagnostic *is* the v10 migration, so this matcher is how it reaches a project. `agents/orchestrator.md:122` and `bin/monitor:188` (`260816-2321_*_step-11s-line-scoped-changes-text-misses-two-stale-lines-in-files-it-already-opens.md`) are both corrected in full rather than at the one clause the line-scoped text named.
    - **Three departures worth recording.** First, the *Guard halt* circuit-breaker row was **deleted** rather than reworded, at both `:626` and its restatement at `:644`, and the two counts around the restatement moved with it ("the other five" → four, "one exit among six" → five). Second, `README-hooks.md`'s `### Clearing a halt` and `### Adding a decision` sections were deleted whole: the first names a script that is gone, the second configures a check that is gone, and neither has a shorter true form. That freed `rules/relevant-file.md` from `EXAMPLE_PATHS`, which the no-dead-weight case caught the moment the section went, and added `lib/escalation.ts` there on the precedent of the four modules the protected-path removal left named on purpose. Third, three surfaces outside the step's list were corrected while open because the same sentence was false in them: `docs/working-model.md:162` (named by `260816-2126_*_the-v9-upgrade-note-tells-a-project-a-halt-still-blocks-and-still-clears-and-no-step-owns-it.md`), `README.md`'s best-practices bullet telling a user to dial the guard down, and `docs/philosophy.md`'s opening claim that drift is caught at write time.
    - History: `260817-0400-step-11-shipped-text-says-what-the-guard-is.md`.
...
13. [DONE] **The decision records are annotated**
    - Executor: `ontocoder`
    ...
    - Changes: three transitions, each with its own instrument from `rules/fusion-workbench-conventions.md`.
      - `_a_` to `_i_`, with an `Implemented:` line citing the commit: the three records this Circle executes, and `260816-1742_*_where-does-the-orchestrators-turn-budget-live-once-the-guard-configuration-file-is-gone.md`.
      - A `Retired:` line with **no rename**, the marker staying `_i_`: every record whose implementation this plan deletes. Found at HEAD: `260804-1631_*_may-a-project-file-set-guard-enabled-and-switch-the-whole-guard-off.md`, `260804-1630_*_what-does-a-project-guard-object-inherit-for-a-key-it-does-not-supply.md`, `260803-1419_*_how-should-the-protected-path-check-treat-the-case-of-a-path.md` and `260802-1912_*_does-the-self-protection-floor-apply-before-the-config-file-exists.md`. Re-derive the list rather than trusting it: `grep` the decision stores for the identifiers this plan deletes.
      - `_a_` to `_s_`, with a `Superseded by:` line: `260807-0945_*_integritaet-des-eskalationsspeichers.md`, whose question is how the integrity of the escalation store is secured. The store is gone, so its recorded answer can never be realised, and the superseding record is the escalation decision this Circle executes.
      - Close the two issues this removal resolves: `260812-1546_*_check-3-the-guards-only-remaining-block-source-allows-from-any-subdirectory-and-nothing-tests-it.md` and `260812-1546_*_the-record-of-the-floors-loss-does-not-say-the-file-it-stopped-defending-arms-the-last-block-source.md`, each with a `Resolved:` line and a rename to `_c_`.
    - Dependencies: step 9
    ...
    - Executed 2026-08-17, under the user's approval of `260816-2316_*_step-13s-retired-line-list-is-wrong-in-three-of-its-four-entries-and-its-marker-scope-misses-a-fifth-record.md`, which **overrides this step's `Retired:` list**. Eleven records edited, ten renamed: six decisions `_a_` → `_i_`, one `Retired:` line with no rename, one decision `_a_` → `_s_`, and three issues `_o_` → `_c_`. `cd hooks && npm test` exits 1 at the unchanged red set of 2 files / 3 cases.
    - **The step's `Retired:` list was wrong in three of its four entries and its safeguard would have confirmed the worst one.** Only `260804-1631_*_may-a-project-file-set-guard-enabled-…` took a line. `260803-1419_*_…` and `260802-1912_*_…` already carry `Retired: 60c9cd8` and got no second. `260804-1630_*_what-does-a-project-guard-object-inherit-…` got none: its *setting* was deleted, its *answer* is live code, and `hooks/lib/config.ts` cites it in the present tense at `:46-49`, `:70-73` and `:363-367` with the per-leaf walk surviving at `:479-486` — all four read at HEAD before deciding. The re-derivation instruction was **not** run: a grep for deleted identifiers returns `260804-1630_*_what-does-a-project-guard-object-inherit-for-a-key-it-does-not-supply.md` first, because its `## Question` names five of them, so the safeguard points straight at the entry that must not carry a line.
    - **Two commit attributions in the dispatch were corrected against the tree.** The stand-down record's two halves were named the wrong way round: `2f624ca` removed the branch from `hooks/guard.ts`, `ec3b6ad` removed the then-callerless `isFusionPluginCwd()`. And `260816-1916_*_does-setup-offer-to-move-…` gained a third commit, `18c125b`, because its recorded answer includes `docs/upgrading-to-v10.md`, which landed at step 12 rather than step 8.
    - **`260804-1632_*_should-findrelevantdecisions-fold-case-…` was left untouched**, which is this step's one finding that produced no edit. The vocabulary holds no instrument for a deferred question whose subject was deleted, and the closest one is what `260815-2056_*_what-marks-an-answered-decision-whose-answer-can-no-longer-be-realised.md` is open to decide. The fact is already in the tree at `hooks/lib/paths.ts:36-41`.
    - History: `260817-0530-step-13-decision-records-annotated.md`.
...
- [x] **How much of the configuration loader survives when its only leaf is the Turn budget?** Filed as `260816-1915_*_how-much-of-the-configuration-loader-survives-when-its-only-leaf-is-the-turn-budget.md`. It covers whether the plugin layer `hooks/config.json` survives and whether `guard.enabled` survives in any form. The plan's steps assume the recommended answer, two layers and no `guard.enabled`, and step 7a is gated on it. Blocks step 7a only.

- [x] **Does `/fusion:setup` offer to move a project's Turn budget out of the retired file?** Filed as `260816-1916_*_does-setup-offer-to-move-a-projects-turn-budget-out-of-the-retired-configuration-file.md`. The plan as written says no, and lets the loader's diagnostic carry the migration. Blocks step 8 only.
...
One defect found next to this work, unrelated to the Directive, is filed in the shared store: `260816-1918_*_the-orchestrators-setup-names-planner-among-the-domain-parameterised-dispatches.md`.
...
session history is `260816-1841-orchestrator-session.md`.
...
- Scope: whatever the curator's survey proposes and the user approves at its gate. The two
  ...
  (issue `260816-2115_*_the-why-the-message-is-english-argument-names-two-removed-mechanisms-as-its-examples.md`).
- Defect: `260816-2123_*_claude-mds-two-dangling-citations-keep-the-citation-lint-red-and-no-step-in-this-plan-may-fix-them.md`
...
`260816-2122_*_step-9s-harness-reduction-deletes-four-fixtures-guard-bash-integration-still-imports.md` (the harness reduction deletes four fixtures that same file imports, so adding the
...
to `260815-2109_*_may-a-circle-close-over-an-uncovered-review-range-and-who-decides.md`
...
hand. Filed as `260817-1417_*_the-release-went-out-over-a-turn-whose-six-shipped-file-commits-no-review-opened.md`.
...
work tree cited `260810-1618_*_a-release-was-tagged-and-pushed-while-its-own-review-pass-was-still-running.md`
