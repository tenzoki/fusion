...
---
Answered: the mechanism is option 5, suppression on the outcome line; and the candidate case is answered by neither (a) nor (b) but by a fifth outcome value naming it, `not-offered`; ruled by user, Kai Stalmann <ks@qantr.com>.

**The mechanism.** The presence of an outcome line already carries what the fifth value of option 1 was invented to carry, and the two premises were re-read before the ruling: the gate puts one option per non-empty consequence group, so a group the user did not take was a group he was shown; and an apply dispatch runs only after a gate answered with at least one approval, so an outcome line exists only where a gate was answered.

**Why neither remedy in `## The candidate case` was taken.** Remedy (a) leaves `skipped` standing on an entry nobody was shown. That is not a missing statement but a false one, and a reader catches it only by holding the entry's group line beside its outcome line, with nothing keeping the two in step. This project has abolished that exact shape once already: the decision record's `Status:` head field was removed because 39 of 94 records carried a head asserting a state their filename denied (`rules/fusion-workbench-conventions.md` `## Decision Record Template`). Remedy (b) does not remove the ambiguity, it moves it — afterwards "no outcome line" means either never offered or no gate answered — so it trades a false statement for an ambiguous blank. That is the better trade, because a blank asserts nothing, and it is not clean.

**Why the fifth value survives the objection that killed option 1's.** Option 1's `declined` denoted the same set as `skipped`, because every non-empty group is always offered, so it bought no separation and was the overlap `rules/critical-stance.md` §4 calls a defect. `not-offered` denotes a set that exists today and is mislabelled today, and it exists in all four subjects rather than only in the edges: an unreadable cited record downgrades a finding to a candidate everywhere (`agents/curator.md` `### The thin spot, stated honestly`). With it, a line present means one thing in all four subjects, no line means no gate was answered, and the third case states itself. Five outcomes, disjoint and complete, none of them resting on a neighbouring line.

**The price is the same as remedy (b)'s and is accepted:** one place in the agent prompt plus the vocabulary list in `README-agents.md`, and it reaches all four subjects rather than the edges alone.

**Two residuals, both named rather than fixed.** A user who leaves one group standing while approving another loses the standing group permanently; that is the ruling's real cost. A user who refuses everything gets everything back, because no apply dispatch runs and no outcome line is written — this falls in the harmless direction, and the obvious repair is barred, since `skills/curate/SKILL.md` writes nothing at all, the run file included.

**One consequence the ruling pays for separately.** Suppression means every future survey reads every prior run file across the whole workbench, deliberately unanchored. Measured at this commit: 975 220 bytes over 15 files, of which exactly one carries an edge entry, and each run adds 60 to 170 KB. Pulling that into an agent's context to learn a handful of answered pairs buys back, on the read side, the uneconomic charge that sank option 3. It moves into a `bin/` helper of the same shape as `bin/fusion-work-order`, which no growth bound measures; its test file is measured, and the user authorised raising that bound where it is genuinely needed.

---
Implemented: 220cdad2 — the fifth outcome value `not-offered` is written in `### Pass 2 — apply` on every candidate in all four subjects, listed with the other four in `## The run file`, and the invariant sits directly under that list. The suppression table is re-keyed on the outcome value and the consequence-group row is removed, so no second route to the same fact survives. The unanchored read moves into `bin/fusion-edge-answers`, which prints the answered pairs with the field beside each, called from the prompt behind an `[ -x ]` guard whose absent branch suppresses nothing and is reported. The hook-test bound rose by exactly the 190 lines of that helper's test file, justified on the constant itself.
