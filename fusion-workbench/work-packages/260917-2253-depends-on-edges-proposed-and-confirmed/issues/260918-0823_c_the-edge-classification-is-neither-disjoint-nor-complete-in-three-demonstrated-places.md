...
Resolved: `agents/curator.md` `### The classification, cut on direction` is recut, and the cut is different in kind rather than patched. **The pre-test is gone**, and with it the overlap: test 1 is now a *lookup* — "which work items does the sentence identify, other than the corpus owner", answered against the workbench index and deciding nothing about meaning — while test 2 stays a *reading*. That is the cut. The four combinations are a table, each with one outcome: empty/no is not a candidate and is not reported; empty/yes is residue; non-empty/no is a citation entry on the corpus owner, which has no direction to get wrong; non-empty/yes is an ordering reading.

**Residue has a stated bound that is not a run's taste**: an ordering whose other end is not a work item, and nothing else. Not "a relation to something", which every sentence in a workbench asserts and which is what made the first run's six a judgement no reader could see behind.

**The ordering cell splits three ways on which end is the dependent, and the three are demonstrably complete** — the corpus owner is live by construction, so where it is the dependent only the target's liveness remains, and where it is not, the target is the corpus owner and is live. Dependent the corpus owner with a live target is a `**Depends-on:**` entry; with a terminal target the live/terminal bound sends it to `**Cross-references:**`; dependent the other item is residue. That names an outcome for each of the four combinations of the other item's liveness and the direction, which is what this record's acceptance asked for.

**Gap C is answered with a sentence, as the analysis said it needed.** An ordering reading routed to `**Cross-references:**` keeps the consequence group **work-item ordering edge**. Field and group part company in exactly that one place, and the reason is stated where a reader meets it: the group names the judgement the pass made, the field names where the bytes go, and `### The gate`'s two groups exist so a user can refuse this pass's direction calls — an entry that *is* a direction call belongs with them wherever it lands. `### Ledger entry schema` now says the group is cut on the reading and never derived from the field, so the routing cannot drift back.

**Section 2's design question is not decided here**, per dispatch. The dependent-is-the-other-item cell routes to residue — reported with the reason rather than discarded unseen, which is what the split needed to be complete — and cites `260918-0828_*_may-the-edge-pass-propose-an-edge-whose-dependent-is-not-the-corpus-owner.md`, filed to give that ruling and the first run's F4 one open home, since this record shows they are one question. The classification stays disjoint and complete under either answer.
