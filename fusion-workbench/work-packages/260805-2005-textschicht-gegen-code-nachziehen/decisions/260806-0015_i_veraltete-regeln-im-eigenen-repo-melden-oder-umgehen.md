...
**Status:** open
**Filed by:** coder
**Cross-references:** 260805-2353_*_plan-textschicht-gegen-code.md (Track 2, Schritt 3; Schritt 16 realisiert die Antwort); 260805-1859_*_im-eigenen-repo-laden-alle-agenten-die-regeln-der-installierten-vorversion-nicht-die-quelle.md (das High-Finding; erhält in Plan-Schritt 13 die Korrektur „kein Defekt, aber realer Rückstand"); hooks/lib/self-detect.ts (`isFusionPluginCwd()` — das existierende Repo-Kriterium)
...
---
Answered: 260805-2350-orchestrator-session.md — User wählt Option (c): bin/fusion-rules und bin/fusion-paths bevorzugen im Plugin-Repo die Repo-eigenen rules/ statt der installierten Kopie (Kriterium: isFusionPluginCwd-Äquivalent). Verhaltensregel (a) bleibt als Dokumentation bestehen. (Gate 260806-0027)
...
Reconciliation 260806-1057-reconciliation.md: the implementing commit is c45fb44 (feat(rules): die Helfer kennen das eigene Repo — Scoping und Repo-Vorrang) (the footer above was written before the orchestrator commit existed). Verified at HEAD: bin/fusion-plugin-cwd exists and gates both the repo-rules preference and the protected-path-internals emission (bin/fusion-rules:162-167); tests in rules-emission-golden.test.ts and fusion-paths.test.ts pass in the green 1611 suite.
