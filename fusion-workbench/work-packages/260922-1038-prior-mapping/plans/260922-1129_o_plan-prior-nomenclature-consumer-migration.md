...
1. [DONE] **Cut the four pre-v4 conversions from `/fusion:migrate`**
   - Executor: `coder`
...
2. [DONE] **Write the store-name pass**
   - Executor: `coder`
   - Files: `skills/migrate/SKILL.md`
...
3. [DONE] **Remove the four pre-v4 probes from `/fusion:setup`**
   - Executor: `coder`
...
4. [DONE] **The tests: delete, narrow, add; regenerate the golden; reword the citation-scan reason**
   - Executor: `coder`
...
7. [DONE] **The release surfaces this plan owns: two README rows, one help sentence, one upgrade-note section**
   - Executor: `coder`
...
8. [DONE] **Preconditions and the before-census for the first run on this repository's workbench**
   - Done 260926-0101, on the user's word, with two preconditions knowingly not met: the run preceded the `12.0.0` release (no `v12.0.0` tag; the release is set for 260927/28), and the session loaded `12.0.0` from this work tree via `--plugin-dir` rather than after `fusion --update`. The tree was clean apart from the setup marker, profiles and event log, which stayed unstaged. The census was not taken before the run; step 9 reconstructs it from `4888358c`, the commit before the migration. Claim state: `260923-0839-implement-prior-nomenclature`.
   - Executor: `coder`
...
9. [DONE] **Verification of the first run, and the migration commit**
   - Done 260926-0101. Migration commit `7d2ea602` (message `chore(workbench): migrate the stores to the v12 names`), before the `12.0.0` tag rather than after it. Survey: 42 containers under `circles/`, 37 of them folding a `planning/`, 8 and 2 entries under the shared stores, 18 empty directories, 0 collisions, 1 untracked entry (`circles/.DS_Store`, moved by `mv`). Yes: 1647 renames, no non-zero numstat row, no `D` row; 3023 basenames equal against `4888358c`; `archive/`, `orchestrator-events.jsonl` and `.guard-state/` unchanged by the commit; no `circles`, `shared/planning` or `shared/consult` left, 0 `planning/` and 37 `plans/` under `work-packages/`; `bin/fusion-paths` scans `work-packages/…/plans` and `shared/plans`; `bin/fusion-claimed-package` names the item recorded in step 8; a second survey reports `FOUND=0`; `git log --follow` reaches pre-migration history. Not taken: the `bin/fusion-citation-check` before/after comparison (inference: unchanged, since no basename moved). `npm test` was red on one test that reads the old names literally, `plan-stopping-section-lint.test.ts` (after the move it read 0 plans); re-keyed onto `hooks/lib/stores.ts` per `## Where this work stops`, golden regenerated (257 lines, down 4), 1009 of 1009 green. `bin/fusion-citation-sweep --dry-run`: `files=0 rewrites=0 residual=3582`; no `--write` run. Also run on krk, a consuming project, on the user's side, 260925, reported working; not pushed there before the release.
   - Executor: `coder`
...
- [x] **The route tag.** Spec C10, ruled on 2026-09-22, names `v11.10.0`, then the newest tag. The newest is now `v11.11.1`, whose migrate also offers the citation sweep after the bracket rename (Step 6). `v11.10.0` still converts every pre-v4 shape, and the 12.0.0 migrate's kept Step 6 offers the sweep afterwards, so the plan keeps `v11.10.0` as ruled. Naming `v11.11.1` (or "the last v11 tag") instead is the user's call, since it changes a ruled literal: in this plan's `## Directive`, the stop table in `## Approach`, and steps 1, 2, 3, 4 (case f), 6 and 7, and in the spec's C10. **Ruled 260923 by the user: `v11.11.1`.** Every step naming `v11.10.0` as the route (steps 2 and 7, the C10 table row) writes `v11.11.1` instead.
- [x] **Step 6's write offer in the first run here.** Step U answers it no, so that step 9's citation figures and the one-commit rename measure the renames alone; a yes would leave unstaged record edits beside the staged renames, which step 9's `--cached` check still passes but its citation-figure equality does not. The default stands unless the user wants the write in the same session, as a second commit after step 9. **Ruled 260923 by the user: second commit.** Step U answers the offer no as planned; after step 9's migration commit the sweep runs with its write offer answered yes, committed as a separate second commit.
