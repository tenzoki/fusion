...
16. [DONE] **Bring the eight voice-profile files under the ceiling** (record 1, profile half)
    - Executor: `ontocoder`
...
17. [DONE] **State in the instrument's header that a deleted file refunds nothing, and close the two head-room records** (+6 lines, not +4: the record citation wraps)
    - Executor: `coder`
...
24. [DONE] **One source for the store names, read by the classifier, the scanner and the literal lint, and checked against the layout tree** (hook tests −1 line net, the literal list gone; four things the step did not foresee: the widened lint caught `shared/checkouts/<id>.md` in `skills/cleanup/SKILL.md` (reworded, −12 bytes, and the sentence's claim filed as `260922-1053_*`); the widened grammar reddened the live-record lint and the own-tree sweep gate on three `shared/forum/` tokens in two records of the message-between-checkouts container, repaired per each gate's remedy; `README-hooks.md` needs a `lib/` row per the enumeration lint, which moved the pin 1697 → 1699, re-approved; `store-prefixed=` read 405, not 408, the three repairs net of the four new)
    - Executor: `coder`
...
34. [DONE] **Close five records on what the tree already shows** (no tree change; each line names the evidence and, for the three terminal targets, the rule that keeps the correction in the record)
    - Executor: `coder`
...
35. [DONE] **Carry seven records into the decisions that hold their choice** (the open-decision count in this container is 7, not the 6 the acceptance predicts: the seventh is the one the orchestrator filed at step 16's skip, after this plan was written; no decision marker moved)
    - Executor: `coder`
