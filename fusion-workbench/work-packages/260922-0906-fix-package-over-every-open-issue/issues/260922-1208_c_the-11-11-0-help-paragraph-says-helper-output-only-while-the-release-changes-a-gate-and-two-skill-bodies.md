...
Resolved: by the commit that carries this line. The 11.11.0 paragraph of `skills/help/SKILL.md` `### 4. Update` now opens on one gate reading, three skill bodies and six helper outputs: one sentence says the orchestrator answers the `ontocoder` gate and applies a curator ledger whole under the field where 11.10.0 filed an open decision and skipped, and names that as the one change a user meets at a gate; one names the three skill-body changes, the registry entry in a cleanup split, the post draft written to a file before the gate, and the news body no longer marking an unrendered entry seen; the six helper readings are kept and compressed into one sentence. Funded by the two cuts the plan named in the same file: the *Older than that* sentence about the v10 note's copy-the-budget step, which stated of itself that it no longer applies, and the duplicate working-model pointer at the end of topic 2, restated verbatim at `:41`. The skill surface moves 227 956 to 227 974 bytes, 18 net against 72 of head-room, so step 10's cut was not taken forward.
