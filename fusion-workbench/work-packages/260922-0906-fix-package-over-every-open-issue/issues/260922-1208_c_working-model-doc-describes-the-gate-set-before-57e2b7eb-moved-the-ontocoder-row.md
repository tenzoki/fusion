...
Resolved: by the commit that carries this line. `docs/working-model.md` `## 3. The gates` now describes the gate set `57e2b7eb` left: the field answers an ordinary `ontocoder` task alongside the plan review, the claim, the finish and the stop-conditions read (`:105`); the ontology bullet keeps the stop and splits it, an ordinary data task proceeding while a structural change to entities, relations or schemas still files and skips (`:111`); and the paragraph below names the file-and-skip set as three rows, adds the two further readings of `57e2b7eb` (a curator ledger applied whole under the field, and the held item's pause confirmed by the instruction to claim another), and cites `agents/orchestrator.md` `## Human Gate Rules` as the authority for which rows the field answers. `agents/orchestrator.md` was not touched.
