...
6. [DONE] **C5 — every gate that reads `CLAUDE.md` follows its text or the text stays**
   - Executor: `coder`
   ...
   - **Partially landed 2026-09-16, first bullet only; the step stays `[IN PROGRESS]`.** The skill-roster retirement is in: `claudeMdDrift()` and its two `it` blocks are gone from `hooks/lib/__tests__/derivable-enumerations-lint.test.ts`, replaced by a six-line comment naming the assertion that now carries the closed direction and the record that moves the passage. **Both redundancy claims were verified against the file before anything was deleted**, because the justification is redundancy and not convenience: `it("README-agents' skill table has exactly one row per skill directory")` compares the parsed rows, sorted, to the sorted directory list with `toEqual`, which is the closed enumeration in both directions, and it is non-vacuous at 13 rows against 13 directories; `it("no shipped doc cites a phantom skill")` names `CLAUDE.md` in its surface list and carries the open-set direction over it. What the retirement drops, stated rather than left unsaid: the completeness direction **over `CLAUDE.md` specifically**, so until step 8 lands, a new skill directory that `CLAUDE.md` never mentions is caught by no gate. That is the property this move retires, and it is retired knowingly.
   - **Measured, for step 5.** The `hook-tests` line surface: **21 814 lines before, 21 790 after**; floor 19 228 and head-room 2 595 both unmoved, so the budget stands at 21 823 and the **margin goes 9 -> 33 lines**. **This retirement freed 24 lines, not the 31 estimated above** — the estimate counted the deleted block and not the comment that replaces it. Step 5's helper test therefore has **33 lines** to spend, and stops for a head-room raise only if it needs more than that. Measured by summing `wc -l` over every `.ts` under `hooks/lib/__tests__/` against the baseline map, which is the arithmetic `growth()` performs; the before-figure of 9 equals the margin the `BASELINE` comment in `reference-resolution-lint.test.ts` independently recorded, and that agreement is what validates the method.
   - **The reference-resolution pin did not move.** That gate is green against `BASELINE = { paths: 1560, anchors: 239, stampBare: 11 }` unedited, so no re-approval was written and the existing comment line is untouched. The workbench citation gate is green too: the one record citation the new comment adds resolves.
   - **One file beyond this step's named set was written:** `hooks/lib/__tests__/fixtures/surface-growth.golden`, regenerated with `UPDATE_SURFACE_GOLDEN=1` because it records the per-file line counts this cut changed and the suite is red until it does. Two lines moved, both inside the `hook-tests` block and both downward (`derivable-enumerations-lint.test.ts` 449 -> 425, total 21 814 -> 21 790). No baseline and no head-room constant was edited, and the golden's own header states that regenerating it moves no baseline and clears no bound.
   - **Not landed, deferred to the session that runs step 8**, because each retargets a gate at text that has not moved: the `CLAIMS` rows for the agent counts, the `DEFINITION_SITES` `read("CLAUDE.md")` swap, the reference-resolution re-measurement, the workbench-citation verification and the dispatch-path re-measurement. Performing any of them now would point a gate at a file that does not yet carry the passage, and redden the suite over work that is correct but early.
   - Verification of the part that landed: `npm test` in `hooks/`, exit 0, 927 tests across 55 files.
   - **The rest landed 2026-09-16, after step 8's text moved, which is the order this step's own dependency line asks for.** The reference-resolution pin went to `{ paths: 1603, anchors: 267, stampBare: 11 }`, appended to the existing `BASELINE` comment line, no new line, attributed by swapping each of the three relocation files back to HEAD in place with everything else standing: the paths delta is additive with no residual (-92 on `CLAUDE.md`, +78 and +45 on the two READMEs), the anchors interact by exactly +3, and those three are named — the pointer anchors that dangle while their destination stands at HEAD. Two of the three `CLAIMS` rows were retargeted to `README-agents.md` rather than dropped, because the destination's own rows gate a different file and a different sentence, so dropping either would leave a live digit gated by nothing; that is the opposite of what this step anticipated, and it was checked before the retarget rather than assumed. The `DEFINITION_SITES` echo swapped to `README-hooks.md`, line-neutral.
   - **A sixth gate failed that this step never listed**, and it was answered by measurement rather than by a number: `REACH.excluded` in `hooks/lib/domain-cascade.ts` declared `CLAUDE.md` as `measured: "fires"`, and the file measures `clean` once the agent-roster bullet leaves it. The exclusion's stated reason has not expired — that identical bullet fires at `README-agents.md` now — so the entry was re-measured to `clean` with a note saying where the firing line went, and a `README-agents.md` entry added at `fires` so the cost stays asserted in the one block whose design is that its claims are measured. `REACH.fileSet` is untouched; nothing the cascade reaches changed. Regenerating `describeReach()`'s marker-delimited block in `README-hooks.md` was unavoidable and is the one write outside the dispatched file list.
   - All three test-surface edits are line-neutral (`derivable-enumerations-lint.test.ts` 425 -> 425, `reference-resolution-lint.test.ts` 978 -> 978), so the `hook-tests` surface stands at zero margin before and after and `surface-growth.golden` needed no regeneration.

7. [DONE] **C2 — the classification pass and the gate** *(new session: `fusion --update`, restart)*
   ...
   - **Landed 2026-09-16, and not through `/fusion:curate`.** That command passes no `**Placement:** on` line on either dispatch, so the run it names proposes no relocation at all and this step's endpoint was unreachable through it — the defect record `260916-1326_*_step-7-runs-fusion-curate-which-passes-no-placement-line-so-it-can-propose-no-relocation.md` states the gap and its two ways out. The orchestrator took option 2 and dispatched the curator directly with the parameter set, which is what the roster row already describes as the way a placement run is asked for, and costs nothing shipped. The criterion's authoring home was named in the dispatch prompt by hand, because the pointer that closes `260916-1157_*` landed in the same session and the agent roster is read at session start.
   - The survey classified all seven top-level sections: one stays whole, four split, two move whole. Ledger: 22 relocation entries, `L01` through `L22`, 59 308 bytes, and **not one deletion**, so the blast-radius stop fired at 94.9 percent of a file none of which was being discarded. Constraint removals, all three evidence tiers and consolidations were zero; two items were reported as candidates and never offered. The scale confirmation was put to the user ahead of the counts and accepted; all 22 were then approved.
   - Run file: `260916-1612-curator-run.md`, committed at `d17f440a` before anything was applied, so the classification survives whatever the gate had decided.

8. [DONE] **C2 — destinations first, then the source removal, then the measurements**
   - Executor: `coder`
   ...
   - Dependencies: step 7.
   - **Landed 2026-09-16 in the fixed order, and the order did its job.** The coder wrote all 22 After texts into their destination READMEs first, extracting each from the ledger's fenced block programmatically rather than retyping it, and verified every one as an exact substring occurring exactly once in the file its `**Destination:**` line names: 22 of 22, none failed. `CLAUDE.md` was byte-identical to HEAD at that point, proved with an empty `git diff --stat`. The curator's apply pass then ran its **own** preconditions rather than taking that report, found every before-text once in `CLAUDE.md` and every After text once at its destination, and removed the source passages: 22 applied, none stale, none failed, no approved entry without an outcome line.
   - **Measured.** `CLAUDE.md` 62 505 bytes over 130 lines to **8 114 over 66**, 54 391 bytes out, 27 pointer lines left behind. `README-agents.md` 46 064 -> 83 407; `README-hooks.md` 123 579 -> 146 328, of which 640 bytes are step 6's regenerated reach block rather than relocated text. `README.md` was no entry's destination and is untouched. All eleven dispatch paths fell by exactly 54 391 and every one is under its row, the tightest being `reviewer` at 110 237 against 189 012; `dispatch-path.baseline` and `DISPATCH_HEAD_ROOM` are both unedited. `skills/` stands at 3 bytes of margin and `agents/` at 49 666, neither touched by this step. `npm test`: 937 tests across 56 files, exit 0.
   - The residue is 8 114 bytes against the 5 000 to 7 000 this plan inferred. The difference is the pointer text, and the inference was never a target — C1's no-target-size decision is what held, and the gate decided passage by passage as it says.
   - One thing the ledger flagged and this step did not repair: candidate `C01`, a sentence claiming a lint asserts a direction no gate asserts, moved with `L04` as it reads and is still false. It belongs to the open defect `260916-1315_*_the-retired-gates-redundancy-argument-covers-one-direction-and-claude-md-still-claims-the-other.md`, which now names text in `README-agents.md`.
