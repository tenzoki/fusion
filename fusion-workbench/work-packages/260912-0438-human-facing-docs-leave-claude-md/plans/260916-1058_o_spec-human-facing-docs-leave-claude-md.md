...
- **The binding size surface was not the skill bodies but the hook-test line surface**, `hooks/lib/__tests__/**.ts`, and this clause is written after the fact because the work met it as a stop rather than as a prediction: C4's drift check could not ship with a test inside the room left, and the user ruled a head-room raise — `TEST_LINE_HEAD_ROOM` 2 595 -> 2 693, logged in `README-hooks.md` `#### The head-room raises, and the reduction read on 2026-10-10` — over hunting a 98-line cut elsewhere in the suite. Measured at `git:e8b455d0`, after that raise: the surface stands at 21 921 lines against a budget of 21 921, a floor of 19 228 plus head-room of 2 693, so **no margin remains** and the next line added anywhere in the suite turns the suite red. Every figure here carries the commit it was measured at, because this margin moved more than once on the day the work ran and an unstamped number on this surface is false within the hour.
