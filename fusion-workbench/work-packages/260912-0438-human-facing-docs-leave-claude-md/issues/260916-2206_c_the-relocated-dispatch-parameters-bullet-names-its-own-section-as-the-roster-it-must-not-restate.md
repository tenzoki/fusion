...
---
Resolved: the bullet at the end of `README-agents.md` `## Dispatch parameters` is reduced to the two sentences the section said nowhere else (a caller outside the orchestrator writes the parameter lines itself; the mechanism dates from v1.11.0); everything else it carried already stood in the roster table and the two paragraphs above it, and the self-pointer and the do-not-restate argument go with the duplicate. Two facts the cut would have lost are kept in place: "option 3" on the editor-halt paragraph, and the `reconciler` row's accepted values spelled as `code` | `data` instead of "the same two", whose antecedent row went at v11. The five citation tokens the bullet carried were duplicates, so the reference lint's pin moves 1679/290 to 1676/288 with a re-approval entry on its existing line. Concept accepted by a consultant read with changes; fixed in the commit that carries this line.
