...
**Cross-references:**
`260804-1602_*_guard-enabled-false-from-the-project-layer-turns-off-the-branch-policy-and-an-active-halt.md` (the measurement that raises this),
...
`260802-1912_*_does-the-self-protection-floor-apply-before-the-config-file-exists.md` (the floor this bypasses)
...
---
Retired: `fab8a4b` (`260816-1915_*_the-compliance-guard-becomes-observation-only.md` step 7a) — `guard.enabled` no longer exists at any layer, so the question of which layer may set it has no subject. The key was retired inside its container by the user's answer to `260816-1915_*_how-much-of-the-configuration-loader-survives-when-its-only-leaf-is-the-turn-budget.md`, option 1: `RETIRED_TOP_LEVEL_KEYS` in `hooks/lib/config.ts` now names `guard` whole, and `validateLayer` no longer takes a layer kind, because the project-only refusal this record chose has nothing left to refuse. The diagnostic this record made non-optional survives one scope up — a project that still declares `guard` earns exactly one advisory naming the container — so the property that mattered, that an ignored key is reported rather than dropped in silence, holds for the whole of what this key was inside. What the answer protected is moot rather than reversed: a project cannot switch off a guard it is governed by because the guard decides nothing to switch off (`260812-1232_*_does-the-escalation-counter-survive-a-block-source-that-ships-inert.md`).
