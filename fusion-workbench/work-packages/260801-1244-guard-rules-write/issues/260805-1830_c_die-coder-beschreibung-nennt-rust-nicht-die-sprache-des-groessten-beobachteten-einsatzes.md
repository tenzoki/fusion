...
Left open and filed separately: the routing table at `agents/orchestrator.md:346` lists the build manifests without `Cargo.toml`. That is a gap rather than a contradiction, because the ontocoder row restricts its `.toml` to ontology, manifest and schema directories and the table's tiebreaker points at the coder. The file was outside this task's file set, so the proposal lives in `260811-1301_*_the-orchestrators-routing-table-omits-cargo-toml-from-the-build-manifests.md`.
