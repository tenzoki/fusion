...
---
Resolved: 2026-08-06 — `shared_of` iteriert über `$(printf '%s\n' "$1")` (beide Shells feld-splitten eine unquotierte Command-Substitution; `skills/archive/SKILL.md:51`); zusätzlich Leerderivat-Check mit benanntem Abbruch statt "nothing to archive". Unter `zsh -c` und `bash -c` verifiziert. Commit 7ef2715, Circle 260805-2005-textschicht-gegen-code-nachziehen (Plan-Schritt 4).
