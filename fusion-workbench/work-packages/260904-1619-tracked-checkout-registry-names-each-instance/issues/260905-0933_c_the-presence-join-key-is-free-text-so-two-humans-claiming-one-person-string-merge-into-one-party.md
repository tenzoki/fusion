...
---
Resolved: the commit that carries this line takes the record's first branch, a report rather than a header statement of acceptance, because it is symmetrical with what `register` already does for the alias: the same loop now prints `person-collision=<stem>` for every other entry holding this entry's `**Person:**` under a different `**Git identity:**`, and stays silent where the identity is the same, since that is one human on two machines, which is what the field is for. The header's `register` entry names the line, and its collision section gains the paragraph that says why this collision is not harmless the way the alias one is (the person is the one registry value that enters a comparison, and `presence` merges two parties on it), that it is reported and never enforced for the reason the section already gives, and what the caller does with the line. The scratch registry of the acceptance test is the new case in `hooks/lib/__tests__/fusion-checkout-name.test.ts`: two other entries carrying one `**Person:**`, one under a foreign git identity and one under this checkout's own, and `register` prints the one `person-collision=` line at exit 0.
