...
**Cross-references:** `260822-1136_*_two-definitions-of-the-turn-count-disagree-and-the-resume-snippet-counts-every-session-in-the-log.md`, assigned to capability C4; `rules/workbench-tracking.md` `## The event log carries a union merge driver`, which states the cost this record is about
...
---
Resolved: referred (C4) — a plan input for C4: sort by `ts` before any session window, or find a window that does not depend on file order; 260822-1136_*_spec-fusion-becomes-a-multi-user-tool.md `### C4`

---
Resolved: `97407df` — the direction this record set for C4 is discharged, and it is discharged by the second half of it rather than the first. `hooks/lib/events-query.ts` `countTurns` scopes the merged log to the reading checkout **before** it does anything else, sorts what remains by `ts`, finds the **first** `session_start` naming this session's history file among those lines, and counts every `turn_start` whose stamp is at or after that anchor's **stamp** — a timestamp inside one checkout's own block, never a position in the merged file, and never the *last* `session_start` the file happens to hold. So the window does not depend on file order at all, which is what this record asked for after establishing that sorting alone does not separate two checkouts' sessions. Both positional sites the record names were converted in the same range: `agents/orchestrator.md` Phase 2 step 3 and the `progress.turn` row of the Persistent-State derivation table now state the definition once and cite `bin/fusion-events turns` as its one implementation, and three further sites were found and converted with them (`6deeb33`). The record's own caution stands as written and was followed: the derivation `260822-1136_*_two-definitions-of-the-turn-count-disagree-and-the-resume-snippet-counts-every-session-in-the-log.md` proposed — find the last `session_start` and count after it — was **not** taken. Criterion 6 of `260825-2140_*_c4-presence-travels-and-the-monitor-reads-its-own-checkout.md`; the marker was already `_c_` from the referral, so this note is appended beside the earlier `Resolved: referred (C4)` rather than replacing it, the same way its sibling `260823-1302_*_…` carries both.
