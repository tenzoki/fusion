...
---
Resolved: the commit that carries this line splits the one `continue` in `computeWorkGraph` (`hooks/lib/work-graph.ts`) into two: a terminal status stays silent by the G1 ruling, and any other unreadable or garbage `**Status:**` on an item-form record is pushed to a new `unreadable` list, counted as `unreadableHead`, printed by `hooks/order.ts` as `unreadable-head=` in the `KEY=value` block and one `unreadable=<container>` row each (the count-plus-name shape part one of the acceptance allows), and documented in the module header, the entry point's header and the `bin/fusion-work-order` header; a readFileSync that throws still skips in silence, since that is the terminal Circle container. Part two: the fixture gains an item-form record whose head is opened and never closed, so only the body heading bounds it and the decoy field below that heading would be read as an edge; with the `/^#{2,6}\s/` line deleted the suite goes red (verified by mutation, two cases fail). Of the two shapes named, the unclosed head is the one taken, because it reaches the heading branch; a record with neither closing `---` nor heading never does.
