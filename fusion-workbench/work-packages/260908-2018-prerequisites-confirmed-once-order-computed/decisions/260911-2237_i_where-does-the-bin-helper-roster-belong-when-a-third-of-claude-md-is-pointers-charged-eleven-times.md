...
---
Implemented: 4ba42bd8 — the roster moved to `README-hooks.md` `### The bin/ helper roster`, `CLAUDE.md`'s Layout row for `bin/` became a 307-byte pointer, and `hooks/lib/__tests__/derivable-enumerations-lint.test.ts` section 7 was retargeted at the new home in the same commit. Verified at `92cd2491`: 23 roster rows against 23 files in `bin/`, the lint reads `pluginRoot/bin` against `README-hooks.md` and asserts both directions.
