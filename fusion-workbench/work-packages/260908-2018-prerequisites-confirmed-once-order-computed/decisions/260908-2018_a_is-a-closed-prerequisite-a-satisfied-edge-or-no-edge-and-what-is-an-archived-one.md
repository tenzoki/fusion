...
---
Answered: 260911-1916-re-grounding-three-open-decisions.md `### F4. The closed-prerequisite record` — the record as filed was not answerable: all three of its options speak about an edge's target, while the store's only edge runs from a `done` dependent to a `claimed` target, so none of them reached the live case. Answered on the three-part split that report substitutes. The dependent side (3c-ii): an item whose `**Status:**` is `done` or `dropped` is not a node, and its outgoing edges are not read. The target side and the resolution side follow by implication rather than by separate ruling: a `done` target is not a node either, so an entry naming one is a dangle, reported by name with the node kept and the edge dropped; and the node map reads `circles/` only, never `archive/**/circles/`, which is 3b-i and what the plan's step C1 already specifies. Accepted consequence, stated at the gate: depth and blocking counts measure unfinished work only, so the graph is blind to what has closed. Immediate effect on this corpus, also stated at the gate: one node and zero edges, because the one candidate edge names a `done` item; ruled by user, Kai Stalmann <ks@qantr.com>.
