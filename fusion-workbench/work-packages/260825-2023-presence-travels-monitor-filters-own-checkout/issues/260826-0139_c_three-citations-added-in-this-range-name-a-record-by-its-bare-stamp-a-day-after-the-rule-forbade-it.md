...
- `CLAUDE.md:43` — "the call site guards with `[ -x ]` per decision `260810-0921_*_how-should-a-prompt-call-a-bin-helper-that-the-installed-copy-may-not-have.md`"
...
---
Resolved: All three now carry the full workbench-relative path with the marker wildcarded, and each was checked against the tree before it was written. `CLAUDE.md:43` twice — the issue as `260823-1302_*_the-monitor-attributes-a-merged-event-log-to-one-session-and-reports-another-checkouts-state.md`, the decision as `260810-0921_*_how-should-a-prompt-call-a-bin-helper-that-the-installed-copy-may-not-have.md` — and `README-hooks.md:192` once, the same issue. The stamp `260823-1302` is carried by three files in that Circle (the issue, an analysis and a history entry), so it is the collision the rule was written against and the prose's word "issue" is what picked the one meant. Nothing else moved: the pre-existing bare stamps in the same two tables, `CLAUDE.md:44`'s second `260810-0921_*_how-should-a-prompt-call-a-bin-helper-that-the-installed-copy-may-not-have.md` among them, were left standing, per the scope this record set. `cd hooks && npx vitest run lib/__tests__/workbench-citation-lint.test.ts` exits 0; the sibling `reference-resolution-lint` pin did not move, because a workbench record citation registers in neither of its two counted classes.
