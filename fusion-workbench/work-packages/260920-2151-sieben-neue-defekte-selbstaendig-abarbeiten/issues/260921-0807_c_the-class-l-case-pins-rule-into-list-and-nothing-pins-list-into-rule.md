...
---
**Resolved:** fixed in the commit that carries this line, for the test half. `LIVE_STATE` and `LIVE_PREFIXES` are exported from `hooks/lib/staging-drift.ts`; `staging-drift.test.ts` gains the case "holds every live-state row to an entry of the rule's L, R2 or R3 row", which parses the three rows the way the class L case parses one, collects every backticked token, and names the offending entry on failure (verified on a scratch copy of `hooks/dist/lib` with `bogus.md` appended: fails naming `bogus.md`). 12 lines, inside the hook-test bound with no cut. The second observation, the two layout-tree lines in `rules/fusion-workbench-conventions.md`, is closed by the other commit of this package that edits that rule file, not by this one.
