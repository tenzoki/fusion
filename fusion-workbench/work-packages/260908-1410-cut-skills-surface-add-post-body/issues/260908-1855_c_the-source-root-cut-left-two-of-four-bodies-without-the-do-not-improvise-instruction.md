...
---
Resolved: the exit-2 paragraph of `bin/fusion-source-root`'s header now carries the behaviour beside the report: a caller reports the root as UNRESOLVED, reads nothing through it, writes nothing in its place from memory, and does not improvise a file it could not open; what it says instead is each caller's own. Of the record's four bodies, `skills/next/SKILL.md` was deleted at `2a785ba2`, `skills/cleanup/SKILL.md` stopped citing the helper at `115be68d`, `skills/help/SKILL.md` carries its own rule and cites the header, and `skills/setup/SKILL.md` cites the header for the branch, which now carries the rule. `skills/news/SKILL.md` carries and cites. Two callers outside the record's four neither carry nor cite: `skills/check/SKILL.md` reads no prose through the root and states its own report, and `skills/archive/SKILL.md` reads `rules/workbench-tracking.md` through the root with no UNRESOLVED branch, filed as `260918-1234_*_the-archive-body-reads-a-rule-through-the-source-root-with-no-unresolved-branch-and-no-do-not-improvise-rule.md`. No skill body is touched. Concept accepted by a consultant read with changes; fixed in the commit that carries this line.
