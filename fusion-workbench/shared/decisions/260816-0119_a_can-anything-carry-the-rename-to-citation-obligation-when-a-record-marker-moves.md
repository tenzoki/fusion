...
---
**Reconciliation 260921-2230 (reconciler, domain `code`, HEAD `cb8776f3`) — marker unchanged at `_a_`.** The no-op answer still has no commit to cite, so `_i_` stays unreachable on the rule as written. What moved is the ground under the question: the wildcarded `_*_` citation form is normative (`rules/fusion-workbench-conventions.md` `## Filename Patterns`), `bin/fusion-citation-sweep` rewrites store-prefixed citations, and `80e8df95` makes the write-time check report a citation that spells a record's current marker. A marker rename now stales nothing written in the mandated form, which retires the obligation rather than carrying it. This is the user's call: accept the class of one as never-realisable, or file the dissolution as the answer.
