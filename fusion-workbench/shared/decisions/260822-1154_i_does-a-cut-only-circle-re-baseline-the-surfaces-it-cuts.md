...
---
Implemented: b90590fc — `hooks/lib/__tests__/helpers/growth-bound.ts` `## Re-baselining` carries the refusal at event 1, with the four-surface measurement as its argument rather than a caution, and sets the head-room raise out beside the three events without numbering it among them, since three move a floor and that one moves no baseline at all. Both residuals are in the text. The rule was funded by a cut rather than a raise: its record half moved to `README-hooks.md`, which sits on no bound, leaving the surface at 21 820 lines against an unchanged budget and a margin of 3 where it had 0.
