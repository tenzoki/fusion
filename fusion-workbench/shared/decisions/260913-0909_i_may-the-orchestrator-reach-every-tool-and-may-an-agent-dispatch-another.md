...
---
Implemented: c066bfd3 — the three sentences banning the orchestrator from dispatching the consultant are deleted from `agents/consultant.md` and `agents/orchestrator.md`, 309 bytes given back. The consultant's exclusion is now prose on the same footing as every other agent's, and the surviving recursion bullet was re-worded from two entries to one in the same commit. The consultant is not added to the routing table: that table answers which executor a task goes to, and the consultant is not one.
