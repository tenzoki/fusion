...
---
**Reconciliation 260921-2230 (reconciler, domain `code`, HEAD `cb8776f3`) — marker unchanged at `_a_`.** Option 1 is now realised on the prompt surface: `agents/orchestrator.md` `## Review coverage` states that `verdict=uncovered` never blocks and the closure note names the gap, citing this record, and it was applied at the 11.0.1 release (`751e7fa0`). Option 3 is still absent: `grep -in shipped hooks/lib/review-coverage.ts` finds nothing, so the uncovered set is filtered by coverage alone, sixth consecutive check.
