...
2. **No. Exclude comment and blank lines from the count, and recompute `TEST_LINE_BASELINE` under the new rule in the same commit.**
   ...
   - Cons: **recomputing the baseline is not one of the two re-baselining events**, and the user rejected declaring a third (`260822-1102_*_what-happens-when-a-planned-circles-required-work-exceeds-the-remaining-head-room.md`, option 3). Taking this option without recomputing is worse: total would drop 20 363 to 11 544 against an unmoved floor of 17 875, absolving every line of growth the surface holds. It also needs a comment classifier that agrees with itself across JSDoc, block comments, and a string containing `//`, which is a new mechanism on a surface whose value is that its measurement is a single shell command.
