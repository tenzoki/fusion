...
Deferred:
Superseded by:
Retired: `260909-1700-cut-fusion-to-working-minimum.md`, commits `2a785ba2` and `76d833be` — the record kind is gone. `skills/next/SKILL.md` went at the first and `rules/circle-records.md` at the second, twenty-one minutes apart on the same day, so the pointer literal, the invariant and the sanctioned writer this answer provided have no record to sit on.
