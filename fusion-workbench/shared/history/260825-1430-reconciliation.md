...
- **The citation resolves.** `shared/issues/260825-1329_*_every-session-runs-one-release-behind-on-a-bin-helper-the-same-repository-just-added.md`
  is on disk and open.
...
- **The reason the annotation gives is true for every record it is written on.** All 31 stamps
  fall between `260824-1621` and `260824-2155`. `bin/fusion-identity` entered the work tree in
...
pass's own record, `shared/history/260825-1241-reconciliation.md`, is already in the window and
...
pass entirely. Written as `shared/history/260825-1430-reconciler-attribution-backfill.md`,
...
`shared/history/260825-0858-orchestrator-session.md` carried `**Directive:** (not yet stated)`,
...
|---|---|
| `260825-1019_*` gitignore partition | No step of `/fusion:setup` reads a project's `.gitignore` for anything but `.claude/settings.local.json`. The repair the answered decision commits to is unbuilt. |
...
| `260825-1259_*` Rebalance gate | `agents/orchestrator.md:992` still says four options; `rules/user-facing-output.md` `## Questions and gates` still caps a gate at three. |
| `260825-1329_*` helper lag | Every call site is still `"$FUSION_PLUGIN_ROOT/bin/<name>"`, and part (c) of decision `260810-1544` is still unanswered. |
...
rather than edited. `260825-1329_*` quotes the superseded counts. `260825-1250_*` on the
...
`260822-2236_*`'s `Resolved:` note refers the work to the backlog as a freeze detection over
...
`shared/history/260825-0858-orchestrator-session.md` under
...
`shared/history/260825-0858-orchestrator-session.md` reads four em-dashes against a ceiling of
