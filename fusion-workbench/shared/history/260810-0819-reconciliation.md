# Reconciliation — session `260810-0241`, Phase 3
...
(`## Close without work`: `260717-0031`, `260717-0115`, `260809-2255`). That proposal was written
...
say-so is not a reconciler's call. But they are inflating the open count by 3, and `260809-2255`'s
...
| `260807-1951_c_` depth bound | Cites the decision as `260809-1731_a_…`; the file is now `_i_`. Dead as a literal path — the instance `260808-0030_o_` predicts. |
| `260810-0457_c_` rebuild collision | Cites the corrected header at `bin/fusion-plane:99-104`; it now sits at `:113-122`. |
| `260810-0745_c_` golden cohorts | The correction table is right on every row; the prose sentence below it swaps shaper's and playmaker's totals (shaper is 101 624, playmaker 95 951). A correction that mis-states the thing it corrects. |
...
Filed: `shared/issues/260810-0819_o_head-carries-six-records-twice-and-the-class-fix-was-deferred-to-a-decision-never-filed.md`
...
"$WORKBENCH/$P/…"`, filed as `260810-0500` and fixed at four sites by `3df0c17`, whose own message
...
later by coderev and never cross-referenced back — `260810-0500_c_`, `260810-0501_o_`,
`260810-0506_o_`, `260810-0511_o_`. Of these `260810-0501_o_` is the consequential one: both skills
...
- **`260810-0502` — still accurate, and understated.** The record grants two of four call points as
  ...
  exists.
- **`260810-0503` — still accurate, with a fourth defeat.** The decoy, the inversion and the dead
  threshold all pass both helpers. So does a token appearing only in a **trailing comment** —
  `branchesFrom` keeps the whole line. `260807-1942` can be reinstated in full with the suite green.
  `assertAbsentCountFirst` still has no negative control (`:143-145` is a second positive assertion).
- **`260810-0510` — still accurate.** Emptying the body of `assertRidesTheAct` (`:130-140`) fails
...
|---|---|---|
| **HIGH** | `260810-0710_o_should-a-rule-be-allowed-to-land-without-the-check-that-enforces-it` (new tonight, `8d66265`) | Nothing mechanically — it governs how every future rule lands. Its own instance already cost a real defect in the same Turn that wrote the rule: `e99f0ef` wrote the convention, `ff70d3a` broke it three commits later. Should be answered with §6's cohort in view. |
| **MEDIUM** | `260809-1224_o_is-the-decision-governed-escalation-check-3-a-live-feature` (20h) | The only open decision a written plan names as its blocker. `shared/planning/260809-1229_c_…:366,412` records C5 as "still inert … blocked on" it; ~60 lines in `hooks/lib/config.ts` can be neither removed nor tested until it resolves. Unblocking action is one file read. |
| **MEDIUM** | `260807-2131_o_which-language-governs-a-customer-deliverable` (3 days) | `agents/editor.md:16,62` ships a default the three-way boundary in `rules/fusion-workbench-conventions.md` `## Project language` does not cover. No rule text names the deliverable case, so every editor dispatch without an explicit language resolves by silence. |
| **LOW** | `260810-0718_o_should-rebuild-map-merge-with-the-existing-map-or-replace-it` (new tonight) | Nothing shipped — the immediate defect (`260810-0457_c_`) is closed and replace-with-loud-report is implemented. Hangs over three open siblings on the same flag (`260810-0746/0747/0748`). The record itself says to answer after the first real recovery. |
| **LOW** | `260806-1152_o_stash-manifest-dirname-and-pointer-content-duplicate` (4 days) | Nothing. The issue that carried the question is closed. Current state is coherent whichever way it goes; churn is the only cost. |
| *(out of shared scan)* | `circles/260807-0923-guard-misst-statt-orakelt/decisions/260807-0945_o_integritaet-des-eskalationsspeichers` | Sits inside a `_c_` Circle. No shared-store scan reaches it, and the 16 out-of-scope open defects in five closed Circles have the same property. |

### Grounding drift found while surveying

`260809-2310_a_should-the-branch-policy-fall-the-way-the-write-classifier-fell` is still `_a_` and
...
## 8. The `260809-1731` `_a_` → `_i_` transition — sound today, premature when made
...
- **The fifth was not.** The `Answered:` block forbids a silent zero. At `2910cf6:134-135`
  ...
  counted_by=git-ls-files`, exit 0. Filed as `260810-0459` and fixed only in **`ea492e6`** (07:07),
...
**One caveat on "shipped":** `260810-0352_o_setup-step-5-now-calls-a-helper-the-installed-copy-does-not-have`
...
| `260810-0508_o_` title and body | fifteen commits with no version bump | **22** |
...
`260810-0751_o_` — "the record about counting instances of a shape gives three different counts" —
is the Turn 2 reviewer filing this class against a Turn 1 record. The reviewer's own file then did
it twice. Filed as `shared/issues/260810-0820_o_the-turn-1-review-totals-table-says-fourteen-findings-and-the-body-carries-seventeen.md`.
...
cannot see it. `260810-0508_o_` covers this and understates it by 7 commits.

---

## Files written by this pass

- `shared/issues/260810-0819_o_head-carries-six-records-twice-and-the-class-fix-was-deferred-to-a-decision-never-filed.md` (new)
- `shared/issues/260810-0820_o_the-turn-1-review-totals-table-says-fourteen-findings-and-the-body-carries-seventeen.md` (new)
- Reconciliation evidence appended to `260801-2038_o_`, `260810-0502_o_`, `260810-0503_o_`, `260810-0510_o_`
- `## Coherence` appended to `shared/history/260810-0241-orchestrator-session.md`
- This file
