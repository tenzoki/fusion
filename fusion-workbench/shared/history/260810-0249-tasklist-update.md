...
|---|---|
| `260717-0031` (lint gate scope) | **Already resolved.** All four scope questions are settled inside `hooks/lib/__tests__/path-literal-lint.test.ts`, each with its reasoning in a comment. The record's secondary claim, that playmaker's `description` was stale, is also fixed. Closed without work. |
| `260717-0115` (two layouts at once) | **No longer reproducible.** The workbench root holds no pre-v4 type folder; the migration completed. Closed without work. |
| `260809-2255` (branch-policy halt) | **Overtaken on both criteria.** `escalation.json` reads `haltActive: false, consecutiveBlocks: 0` with two `halt_cleared` events, and commit `7598073` deleted the branch policy outright, so the second criterion has no subject. Closed without work, with one residual noted (the clearing is not written into a session history file). |
...
themselves: `260807-1942` and `260807-1951` edit the same nine-line domain-detection block but
