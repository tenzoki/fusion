...
- Grounding↔Directive: 5 active decisions across `shared/decisions/` (2 `_o_`, 3 `_a_`), 5 consistent with the Directive, 0 conflicting. `260807-2131_*_which-language-governs-a-customer-deliverable.md` is a gap the Directive opened and deliberately left open rather than a contradiction of it; the other four are unrelated to the language boundary. `260807-1515_*_`, the decision the Directive existed to realise, is now implemented and its own stated condition is met.
