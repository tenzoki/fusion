...
5. **Narrow the open frozen-stores decision to what is still open, and file the residual**
   - Executor: `analyst`
...
6. **The tripwire: no rewrite may turn a token the checker reports into one the checker cannot see**
   - Executor: `coder`
...
- [ ] Should an archived violation move the checker's `verdict=` line? Filed as a new `_o_` record by step 5, once step 4 has produced the figures it turns on.
