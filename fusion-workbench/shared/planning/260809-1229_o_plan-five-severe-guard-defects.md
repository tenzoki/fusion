...
### Step 5 — Actor identity: preserve the bytes, and stop asserting a cause the hook cannot know
...
- [ ] **Should the revert itself narrow for the four write tools, not only the message?** Step 5 takes the decidable distinction (a changed path other than the payload's) and uses it only to describe. Using it to *skip* the revert would reduce the destruction of concurrent work, and would also mean an agent-caused change to a second protected path goes unreverted. That trade is a user decision and belongs in a decision record, not in a defect fix.
...
- [ ] **Retention for the preserved copies: 20 files, or an age bound, or both?** Proposed as a count because a count cannot expire something the user still needs during a long session. Cheap to change.
