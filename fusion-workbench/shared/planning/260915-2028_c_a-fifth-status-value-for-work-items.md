...
**Date:** 2026-09-15
**Status:** Complete — steps 1 through 8 done. Released as 11.3.0.
...
8. [DONE] **The release surfaces, at release time only**
   - Executor: `coder`
   ...
   - **Landed 260915 for release 11.3.0.** The four version surfaces carry `11.3.0`: `.claude-plugin/plugin.json`, the fusion entry in the marketplace clone's `.claude-plugin/marketplace.json`, and the `FUSION_REF=tags/v<version>` example in `install.sh`'s header and in `README.md`. The update topic gained its paragraph and dropped the 11.0.1 one, so it carries 11.3.0, 11.2.0 and 11.1.0; the per-install labels needed no relabelling, being keyed on the install a reader comes from rather than on position. **No `docs/upgrading-to-11-3.md` was written**: the release removes nothing and rewrites no record, and the help paragraph says so where a reader meets it. The two `description` strings were read side by side and are still character-identical and still true — neither states the status cardinality — so neither was rewritten.
