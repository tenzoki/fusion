...
afterwards, and not one carries a Class: block. The bullet is an instruction, not a mechanism, and
the next agent to file simply did not apply it." The idea generalises the example — a norm written
as prose into `CLAUDE.md` or a rule file governs nothing on its own, so a rule either lands together
with something that executes it at the moment of the act, or it does not land at all. The dump's two
examples of agents passing unverified claims to the user are the same idea's symptom, and this is
its cause.
