...
**Cross-references:** issues `260811-1610` … `260811-1617` filed by this pass; open records `260811-1301`, `260811-1149`, `260811-1547`, `260811-1146`, `260811-1522`, `260811-1534`, `260810-1135`, `260811-1409`
...
| `260811-1406` → `_c_` | 75 | 105 | 105 | same |
| `260811-1407` → `_c_` | 63 | 95 | 95 | same |
| `260811-1408` → `_c_` | 54 | 75 | 75 | same |
| `260811-1410` → `_c_` | 52 | 86 | 86 | same |
| `260811-1411` → `_c_` | 51 | 72 | 72 | same |
| `260811-1412` → `_c_` | 46 | 66 | 66 | same |
| `260811-1413` → `_c_` | 38 | 57 | 57 | same |
...
**The reachable-but-discarded half.** `260811-1610`, **High**. Both `unmeasured` branches print the
...
into all four cells. This is `260811-1406`'s own closing standard failing in the other direction, and
its reach is **wider** than the defect it replaced, not narrower: `260811-1406` fired for Circle
...
**The mis-assigned cause.** `260811-1616`, **Low**. `:649` lists "a project outside git" under
...
(`shared/history/260811-1502-…:64`): *"that `unmeasured` fires for exactly the two causes"*. The
suite asserts that two constructed causes produce the right `why=`; nothing asserts exhaustiveness,
and `260811-1616` above is a case the enumeration misses. Not filed — it is one sentence in a
...
`260811-1611`, **Medium**, and this is the question the dispatch asked.
...
Answer `260811-1149` by narrowing that regex — which is what `1149` is filed against — and the
...
`260811-1613`, **Medium**. The rule the four prompts now state is right, and stating a rule instead of
...
tiebreaker. That is `260811-1301`, still `_o_`. Its stated reason for being a record rather than a
...
`260811-1612`, **Medium**. The `noise=` key is in the right position (`absent=` then `noise=` then
...
session in this repository reads first, and it is the same shape as `260811-1413`, which this Turn
...
`260811-1615`, **Low**. Putting the bound in `rulesWriteDetail` rather than in a new `lib/events.ts`
...
| the log has an upper bound or documented rotation, and the monitor stops re-parsing the whole file | **not done**, in `260811-1534` |
...
clauses are both *in* `260811-1534`, one as an option set and one as a constraint, and the record's
...
`260811-1617`, **Low**. `260811-1547` is correct that `/fusion:monitor-reset` does not exist and that
...
|---|---|---|
| the churn output contract (`260811-1612`) | 4 surfaces + 2 README rows | `CLAUDE.md:33` |
| the `.toml` boundary (`260811-1613`) | coder, ontocoder, planner, README-agents | `agents/orchestrator.md:346`, now cited as the authority |
| the widened gate's dependency (`260811-1611`) | a comment | the assertion |
...
set) was used once here, for `260811-1547`. What it did not reach is the case where the surface left
...
with that sentence about `260811-1406`. It holds a second time, one branch narrower:
`260811-1406` was "a number that could have been taken and was not, reported as unmeasurable"; the
...
`260811-1547`. Turn 2 found the same asymmetry between measurement and generalisation. The pattern is
...
**4. The session's own event log froze in the Turn about measured bookkeeping.** `260811-1614`,
...
1. `260811-1610` (High) — the `filed` count. The fix is the one `if` that splits on what each half
   needs, and it has to replace `record-counts-measurement.test.ts:271` rather than only add a case.
2. `260811-1616` — the cause list, same paragraph, same edit.

**Before anyone answers `260811-1149`**

3. `260811-1611` — the exemption dependency. Answering `1149` without it means discovering the two
   prompt lines from a red suite in a different file.

**Before the next dispatch that routes a `.toml`, and in the same edit as `260811-1301`**

4. `260811-1613` — the routing table. `260811-1301` is one line in a file this Turn already edited
   twice; the reason it was deferred no longer applies.

**Documentation, before the next Setup read**

5. `260811-1612` — `CLAUDE.md:33`. Cheap, and it is the first text a session in this repository reads.

**Cleanup, any time**

6. `260811-1614` — the drift check's fifth row. Not urgent, but it is the mechanism the last two
   Turns were built around and it currently cannot see the failure it was built for.
7. `260811-1615`, `260811-1617`.
