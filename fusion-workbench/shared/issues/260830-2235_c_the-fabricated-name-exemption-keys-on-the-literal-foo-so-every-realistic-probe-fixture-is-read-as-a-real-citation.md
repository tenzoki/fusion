...
---
Resolved: closed on the bound rather than on a new exemption. The `FABRICATED_NAME` docstring in `hooks/lib/citation-scan.ts` now states that `foo` is the one placeholder the grammar reads because it is the one decidable one; that a fixture written to look like a record is judged like one and reported `dangling` until it is fenced or restated naming file and line (`rules/fusion-workbench-conventions.md` `## Marker globs`, which already binds every agent to that); and that no wider list is admitted, a second placeholder word being the substring thicket one level up and a context-keyed exemption reading intent. The acceptance this record asked for, a quote that is safe without the writer knowing the fence, is not met and is refused with the reason: no decidable property keys it (`rules/critical-stance.md` §4). The residual is stated in the docstring: a writer meets the `dangling` row at the next scan and repairs it once. No code changed; `dist/` rebuilt. Working answer per `260921-1718_*_which-decidable-property-if-any-exempts-a-realistic-probe-fixture-from-the-citation-gate.md` option 1, marker untouched. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 15, fixed in the commit that carries this line.
