...
---
Resolved: the grammar gained the status `spelled-marker` (`hooks/lib/citation-scan.ts`, the `BARE_RE` branch): a bare-record token whose literal lookup finds exactly one record and whose marker slot spells a letter is that record cited under its current marker, and the hit carries `problem` "spells the marker '_c_', which the record's next transition invalidates" and `fix` "cite the marker position as '_*_'", neither with a fusion stamp. `REPORTED_STATUSES` in `hooks/lib/citation-form.ts` names it beside `store-prefixed` and `stale-marker`, and the header's split-by-what-the-verdict-is-decided-from gains the bullet: decided by a lookup that found the record under the spelled marker, no invented name reaches it. `partition()` and `scanRecordCitations` count it as resolved, so `bin/fusion-citation-check` prints the same `dangling=`, `store-prefixed=` and `verdict=` and `bin/fusion-citation-sweep --dry-run` the same `rewrites=0` (17 such tokens stand in this workbench today, all inside `resolved`). Acceptance case: `citation-form.test.ts` "reports a citation spelling the record's current marker", the `CLOSED_ISSUE` fixture cited as `CURRENT_MARKER` with its `c` written out, reported `["spelled-marker"]` with `_*_` in the sentence; the `dangling`-silent, wildcard-silent and fenced-silent cases pass unchanged. The closure note of `260906-0115_*_three-agents-in-one-session-wrote-a-citation-the-always-on-rule-forbids-and-only-a-later-gate-caught-it.md` needs no edit: its fallback branch was for the class staying unreported. Second opinion: consultant, accepted. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 19, fixed in the commit that carries this line.
