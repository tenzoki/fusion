...
---
Resolved: `rules/fusion-workbench-conventions.md` `## Dispatching another agent` now bans the thing the ruling banned and nothing wider — no agent answers a gate on the user's behalf, and no agent writes a `gate_response` for an answer nobody gave — and names the party that does write it: the one that put the gate to the user, which is the orchestrator. The three mandates in `agents/orchestrator.md` (`## Setup`'s dispatch loop, the stop-conditions gate, the event table) are left as they stand and no longer contradict the rule; the two literal strings the stop-conditions gate pins are untouched, so the corpus `260817-1613_*_does-a-plan-stated-precondition-get-any-mechanism-or-is-it-read-by-a-human-or-not-at-all.md` reserves a measurement over is not split, and `skills/cadence/SKILL.md` keeps its reading. The edit costs 158 bytes, charged to all eleven dispatch paths; the tightest, `reviewer`, went from 27 596 bytes of margin to 27 438.
