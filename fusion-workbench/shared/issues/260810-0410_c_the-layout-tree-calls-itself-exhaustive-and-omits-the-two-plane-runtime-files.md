...
---
Resolved: the three Plane entries left the root-anchored layout tree in `rules/fusion-workbench-conventions.md` when the Plane mirror was removed (step 2 of Circle `260815-0007-remove-eight-mechanisms-and-cap-growth`). The tree no longer omits them because they no longer exist. Closed by the removal rather than by the extension the record asked for.
