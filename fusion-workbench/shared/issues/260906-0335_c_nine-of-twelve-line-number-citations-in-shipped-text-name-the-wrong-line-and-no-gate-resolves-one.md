...
---
Resolved: the second of the two routes the acceptance offers, taken in two halves. Text half, commit `3db8e1a3` (plan step 3): the two `path:N` tokens the shipped surface still carried were rewritten as prose naming what they point at (the twelve measured here and the six the reviewer added had been cut or rewritten with the files they sat in before that step; none stands on the surface now, and the acceptance grep over `agents skills rules README*.md CLAUDE.md docs bin install.sh` prints nothing). Lint half, this commit (step 20): `scanPluginPaths` in `hooks/lib/__tests__/reference-resolution-lint.test.ts` refuses a resolved plugin path followed by `:N` or `:N-M` inside its backticks, with the fix "cite a heading anchor or name the symbol; a line number is not a pointer in living text", and the header's class list states it; made to fail on a probe `rules/commit-lock.md:12` token appended to `docs/philosophy.md` (restored) and passing on the tree, where it found no token beyond the two step 3 removed. Two `path:N` mentions remain in `hooks/lib/` comments, a records-only surface the path class has never scanned, and are outside this refusal by that scope. The project thereby states, in the gate, that a line number in shipped text is decoration rather than a pointer (the same reading `260905-1228_*_does-a-resolution-line-cite-path-line-or-a-heading-anchor.md` took for resolution lines). Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 20, fixed in the commit that carries this line.
