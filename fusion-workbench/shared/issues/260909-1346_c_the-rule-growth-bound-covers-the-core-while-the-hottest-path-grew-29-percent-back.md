...
---
Resolved: the hard universal-core growth bound was retired on 2026-09-11, and the per-dispatch-path byte bound in `hooks/lib/__tests__/rules-emission-golden.test.ts` is what answers this record. This finding is that the core bound could not see the two things that carried the regrowth — `CLAUDE.md`, which it never measured, and a conditionally emitted rule file such as the `bounded-dispatch.md` named above, which is outside the core intersection by construction — and the per-path bound measures exactly those, as each path's own prompt plus every file `bin/fusion-rules` emits for it plus `CLAUDE.md`, at zero head-room. It also charges an always-on byte once per path rather than once in total, which is why retiring the core bound opened no window: measured at `9ff123c5`, the core stood at 74 873 bytes against a budget of 77 498 (2 625 of margin) while the tightest path, `reviewer`, stood at 188 842 against a baseline of 189 012 (170 bytes), so the per-path bound already refuses every core addition the core bound would have refused, and 2 455 bytes earlier. Consequence 1 above — that the 2026-08-14 arming baseline silently created head-room the following fortnight spent — is answered by the arming of the per-path bound on 2026-09-09, taken deliberately before the cut it bounds rather than at it. Consequence 2 is answered by the per-path bound measuring the emitted set rather than the floor; what is left in the retired bound's file is the never-failing report on each role's own rule text, which still only reports. The argument is written into that file's header and into `README-hooks.md` `### Growth bounds on the shipped text`, and it lands in the commit that carries this closure.
