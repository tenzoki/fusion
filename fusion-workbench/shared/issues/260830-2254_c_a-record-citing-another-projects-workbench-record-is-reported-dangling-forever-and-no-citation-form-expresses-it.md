...
---
Resolved: A citation of a record held in another project workbench is now written foreign colon project colon citation, both leading segments literal and required. The qualifier is read out of the text ahead of the citing token and before any lookup, so the token is reported neither dangling nor store-prefixed, and it is deliberately outside the resolution-premised exemptions so it also reaches the shape-decided verdict: the storeless spelling rule is this project rule, and an archive sweep here moves nothing in another project tree. The cheaper form without the keyword was measured and rejected: a bare project colon prefix already occurs 214 times in this corpus as a legacy task-id spelling, every one naming a local record, so that form would have silenced 214 live citations. The form is written where a writer reads it, in the citation paragraph of the conventions. Not covered, and stated in the rule itself rather than hidden here: nothing mechanical separates a genuine foreign record from a local one a writer mislabelled. The qualifier is a claim by the writer, not a fact a gate checked. What bounds it is that the token is never counted as resolved, that a single grep enumerates every use, and that the two literal segments are a marker no ordinary spelling reaches. A test pins that limit explicitly, so nobody can later read the form as self-policing.
