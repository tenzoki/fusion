...
Reconciliation 260829-1109 (HEAD `7a2361aa`): still open. `rules/agent-setup.md` carries no line joining `OUT_*`/`SCAN_*` to `$WORKBENCH` (`grep -n 'joins\|relative to'` empty), so the acceptance is unmet. The stray root `shared/` is gone (`ls shared` at the root: no such directory), and the second release bump wrote its entry inside the workbench (`fusion-workbench/shared/history/260829-0906_coder_release-bump-v10-19-2.md`, commit `7a2361aa`), so the misplacement did not recur; the rule fix is what closes this.
