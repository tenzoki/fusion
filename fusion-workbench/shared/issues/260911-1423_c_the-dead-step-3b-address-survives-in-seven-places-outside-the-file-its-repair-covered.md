...
---
Resolved: every surviving `Step 3b` address in `hooks/`, `bin/` and the compiled `dist/` now cites `agents/orchestrator.md` `### Step 4 — commit` with its item number (item 3 for the commit-message file, item 4 for the staging-list shape, item 5 for the `git add` inside the held command): the tracker comment, the staging-drift test header, the commit-message-path test's doc comment, three test names and one assertion message. An eighth site the record did not list, the comment in `bin/fusion-staging-drift`, takes the plain form `Step 4 item 4` with its path left as it was, so the reference lint's pin does not move. In-place edits; both test files keep their line counts; `hooks/dist/tracker.js` rebuilt. Concept accepted by a consultant read with one change; fixed in the commit that carries this line.
