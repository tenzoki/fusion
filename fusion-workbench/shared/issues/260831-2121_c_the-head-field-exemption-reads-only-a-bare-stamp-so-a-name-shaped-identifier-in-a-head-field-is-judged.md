...
---
Resolved: in a head field, a `stamp-name` token that carries no `.md` and resolves to nothing is classified `undecidable` rather than `dangling` (`hooks/lib/citation-scan.ts`, the `STAMP_RE` branch; `consider()` passes `isHeadFieldValue` into `check()`; `partition()` puts the status in `undecidable`, `scanRecordCitations()` neither counts nor reports it; `bin/fusion-citation-check` prints it under `--undecidable`). The property is named in the scanner's header as a property, not a list of fields: the split is on the token's shape, so a `stamp-name` carrying `.md` or any value carrying a marker slot is a citation and stays judged, which keeps `**Active spec/plan:**` judged as this record's acceptance requires; the bare-stamp `head-field` exemption is untouched. One case in `hooks/lib/__tests__/citation-sweep.test.ts` pins both halves. Over this tree `dangling=301` and `undecidable=3610` before and after (no head-field `stamp-name` in this corpus resolves to nothing, measured by the consultant over the 764 `--undecidable` rows), `verdict=clean` unchanged. Working answer under `260831-2142_*_which-property-separates-a-head-field-identifier-from-a-head-field-citation.md`, its fourth direction; the marker there stays `_o_` for the user to rule. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 24, fixed in the commit that carries this line.
