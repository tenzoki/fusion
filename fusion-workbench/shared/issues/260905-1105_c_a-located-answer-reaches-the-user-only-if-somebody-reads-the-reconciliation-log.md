...
---
Resolved: 260905-1640-coder-the-located-answer-rides-the-record-the-listing-already-opens.md `## Where the join lives` — the reconciler appends `Answer located: <citation> — <summary>` to the record without renaming it, and the orchestrator's listing reads that line and names the location beside the question. The join survives because it sits inside the unit that gets archived, moved and cited. Landed at `e1e72f77`; the record was left open by the implementing agent because the suite was red on stale inventories at the time, and closes here with the suite green.
