...
---
Resolved: the `.active-circle` row leaves `LIVE_STATE` in `hooks/lib/staging-drift.ts`, with the example that justified it now naming `.session-marker`; ruled construction rather than consent by the consultant read, because `rules/workbench-tracking.md` says nothing creates the file and `/fusion:migrate` deletes one, no shipped writer exists, and the removal changes no verdict, signature or sentence (a leftover copy now classifies `unclassified` instead of `in-flight`, `fault` false in both). The trigger prose in the module and in the CLI wrapper names the two read points the orchestrator prompt has (`### Step 4 — commit` item 7 and `## Ending the session`), the runtime message loses "queue rebuild at Phase 1", and `hooks/lib/review-coverage.ts` states the pointer as retired and its two other present-tense Circle/Turn sentences as work item and session. Every remaining `Circle` or `Turn` in the three files narrates a dated event or the `_circle.md` branch, which stays accurate. Six dist files rebuilt. Filed beside it: `260918-1334_*_the-review-coverage-cli-header-names-step-3c-and-phase-4-which-the-orchestrator-prompt-does-not-have.md` and `260918-1335_*_the-live-state-comment-keeps-two-retired-rows-for-a-consequence-classify-cannot-produce.md`. Fixed in the commit that carries this line.
