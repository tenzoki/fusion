...
---
`skills/archive/SKILL.md:193-201` (filter 3) keeps a candidate out of a sweep when `grep -r -l -F -e "$bn" -e "$rel"` finds its basename or relative path in the shipped text. `$bn` carries the live marker (`260811-1534_i_...`); the form `rules/fusion-workbench-conventions.md` `## Filename Patterns` mandates spells `_*_` at that position, and `grep -F` matches no wildcard. So every record cited the way the rule says to cite it is invisible to the filter that exists to keep cited records out of the archive.
---
**Domain:** code
**Filed by:** analyst, Kai Stalmann <ks@qantr.com>
**Related:** `shared/analyses/260828-0859-citation-bookkeeping-defect-report-measured-against-fusions-own-corpus.md` (section "The archive safety filter cannot see the mandated citation form"); `circles/260826-1613-cardinality-answered-cut-once-nineteen-cleared/decisions/260827-1756_*_which-citation-corpus-does-the-archive-safety-filter-protect.md` (defines the corpus the filter reads, not the match); `circles/260826-1613-cardinality-answered-cut-once-nineteen-cleared/issues/260827-2042_*_the-archive-citation-corpus-reads-a-source-root-set-in-another-shell-and-collapses-silently-to-claude-md-rules-and-the-system-bin.md` (the previous defect in the same step)
