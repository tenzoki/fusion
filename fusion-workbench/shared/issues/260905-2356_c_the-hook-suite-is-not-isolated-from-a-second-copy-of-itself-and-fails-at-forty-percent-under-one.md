...
---
Resolved: the first acceptance branch, the repair. The mechanism the body names, `hooks/lib/git.ts` collapsing a timeout into the `null` every caller narrates as a decline, was repaired at `91515fd5`: `git()` now returns a distinct `GIT_TIMED_OUT` after retrying a first timeout once, `GIT_TIMEOUT_MS` is 10 000 per attempt drawn from the measured tail (finding 2 of `260906-0026-what-shared-state-the-hook-suite-reaches.md`), `GIT_STATUS_TIMEOUT_MS` 20 000, and each caller that had a sentence for the decline has its own for the timeout. The acceptance experiment was then run as this record prescribes: ten pairs of concurrent `cd hooks && npm test`, started together on an otherwise idle machine, at `73c11cfd` (the git helper byte-identical to `91515fd5`; that commit only wildcarded literal-marker tokens in two workbench files that were reddening the sweep gate). **0 red of 20**: every run 958 of 958, no failing file, no assertion carrying a timeout sentence, 64.7 s to 83.1 s per run. Per `260906-0026-what-shared-state-the-hook-suite-reaches.md` `### On the acceptance, honestly`, this certifies a rate below roughly five percent on this machine under that load, and not zero: the failure is probabilistic over wall-clock budgets, and no budget makes it impossible. The single-instance route of the second branch was not taken, for the three reasons that analysis gives under `### The route: repair, not a single-instance declaration`. The single-slot lead in the `Also seen:` line above is refuted by reading at that commit: `hooks/tracker.ts` joins the three sentences with a space, so a staging sentence standing where a coverage one was expected is what a timed-out window listing produced, not two reporters contending for one slot.
