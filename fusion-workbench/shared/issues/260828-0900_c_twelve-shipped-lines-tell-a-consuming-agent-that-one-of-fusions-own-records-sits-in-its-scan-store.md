...
Resolved: f1099c5f — every line now names the record as fusion's own in the storeless form; the widened grep (`.{0,160}`) returns nothing and the `it` in `hooks/lib/__tests__/reference-resolution-lint.test.ts` keeps a stamp and `$SCAN_` off one line.
