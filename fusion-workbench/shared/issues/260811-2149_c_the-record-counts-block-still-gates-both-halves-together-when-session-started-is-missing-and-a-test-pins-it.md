...
---
Resolved: The block now carries two cause variables instead of one — `WHY_A` for the anchor, `WHY_T` (`no-session-start`) for the start stamp — and each half is gated on its own. The four cases this record listed are what it computes and what the prose lists: both usable → `records`; anchor unusable → `records=partial why=<anchor cause>` with the filed half taken; start stamp missing → `records=partial why=no-session-start` with the `now_` half taken; neither → `records=unmeasured why=<anchor cause>,no-session-start`, both causes named because both are true. The second point is answered by that same split: `no-anchor-in-agentstate` now describes the anchor field alone, and the cause list in `agents/orchestrator.md` documents all three values. Two pinning assertions were replaced rather than deleted, each with a comment saying what it was right about and why the branch moved — the `:322` case (now asserting both causes and no counts) and "describes the no-anchor branch as the disjunction it is" (the disjunction was an accurate description of the combined gate and is now a branch the code cannot take). A new case per shell asserts the `now_` counts are taken with `session.started` absent, via a new `start` fixture shape. `cd hooks && npm test` — 52 files, 1349 tests, exit 0.
