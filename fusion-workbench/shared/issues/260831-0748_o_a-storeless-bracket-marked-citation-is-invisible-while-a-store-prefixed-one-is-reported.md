...
they are exhibits rather than pointers. Row one's store segment is the shared issues store, written
`<store>/` here rather than spelled: `store-prefixed` is decided from a token's shape before any
lookup, so no exemption premised on not looking a record up reaches it, the fence included, and a
spelled segment would make this record one more finding of the kind it reports.

```
<store>/260519-0438[o]-loader-check.md         record/store-prefixed
