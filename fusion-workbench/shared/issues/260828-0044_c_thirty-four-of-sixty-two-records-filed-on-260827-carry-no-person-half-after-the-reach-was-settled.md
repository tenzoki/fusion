...
---
Resolved: closed on a re-measurement, not on a fix in this package; the tree had already moved. Survey `260921-1653-open-defect-survey-at-11-9-1.md` row 2, over every `2609*` record under `issues/`, `decisions/` and `reviews/` in the live tree, archive excluded: 330 files, 329 carry `**Filed by:** <agent>, Name <email>`, and the one miss is a closed record with no `Filed by` line at all. Re-taken at `a891c50e` with `find` over the same three kinds and `/usr/bin/grep -lE '^\*\*Filed by:\*\* .*<.*@'`: 340 files, 339 carry the person half, the same single miss (`260917-1508_*_the-reference-resolution-pin-is-red-on-commit-bs-tree-and-no-step-re-approves-it.md`). The history kind, which carried 21 of the original 34 and 5 of the September re-measurement, is closed to writes since `0ec15cb9`, so its misses can neither grow nor be corrected by any writer. The parenthesised second form is read by that grep as carrying the person; whether it stays a second spelling is nobody's defect and was not reopened here. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 27, closed in the commit that carries this line.
