...
---
Resolved: the deferred dispatch cases land in `hooks/lib/__tests__/guard-state-shape.test.ts`, inside the growth bound after the cut of step 17 (`fcff85b8`), which is the condition this record set. Three cases, 23 lines: the existing first dispatch case now runs with the SessionStart identity export blanked so `bin/fusion-identity` decides, and asserts that a `task_start` row in a scratch root that is no git tree carries neither `person` nor `checkout` as a key (the absent-key rule: absent, never empty); a new case runs the dispatch with `FUSION_PERSON` and `FUSION_CHECKOUT` set and asserts the row carries both; a new heartbeat case drives `hooks/tracker.ts` three times and asserts that no `.session-marker` means none afterwards, that a marker younger than 60 s keeps its mtime, and that one aged past 60 s by `utimesSync` is refreshed. Of the seven cases the 2026-08-27 split deferred, the `agentstate.yaml` gate is moot since `6357ebfc` (the file no longer admits a row, and the case that pins that stands in the same describe since then); the one-row-per-hook, identity, `task`, `session_id`, absent-key and heartbeat cases are the three above. The three `fusion-commit-lock` cases landed at the split. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 22, fixed in the commit that carries this line.
