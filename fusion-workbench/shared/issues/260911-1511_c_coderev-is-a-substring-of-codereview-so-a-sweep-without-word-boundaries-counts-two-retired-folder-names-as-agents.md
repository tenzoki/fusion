...
---
Resolved: the acceptance's deliverable exists, and the remainder is nothing a file can hold. `260911-1316-five-retired-agents-and-the-container-contradiction-read-site-by-site.md` `### 1a. Before re-running the count` carries the hazard beside the reading's own figures: line 73 the word-boundary trap (`coderev` inside `codereview`, `ontorev` inside `ontoreview`), lines 96 to 97 the ugrep-versus-`/usr/bin/grep` pin. The other half of the acceptance, that any later pass states its `\b` pattern and names its grep, is a habit and holds only in the passes that follow it. Survey `260921-1653-open-defect-survey-at-11-9-1.md` row 22. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 27, closed in the commit that carries this line.
