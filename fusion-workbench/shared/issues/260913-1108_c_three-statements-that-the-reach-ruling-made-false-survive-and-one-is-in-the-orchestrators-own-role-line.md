...
---
Resolved: The three are corrected, and the sweep for a fourth found one the report did not name. `agents/orchestrator.md`, the role paragraph above `## How you ask the user anything`, now reads "You are the session's dispatcher, and not the only agent that can dispatch one" (+27 bytes, on the orchestrator path alone). `docs/philosophy.md` `## How a session runs` names the orchestrator the session's dispatcher rather than the only one. `README-agents.md` `## The agents` drops the monopoly paragraph for one that states the positive rule, keeps the recommend-and-return habit as a habit, says the constraint is now prose in none of the eleven prompts, and carries the gate halt; its v2.8.1 `disallowedTools` parenthetical, the only true part of what it replaced, stands.

**The fourth.** `README-agents.md` `## Workflow patterns` also read "The orchestrator is the **only** agent that dispatches other agents" — same file the report swept, one paragraph the report did not reach. Corrected the same way. `docs/**` beyond `philosophy.md` and every `skills/*/SKILL.md` were swept and hold no fifth: the only other hits are in `docs/upgrading-to-v10-*.md`, which describe the state at their own releases and are correct there, and `skills/help/SKILL.md`, whose 11.1.0 paragraph already carries the new rule.

The acceptance grep returns nothing. `rules-emission.golden` and `surface-growth.golden` were regenerated, each mandated by its own header after a deliberate edit; no baseline moved.
