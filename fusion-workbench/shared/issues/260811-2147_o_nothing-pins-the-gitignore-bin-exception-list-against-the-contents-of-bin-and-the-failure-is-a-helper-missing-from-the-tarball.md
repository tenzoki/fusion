...
`git ls-files` is the right question because it answers what actually ships, rather than re-reading `.gitignore` and re-implementing its precedence rules — which is the re-implementation defect `260810-0510` is open against. The failure message should name the missing file and the exact line to add.
