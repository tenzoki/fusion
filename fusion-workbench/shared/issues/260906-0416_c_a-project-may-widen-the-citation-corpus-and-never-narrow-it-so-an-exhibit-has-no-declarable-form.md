...
---
Resolved: a project can now declare a record an exhibit in its own `fusion.json`, without a fusion release: `citations.exhibits`, an array of storeless record basenames with the marker wildcarded, each ending in `.md` (`hooks/lib/config.ts`, validated like `extraPaths` plus the `.md` rule, whole-array drop with the index named, because a prefix entry would declare a month of records at once). `createScanner()` takes the list; a corpus file whose own basename matches an entry by `basenameMatcher` has every token reported `exempt` with the reason `declared-exhibit`, shape-decided verdicts included, so a fenced store-prefixed transcript in a declared record is no longer a row; the reason is first in the chain and outside `RESOLUTION_PREMISED_EXEMPTIONS`, and the sweep rewrites nothing that carries a reason, so an exhibit stays as written. `bin/fusion-citation-check` prints `declared-exhibits=` beside its verdict; the scope and the accepted residual (a genuine violation declared away is silenced, the same residual `foreign:` carries; the write-time hook reads no configuration, so an unfenced token in a declared exhibit still draws its sentence) are written in the scanner's header, the checker's header, both `_citations` notes and `README-hooks.md`. `RECORD_EXAMPLE_FILES` and the teaching file's coverage are untouched, and this repository declares no exhibit: `declared-exhibits=0`, `dangling=301`, `store-prefixed=404`, `verdict=clean` unchanged. One case in `hooks/lib/__tests__/declared-citation-paths.test.ts`. Working answer under `260906-0416_*_should-a-project-be-able-to-declare-a-record-an-exhibit-and-what-does-that-declaration-cover.md` option 2 (option 4 neither taken nor foreclosed); the marker there stays `_o_` for the user to rule. Plan `260921-1726_*_the-33-open-defects-of-the-11-9-1-survey-worked-autonomously-as-one-package.md` step 26, fixed in the commit that carries this line.
