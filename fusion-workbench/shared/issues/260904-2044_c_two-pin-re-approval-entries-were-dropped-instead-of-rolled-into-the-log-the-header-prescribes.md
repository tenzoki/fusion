...
---
Resolved: Both entries were recovered verbatim from `git:d5a27230` into `260904-2202-reference-resolution-pin-re-approval-log-the-two-dropped-2026-08-29-entries.md`, in the form the two earlier rolls use, and the pin's header block now names that file beside the other two and closes on "Roll, never drop." The drop confession in the trailing comment was corrected to record the recovery rather than the loss. The record states the one departure from byte-for-byte recovery it makes: at `d5a27230` the grammar entry opens `2026-08-29` because the chaining dropped its `Re-approved`, which stands at `git:3276b1e1`, and the entry is reproduced as the named commit holds it. No number in the pin moved with the recovery.
