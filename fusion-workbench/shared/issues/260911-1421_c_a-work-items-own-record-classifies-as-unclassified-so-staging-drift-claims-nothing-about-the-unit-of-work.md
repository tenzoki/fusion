...
---
Resolved: `classify()` in `hooks/lib/staging-drift.ts` has a branch for `circles/<item>/<item>.md`, the same criterion `bin/fusion-claimed-item` recognises the record by, returning `record` with the reason "a work item's own record"; a nested path with the same basename keeps the store loop's reason. One subprocess case in `staging-drift.test.ts` pins both shapes, the item's own record as an unstaged record and a sibling note as unclassified, funded by four comment cuts that restated the module's own header, so the file stays at 653 lines and the hook-test bound and its golden do not move. `backlog` stays in `STORES`: `shared/backlog/` holds two entries at HEAD, the test pins the store, and when it empties is `260910-2020_*_the-two-existing-backlog-entries-keep-the-retired-marker-form-that-d1-migrates-into.md`, out for consent. Two dist files rebuilt. Concept accepted by a consultant read with changes; fixed in the commit that carries this line.
