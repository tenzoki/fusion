...
**What justifies the prose.** Each gate closed a measured bypass, and each is recorded as a closed issue in the same Circle. `260802-2229_*_rules-write-flag-is-a-write-anywhere-primitive-via-a-symlink-planted-in-rules.md` measured the flag becoming a write-anywhere primitive through a symlink planted in `rules/`, which is why gate 2 touches the filesystem at all. `260802-2330_*_the-lexical-dotdot-collapse-erases-the-symlink-gate-2-was-added-to-resolve.md` measured the lexical `..` collapse erasing the component gate 2 was added to resolve, which is why gate 0 reads the caller's original spelling. For a security boundary that has been wrong twice, an audit trail in the file is the cheapest place to keep the reasoning. The comment density is defensible on those grounds.
...
- `260809-1101_*_churn-and-cross-file-state-are-cast-not-coerced-so-a-shape-valid-file-swallows-the-halt-message.md` — the escalation-shape defect closed by `260802-2334_*_a-shape-valid-escalation-json-makes-the-whole-guard-fail-open-on-both-surfaces.md` is still open in the two sibling state modules, and the throw discards the protected-path halt message on the same call. Verified by execution.
