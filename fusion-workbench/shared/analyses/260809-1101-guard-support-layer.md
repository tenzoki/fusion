...
**What justifies the prose.** Each gate closed a measured bypass, and each is recorded as a closed issue in the same Circle. `260802-2229_*_rules-write-flag-is-a-write-anywhere-primitive-via-a-symlink-planted-in-rules.md_c` measured the flag becoming a write-anywhere primitive through a symlink planted in `rules/`, which is why gate 2 touches the filesystem at all. `260802-2330_*_the-lexical-dotdot-collapse-erases-the-symlink-gate-2-was-added-to-resolve.md_c` measured the lexical `..` collapse erasing the component gate 2 was added to resolve, which is why gate 0 reads the caller's original spelling. For a security boundary that has been wrong twice, an audit trail in the file is the cheapest place to keep the reasoning. The comment density is defensible on those grounds.
...
- `260809-1101_*_churn-and-cross-file-state-are-cast-not-coerced-so-a-shape-valid-file-swallows-the-halt-message.md` — the escalation-shape defect closed by `260802-2334_*_a-shape-valid-escalation-json-makes-the-whole-guard-fail-open-on-both-surfaces.md_c` is still open in the two sibling state modules, and the throw discards the protected-path halt message on the same call. Verified by execution.
- `260809-1101_*_churn-and-cross-file-criticals-latch-permanently-and-never-reset.md` — `totalChanges` and `pingBackCount` are monotonic for the life of the project, so two critical event types fire on every write once any file crosses a threshold. Twenty-one percent of the measured event log.
- `260809-1101_*_escalation-json-read-modify-write-can-lose-a-halt-raised-by-a-parallel-tool-call.md` — atomic rename without a lock, with the allow path holding a state object loaded before the halt.
- `260809-1101_*_an-absent-plugin-config-layer-yields-an-empty-protected-list-with-no-diagnostic.md` — the one silence that contradicts the loader's own stated diagnostics contract.
...
Cross-referenced records: `260801-1020_*_guard-protects-rules-but-not-claude-rules.md`; `260804-2100_*_from-a-subdirectory-cwd-the-protected-list-matches-nothing-while-fail-closed-still-denies.md`; `260805-1859_*_das-guard-event-log-waechst-unbegrenzt-und-sein-groesster-schreiber-liefert-null-information.md`; `260802-2334_*_a-shape-valid-escalation-json-makes-the-whole-guard-fail-open-on-both-surfaces.md`; `260802-2229_*_rules-write-flag-is-a-write-anywhere-primitive-via-a-symlink-planted-in-rules.md`; `260802-2330_*_the-lexical-dotdot-collapse-erases-the-symlink-gate-2-was-added-to-resolve.md`; `260804-1603_*_the-project-config-layer-is-not-type-validated-so-a-wrong-type-fails-the-guard-open.md`; `260807-0945_*_integritaet-des-eskalationsspeichers.md`.
