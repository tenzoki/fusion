...
(decision `260815-2312_*_should-the-circle-records-status-field-exist-at-all-now-that-both-transitions-maintain-it.md`) and the decision record's (`260818-2212_*_should-the-decision-records-status-field-exist-at-all-now-that-the-circle-records-has-been-removed.md`) — but they state the same
...
paragraph. Then either add the site to `260819-0756_*_the-v10-2-migration-note-still-states-the-status-position-with-the-qualifier-the-rule-just-dropped.md` or state in that record why it is out of scope.
...
---
Resolved: `agents/orchestrator.md:302-303` now reads "leave it exactly as it stands, including when you transition it — nothing writes it, nothing reads it, and those drifted headers are the evidence the removal was decided on." It takes both halves of the rule's construction and keeps the paragraph's own em-dash shape, so the two shipped surfaces state one position while remaining separate paragraphs for two record kinds; neither points at the other. The marker-is-the-state sentence and the citation of `260815-2312_*_should-the-circle-records-status-field-exist-at-all-now-that-both-transitions-maintain-it.md` are byte-identical.
