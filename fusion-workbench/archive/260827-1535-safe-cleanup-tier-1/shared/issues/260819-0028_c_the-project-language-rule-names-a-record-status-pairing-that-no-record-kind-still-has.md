...
---
Resolved: fixed — the head-labels paragraph no longer pairs `**Status:**` with a record; `grep -o "the plan head's \`\*\*Decidability:\*\*\` and a record's \`\*\*Domain:\*\*\`" rules/fusion-workbench-conventions.md` at HEAD 260824 matches, and the middle example is gone.
