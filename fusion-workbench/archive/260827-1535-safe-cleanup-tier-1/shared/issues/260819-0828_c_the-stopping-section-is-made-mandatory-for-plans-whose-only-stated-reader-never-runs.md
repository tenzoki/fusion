...
---
Resolved: fixed — `hooks/lib/__tests__/plan-stopping-section-lint.test.ts` reads every live plan for presence, so the no-Circle case has a reader, and `agents/planner.md` says so; `grep -c plan-stopping-section-lint agents/planner.md` at HEAD 260824 prints `1`.
