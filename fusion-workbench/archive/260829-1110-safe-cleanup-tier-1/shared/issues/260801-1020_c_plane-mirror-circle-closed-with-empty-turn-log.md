...
`archive/260817-1907-safe-cleanup-scoped/260719-1536-plane-mirror-integration` (live under `circles/` until the archive sweep of 2026-08-17) carries the marker `_c_` (closed-coherent) and a full Closure note citing six commits `eb9cf59..aefbf39`. Its `## Turn log` section still reads:
...
1. Backfill this record's Turn log from `260719-1632-orchestrator-session.md` and the six commits named in its Closure note.
...
Filed by: analyst, from `260801-1020-normative-surface-drift-gap-analysis.md` (Question 4, third thin spot).

---
**Reconciliation 260817-1836** (reconciler, domain `code`). Re-verified reproducible at HEAD `2552586`: `260719-1536-plane-mirror-integration:56-58` still carries the anticipation-time placeholder under `## Turn log`, and no backfill or detection step exists. The mirror itself is gone, but the class of defect (a Circle closing over an unwritten Turn log) is not tied to it. Marker stays open. Log: `260817-1836-reconciliation.md`.

---
Also seen: 260822-0234 by reconciler — `260821-1042-reply-bounded-whole-question-answered` carries an empty `## Turn log` after two sessions and seven Turns, and is about to go terminal; part 2 of this record's fix (a detectable condition at closure) would have caught it.
...
`260821-1042-reply-bounded-whole-question-answered` `## Turn log` at Phase 4,
...
Also seen: 260822-2236_*_the-four-session-bookkeeping-surfaces-froze-again-and-the-detection-that-closed-the-first-record-has-been-removed.md by reconciler — `260822-1921-measure-what-two-checkouts-share`
...
---
Resolved: referred (backlog) — part 1 is moot, the Circle is archived and its Turn log content sits in its Closure note; part 2, a closure step that detects a Turn log still holding the anticipation placeholder, is one idea with the freeze detection of 260822-2236_*_the-four-session-bookkeeping-surfaces-froze-again-and-the-detection-that-closed-the-first-record-has-been-removed.md; backlog entry to be filed by the user
