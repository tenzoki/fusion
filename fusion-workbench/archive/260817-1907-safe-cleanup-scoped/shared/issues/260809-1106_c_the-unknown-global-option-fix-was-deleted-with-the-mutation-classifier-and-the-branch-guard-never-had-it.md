...
**Cross-references:**
`260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md` (closed; the same defect, found and fixed in `bash-mutation-guard.ts`),
`260804-1344_*_the-git-option-walk-stops-at-an-unknown-options-value-so-a-c-behind-it-is-invisible.md` (closed; the follow-on in the same walk),
`260809-1103-guard-enforced-policies.md` §Findings 2b-2
...
This is verbatim the defect recorded and closed on 2026-08-04 as `260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md`, severity High, marked there as pre-existing and structural. That record's fix was applied to `resolveGit` in `hooks/lib/bash-mutation-guard.ts`. When the mutation classifier was retired in v6.0.0, the module went and the fix went with it. The git branch classifier has the identical eight lines and never received the correction.

`260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md` says it plainly: "`--namespace` is the instance git 2.49 happens to ship. It is not the defect. Every option the table does not carry has this shape, including options git has not shipped yet, which is why it was not closed by adding a row."
...
Take the fix that already exists rather than inventing a second one. `260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md` closed this structurally: when the word in subcommand position matches no known subcommand **and** an unrecognised option stands immediately in front of it, try the next word as a subcommand too. A second candidate can only add a match, so it can only add a deny.
...
- [x] `git --namespace foo -C sub switch main` denies (the `260804-1344_*_the-git-option-walk-stops-at-an-unknown-options-value-so-a-c-behind-it-is-invisible.md` residual, resumed walk).
...
- [x] The cost is stated as a rule with an open example set, matching the wording `260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md` settled on, in `rules/git-branch-discipline.md`.
...
arguments. The remedy taken is `260804-1344_*_the-git-option-walk-stops-at-an-unknown-options-value-so-a-c-behind-it-is-invisible.md`'s resumed walk, not `260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md`'s
first two-adjacent-candidate attempt. Pinned by the describe block "an
unrecognised global option no longer hides the subcommand (260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md,
260804-1344_*_the-git-option-walk-stops-at-an-unknown-options-value-so-a-c-behind-it-is-invisible.md)" in `hooks/lib/__tests__/git-branch-guard.test.ts` — the test name
...
`260809-1229_*_plan-five-severe-guard-defects.md` carried the
cost into `rules/git-branch-discipline.md` as a rule with an open example set,
in the wording `260804-1333_*_an-unrecognised-git-global-option-swallows-the-subcommand-and-the-invocation-reads-as-an-unrecognised-program.md` settled on.
