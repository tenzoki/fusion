...
**Cross-references:** 260719-2223_*_plan-plane-bounded-bridge.md (Step 3 build_write_body, Step 5 seed round-trip); 260719-2141_*_plane-rolle-push-only-vs-bounded-readback-martin.md (the bounded-bridge decision this refines)
...
3. **Push posts a comment for status, never edits seeded descriptions** (Martin's own flow posts closing comments).
   - Pros: matches Martin's verified `issues/{id}/comments/` habit; zero description contention. Cons: another endpoint (comments body unverified in MARTIN.md, see issue 260719-2304_*_verify-plane-create-patch-body-against-live-instance.md); status spread across comments.
...
- The comments-body shape is not yet verified against the live instance (issue 260719-2304_*_verify-plane-create-patch-body-against-live-instance.md).
...
---
Answered: user chose **Option 1** (session 260719-1632-orchestrator-session.md, follow-up on 2026-07-20) — push writes state only for seed-origin issues and never touches their title or description; fusion-created issues keep the full mirror body.
...
---
Retired: `d0ddabb` + `7c12d6a` (steps 2 and 3 of 260815-0029_*_plan-remove-eight-mechanisms-and-cap-growth.md) — the durable `origin` field this record's answer added lived in `.plane-map.json`, which went with the mirror's code and data. There is no round-trip write left to preserve an origin story's description against.
