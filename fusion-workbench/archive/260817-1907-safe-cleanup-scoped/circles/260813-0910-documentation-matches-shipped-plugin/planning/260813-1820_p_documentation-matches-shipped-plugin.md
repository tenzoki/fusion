...
**Spec:** none — planned from the Circle Directive at `260813-0910-documentation-matches-shipped-plugin`, against the bounded finding list in `260813-0828-documentation-staleness-survey.md` and the acceptance conditions in `260813-0825_*_the-v8-1-0-documentation-step-reached-three-files-and-the-feature-reached-seven-surfaces.md`
...
**The parameter roster is larger than any documented count, and one documented claim contradicts the prompt.** Read on both sides while planning, as leads for step 6 rather than as the roster: `agents/planner.md` declares `**Executors:**` and `**Circle:**` and contains **zero** occurrences of the string "Domain"; `agents/taskplanner.md`, `agents/reconciler.md` and `agents/playmaker.md` each carry a `Parameter parsing` section for `**Domain:**`; `agents/editor.md` declares `**Deliverable language:**` with no default and a halt; `agents/shaper.md` declares a four-line mode contract (`**Mode:**`, `**Draft:**`, `**Circle file:**`, `**Parent task:**`). The work-tree and installed copies of `agents/planner.md` are byte-identical, so this is not an install-drift artefact. Three shipped surfaces nevertheless state that the planner takes a domain parameter: `CLAUDE.md:14`, `CLAUDE.md:56-60` and `docs/philosophy.md:19`, and the manifest description in `.claude-plugin/plugin.json:3` states a count of three domain-parameterised agents. That is a documentation defect against the prompt, and separately it raises a design question this plan does not answer (decision record `260813-1820_*_should-the-planner-accept-a-domain-parameter…`).

**The dependency that once split this Circle in half has closed.** `260813-0858-playmaker-maintains-backlog-store` carries the closed-coherent marker, so the four passages the Directive named as waiting are workable now. They are folded into steps 2, 6, 7 and 9 rather than carried as a separate deferred group.

**The test suite is green at HEAD, on the closure note rather than on a run of my own.** `260813-0828_*_three-tests-fail-at-head…` records `cd hooks && npx vitest run` at exit 0, 48 files, 1010 tests passed, after two fixes. *Inference:* the first run of the extended gate in step 3 should therefore be green if steps 1 and 2 land first. The executor verifies this by running the suite rather than by trusting the note.
...
3. [DONE] **Extend `derivable-enumerations-lint.test.ts` with the `bin/` roster**
   - Executor: `coder`
...
4. [DONE] **README-agents.md — the shaper and planner rows, and the Turn-budget diagram**
   - Executor: `coder`
   - Files: `README-agents.md`
...
5. [DONE] **README.md — the configuration section and the tuning table**
   - Executor: `coder`
   - Files: `README.md`
...
6. [DONE] **All sixteen agent rows, and the dispatch-parameter roster**
   - Executor: `coder`
   ...
   - Completion: all sixteen rows read against their prompt and their `bin/fusion-paths` key set; twelve corrected, four left standing after a reading (`shaper`, `planner`, `bugfixer`, `editor` — the last three of those gained no change beyond the parameter clause moving out of the `editor` row). The dispatch-parameter table is authored at `README-agents.md` `## Dispatch parameters`, eleven rows across six agents, each cited to the prompt line it was read against. The planner claim was confirmed on the prompt — `agents/planner.md` declares `**Executors:**` and `**Circle:**` and parses no `**Domain:**` — and corrected at `CLAUDE.md:14`, the `CLAUDE.md` dispatch bullet, and `docs/philosophy.md:19`. `.claude-plugin/plugin.json:3` was read and **not** edited: its claim is a bare count of three domain-parameterised agents, naming no member, and the count survives the membership change (`taskplanner`, `reconciler`, `playmaker`). A fifth carrier of the same claim, `agents/orchestrator.md:153`, is outside this step's file scope and is filed as `260813-2045_*_the-orchestrator-prompt-names-the-planner-among-the-domain-parameterised-agents-and-omits-the-playmaker.md`. Two reviewer issues against the step-4 `shaper` row (`260813-2009_*_…in-place-record-edits`, `…backlog-promotion-unconditionally`) were resolved in the same pass and closed.

7. [DONE] **docs/working-model.md — the Circle-first flow, the backlog store, the playmaker's role**
   - Executor: `coder`
   ...
   - Dependencies: step 6
   - Completion: all three passages authored, each against a named artifact. §2 gained a second diagram line for the `/fusion:direct` entrance and two paragraphs distinguishing the three spec-producing shaper modes from anticipated-circle mode, read against `agents/shaper.md` `## Four invocation modes` (`:39-105`) and its Scope exception at `:28`; the Circle-first placement sentence is cited to `rules/fusion-workbench-conventions.md` `## Path Resolution`. §1 gained a `### How a Circle comes into existence` subsection (three entrances), a backlog paragraph read against `rules/fusion-workbench-conventions.md` `## Backlog entries` and `skills/memo/SKILL.md` (`:9-15`, `:37`, `:110-126`, `:152`), and an idea-to-Circle sketch read against `skills/direct/SKILL.md` (`:77`, `:88-91`, `:102-108`) and `skills/next/SKILL.md` (`:124`). The plan's recommendation was **followed**: §5 became "Two worked walkthroughs" with the existing code session as 5a and a five-step portfolio path as 5b, the playmaker sentences taken from step 6's reading (`agents/playmaker.md` `## Two mandates, by dispatch path`, Step 2b at `:108-121`, the `$OUT_BACKLOG` write bullet at `:60`) rather than re-derived. **One correction outside the three passages, inside the same file:** §5a step 5 said "the middleware file isn't protected", naming the protected-path check that §4 of this same document records as removed on 2026-08-12; it now reads that the file sits in no decision-governed area. Verified `cd hooks && npx vitest run` — exit 0, 49 files, 1022 tests, unchanged from step 6's baseline.

8. [DONE] **docs/philosophy.md — the traceability paragraph**
   - Executor: `coder`
   ...
   - Dependencies: none
   - Completion: §3's one-sentence surface list became four sentences, each read against a skill body. `/fusion:memo` now carries all three of its captures — notes, todos, and an idea filed as its own new entry in the backlog — and the sentence states that it is the one surface a user files such an entry from, read against `skills/memo/SKILL.md:11-13` (the three kinds), `:37` (one file per idea, never an append) and the last guardrail at `:153` ("the backlog holds what the *user* files … and this skill is that surface"). `/fusion:cadence` was added with what it actually reads and writes: three ranked lists (`skills/cadence/SKILL.md:9-13`) over session histories, the activity log and git commits (its source-legend table at `:76-82`). `/fusion:log-activity`'s clause is unchanged. §5 at `:19` was not touched.

9. [DONE] **skills/help/SKILL.md — the two missing entry points and the backlog store**
   - Executor: `coder`
...
- **After step 3 and at the end of each Turn:** `cd hooks && npx vitest run` — the full suite. The baseline is 48 files and 1010 tests green, recorded in the closure note of `260813-0828_*_three-tests-fail-at-head…` and not re-run while planning. Establish it by running, not by citing.
...
- [ ] **Should `agents/planner.md` accept a `**Domain:**` parameter?** Three shipped surfaces say it does; the prompt never parses the string. This plan corrects the documentation to the prompt and leaves the design question open. Filed as `260813-1820_*_should-the-planner-accept-a-domain-parameter-that-three-documented-surfaces-already-promise.md`.
- [ ] **How do fusion's own documents treat a hand-measured number that decays?** The tracked-file count is deleted by Directive and the byte-budget claim takes the same treatment in step 2, which makes two instances of one rule and no stated convention. Filed as `260813-1820_*_how-fusion-s-own-documentation-treats-a-hand-measured-number-that-decays.md`.
...
- [x] **Does `docs/working-model.md` §5 gain a step or a second walkthrough?** Answered by step 7: a second walkthrough. §5 is now "Two worked walkthroughs", 5a the existing code session unchanged in shape, 5b a five-step idea-to-Circle path. The recommendation held on reading — 5a marks a gate or a hook at almost every step, and the portfolio path crosses neither, so a step folded into 5a would have inherited a gate count that is wrong for it. Plan-scoped, no record filed.
...
**260813-2258-reconciliation.md — reconciler, domain `code`, range `267a65c..HEAD` (`c0e4219`), 15 commits.**
...
2. **Step 6's completion note is wrong about its own outcome, and the reviewer already filed it.** The note
   ...
   `260813-2052_*_the-step-6-completion-note-says-twelve-rows-corrected-and-names-three-that-changed.md`
...
3. **Both conceptrev recommendations are unapplied, and one of them is load-bearing for the deferral that
   happened.** `260813-1831-conceptrev-…` asked for (1) an `S3 -.-> S4` edge in diagram 2 and (2) a
