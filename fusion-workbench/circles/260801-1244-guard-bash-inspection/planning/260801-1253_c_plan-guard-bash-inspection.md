...
**Spec:** `shared/planning/260801-1122_o_spec-normative-consolidation.md` — capability C5c only
**Closes:** `shared/issues/260801-1156_o_bash-bypasses-the-protected-path-check-entirely.md`
...
<!-- FINDING: `rm \<newline>rules/x.md` is ALLOWED. shell-parse does not honour a backslash line continuation — the escape pair is emitted verbatim and the newline then terminates the segment, so the operand lands outside the verb's command. The same hole allows `git worktree \<newline>add ../wt x` in the pre-existing git classifier. Filed as issues/260801-1513_o_backslash-line-continuation-…; the fix is in shell-parse and widens both classifiers, so it needs a gate. -->
...
- The same five against `/tmp/…` return `{}`, and after each the escalation file's `consecutiveBlocks` is unchanged and `events.jsonl` has gained no line. This is the 260707-0750 and 260707-0751 regression guard and must be asserted on the files, not inferred.
...
| The allow path silently acquires a side effect and re-opens issues 260707-0750 and 260707-0751. | Step 6 asserts on the state files themselves before and after an innocuous Bash call, not on the verdict. Step 5's acceptance names the two forbidden calls. |
...
|---|---|---|
| `56a41c4` | S1 | plan |
| `a342e9b` | — | out-of-band prompt edit found in `git status`; `shared/issues/260801-1410` |
| `9a35b8e` | S2 | plan |
...
| `2a29c90` | — | issues `260801-1610`, `260801-1857`-adjacent; two pre-existing git-classifier holes |
| `85c043c` | S6 | plan |
| `3806a49` | S7 | plan |
| `e31c0f3` | S8 | plan |
| `5d9bbcc` | — | Turn 2: issues `260801-1857`, `260801-1858` (both High) |
| `18e2e4f` | — | Turn 2: issues `260801-1859`, `260801-1900`, `260801-1901`, `260801-1902`, `260801-1903` (five Medium) |
| `9ab5a2a` | — | Turn 3: issues `260801-1955`, `260801-1956`, both regressions introduced by `18e2e4f` |
...
- Nine of C5c's ten spec acceptance criteria are met. The tenth is Q1's deliberate deferral. Ticked in `shared/planning/260801-1122_o_spec-normative-consolidation.md`.
