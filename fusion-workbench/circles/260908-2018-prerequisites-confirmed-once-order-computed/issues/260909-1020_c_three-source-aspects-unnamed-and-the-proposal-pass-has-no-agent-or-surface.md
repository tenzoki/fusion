...
---
Resolved: closed on evidence by the commit that carries this line, with no change to the tree. The second half, the proposal pass with no performer, is answered: the pass has an agent, `curator`, a surface, `/fusion:curate --edges`, which dispatches with `**Edges:** on` (`agents/curator.md` `## The fourth subject — work-item edges`), and re-run semantics, `agents/curator.md` `### Pass 2 — apply. Approved entries only.`, where a basename already present is `applied` with nothing written. The first half, the three source aspects absent without being named, concerns `260908-2018-prerequisites-confirmed-once-order-computed.md`, whose `**Status:**` is `done`, a terminal record that `rules/fusion-workbench-conventions.md` `## Terminal states are history` forbids editing; the six unsettled points, this record's own list, are carried by name in the Directive of the successor item `260917-2253-depends-on-edges-proposed-and-confirmed.md`, which cites this record in its `**Cross-references:**`. What this record asked for is either written where it can be or stated as out of reach, so nothing is left for a later pass to do here.
