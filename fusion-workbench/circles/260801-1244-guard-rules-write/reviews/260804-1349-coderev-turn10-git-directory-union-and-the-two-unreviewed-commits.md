...
rule in both shells, so the class `260804-1333` says it closed structurally is still open one
word further along; and `git checkout HEAD~1 -- .` allows and overwrites every protected
file, so the `checkout` row `260804-1026` was closed by has a root-pathspec hole of exactly
...
not.** The union is the correct answer to `260804-1024` and it is argued honestly. Two High
...
|---|---|---|
| Critical | 0 | — |
| High | 2 | `260804-1344`, `260804-1345` |
| Medium | 4 | `260804-1346`, `260804-1347`, `260804-1348`, `260804-1349` |
| Low | 2 | `260804-1350`, `260804-1351` |
...
**`260804-1344` (High) — the git option walk stops at an unrecognised option's VALUE.**
...
`260804-1333` is correctly closed for its own instance and must not be reopened; what is
...
**`260804-1345` (High) — `git checkout <treeish> -- .` overwrites the whole protected list.**
...
**`260804-1346` (Medium).** The old residual entry for `git clean -fdx` with no path operand
...
**`260804-1347` (Medium).** The union added a fourth source of `CWD_UNKNOWN` (`stepDir`,
...
**`260804-1348` (Medium) — "Now they agree" is false at `HEAD`.**
...
Same operation, two spellings, opposite verdicts — which is `260804-1026`'s own complaint,
...
four-question block to be falsified — question 3 by `260804-1025` / `260804-1223`, the count
by `260804-1220`, question 1 here. All four are one paragraph and should be one edit.

### Theme 5 — the two Job-2 design questions, answered

**`260804-1350` (Low) — the `DirStack` sum type buys one of the two facts the claim needs.**
The compiler proves a give-up is total: the `unknown` arm has no `entries`, so the
zeroed-but-still-N-deep value of `260803-2237` is not writable and `popd` cannot read the
...
**`260804-1351` (Low) — the `runsBuiltins` shape does reappear, one table down.**
...
hop and an ambient `CDPATH` (`260804-1025` / `260804-1223`, already filed and re-confirmed at
...
- **`260804-1332` (High, GIT_WORK_TREE) is genuinely open and correctly out of reach.**
  ...
  `260804-1026` was filed about, at the moment that finding is being closed — and it is
...
- **`260804-1333` (the `--namespace` finding) is closed for its own instance and not for its
  class.** See `260804-1344`. Do not reopen `1333`; the row it pins still passes.
...
1. **`260804-1344` and `260804-1345` together, one pass in `resolveGit` / the verb table.**
...
2. **`260804-1346` with `1345`** if the `writesThrough` field is taken — they share the field
   and the test. Otherwise the documentation half alone, as its own small edit.
3. **`260804-1347`** — one constructor, one reason function, one dispatch arm. Cheap, and it
   is the thing an agent actually meets.
4. **One edit to `rules/protected-path-discipline.md`'s four-question block**, closing
   `260804-1349`, `260804-1025`, `260804-1223` and `260804-1220` at once. Four separate edits
   to one paragraph is how `1220` went stale.
5. **`260804-1348`** — documentation now; the `restore --source=HEAD` code half is a decision
   (it would newly ALLOW, a first for this Circle) and should not be taken inside another
   task.
6. **`260804-1350` and `260804-1351`** — wording, with the Low tail.
