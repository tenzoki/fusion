...
---
Resolved: 2026-08-06 — alle sechs behoben: (1) `AskUserQuestion` in der commit-Frontmatter (`skills/commit/SKILL.md:4`), (2) das archive-Snippet baut `$KEEP` und meldet Behaltenes (`skills/archive/SKILL.md:158-161`), (3) das Bracket-Relikt in next entfernt, (4) die BSD/GNU-`ls -T`-Weiche dokumentiert (`skills/log-activity/SKILL.md:79-80`), (5) circle-pop nennt `original_circle_dirname` mit Legacy-Fallback (:158), (6) seed-from-plane trennt Exit 1 (Config) von "bad story number" (:63). Commit 9a96466, Circle 260805-2005-textschicht-gegen-code-nachziehen (Plan-Schritt 12, Batch C).
