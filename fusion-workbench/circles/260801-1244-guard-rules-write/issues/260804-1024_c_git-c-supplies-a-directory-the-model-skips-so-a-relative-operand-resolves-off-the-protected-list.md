...
**Filed by:** reconciler, closing pass of session `history/260803-1737-orchestrator-session.md` (Circle Turns 4 to 8)
...
**Cross-references:**
`260804-0836_o_…` and `260804-0837_o_…` (the same family — the model believes the shell is somewhere it is not — but a different mechanism and a different fix; see `## Why this is not those two`),
`hooks/lib/__tests__/bash-mutation-guard.test.ts:196` (the only `-C` case in the suite, which pins the opposite direction),
`circles/260801-1244-guard-bash-inspection/reviews/260801-1905-coderev-guard-bash-inspection.md:53` (the only prior mention anywhere, a coverage list).
...
## Why this is not `260804-0836` or `260804-0837`

Those two are reachability defects: the `cd` is in the command text, the model sees it, applies it, and is wrong about whether the shell ever ran it. The fix is to consult the joiner for the moving segment, and one decision (`decisions/260804-0947_o_`) closes both.

This one is not about reachability. The directory is in the command text and the model **never looks at it**. `git -C` runs unconditionally; there is no joiner question. Answering `260804-0947` in any of its options leaves every row above allowing. It needs its own change in `resolveGit`, and the three candidate directions below are unrelated to that decision's options.

Keeping them separate matters because `260804-0947` is currently described as the release blocker for any claim about the boundary. It is not sufficient for that claim while this is open.
...
Direction 1 or 2 is a design choice with a measurable cost on both sides and wants a decision record, the way `260803-2238` and `260803-1835` each got one before code moved. That has been this Circle's rule and it has worked.
