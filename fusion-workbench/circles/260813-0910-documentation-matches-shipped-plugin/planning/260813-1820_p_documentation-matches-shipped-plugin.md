...
6. [DONE] **All sixteen agent rows, and the dispatch-parameter roster**
   ...
   - Completion: all sixteen rows read against their prompt and their `bin/fusion-paths` key set; twelve corrected, four left standing after a reading (`shaper`, `planner`, `bugfixer`, `editor` — the last three of those gained no change beyond the parameter clause moving out of the `editor` row). The dispatch-parameter table is authored at `README-agents.md` `## Dispatch parameters`, eleven rows across six agents, each cited to the prompt line it was read against. The planner claim was confirmed on the prompt — `agents/planner.md` declares `**Executors:**` and `**Circle:**` and parses no `**Domain:**` — and corrected at `CLAUDE.md:14`, the `CLAUDE.md` dispatch bullet, and `docs/philosophy.md:19`. `.claude-plugin/plugin.json:3` was read and **not** edited: its claim is a bare count of three domain-parameterised agents, naming no member, and the count survives the membership change (`taskplanner`, `reconciler`, `playmaker`). A fifth carrier of the same claim, `agents/orchestrator.md:153`, is outside this step's file scope and is filed as `issues/260813-2045_o_the-orchestrator-prompt-names-the-planner-among-the-domain-parameterised-agents-and-omits-the-playmaker.md`. Two reviewer issues against the step-4 `shaper` row (`260813-2009_o_…in-place-record-edits`, `…backlog-promotion-unconditionally`) were resolved in the same pass and closed.
