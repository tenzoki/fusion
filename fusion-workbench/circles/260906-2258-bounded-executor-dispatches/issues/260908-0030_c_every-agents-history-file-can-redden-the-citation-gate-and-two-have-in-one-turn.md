...
---
Also seen: 260908-2210 by orchestrator, correcting the count this record carried. The reconciliation pass found the earlier tallies here wrong on two grounds: they equated history files with plan steps, where steps 6 and 7 shared one file, and they stopped at step 9 while the run went to 16. **The rate over the completed run of plan steps 4 to 16 is 4 rejections in 13 executor history files.** Two further instances landed after that count, in the Rebalance passes that followed: the re-take verdict's text in `260908-1529-orchestrator-session.md` and the fifth-passage correction's own history file, each spelling `_c_` where the wildcard belongs, each repaired by hand at two characters. **The class is not closed and its acceptance test has now been run**: the test asks for zero rejections over a run of steps in which no dispatch prompt mentioned the citation form, and no prompt did until the very last two, which named it explicitly. The answer is not zero. The marker stays `_o_`.

---
Resolved: The history store was closed to writes at `0ec15cb9`, so no agent writes a history file any more, and the citation form is measured at the write in `hooks/lib/citation-form.ts` for the records that are still written. — closed by a closing pass on 260921 over the plan's "Already resolved at HEAD" list, the evidence re-verified at HEAD in the commit that carries this line.
