...
- **Changes:** the file has five sections and a stated audience.
  ...
  - **`## When you read the clock.`** Once, immediately before starting the next unit of your work, and at no finer grain. Never inside a unit. State the cost of that plainly, because it is a residual the user bought: an agent that reads the clock just short of its stopping time and then enters a unit longer than its whole bound returns well past that time, and nothing here bounds or measures that overshoot. State it **without numerals** — the acceptance criterion below forbids any number of minutes in the file, and that criterion governs.
...
- **Verification:** in order, exit codes in hand.
  ...
  3. `grep -c '045a14f\|f38f37d' rules/commit-lock.md` returns 2. Over `agents/orchestrator.md` the two hashes are read separately, because they do not both reach 0: `grep -c 045a14f agents/orchestrator.md` returns 0, and `grep -c f38f37d agents/orchestrator.md` returns 1 for a third passage this step forbids moving, the one in `## Staging check` led in by **Do not answer it by widening `git add`.** The two narratives are in one file and one file only.
...
- Does `npm test` pass on the tree this Circle leaves, with `AGENT_BASELINE`, `RULE_BASELINE`, `RELEASE_CAP` and `DRIFT_CEILING` all carrying the same **values** they carried at `abcaa823`, compared entry by entry? A green suite bought by moving a baseline does not answer this clause yes. **The clause asks about values, not bytes**, and the distinction is load-bearing rather than pedantic: `RULE_BASELINE`'s comment block was rewritten by commit `01e0f688`, made in another checkout and pulled in before this Circle's last session began, with every numeric entry unchanged — so a byte comparison answers no while no baseline moved. The four are unchanged in value from `abcaa823` through `223f916a` and in the working tree as this plan is written, so the anchor is a live comparison and not a historical one.
