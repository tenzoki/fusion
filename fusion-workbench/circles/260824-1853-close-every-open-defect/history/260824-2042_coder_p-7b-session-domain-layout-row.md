...
- `CLAUDE.md` line 44: added the `## Layout` row for `bin/fusion-session-domain`, the text the issue carried, checked against the helper's header (prints `domain=` and `source=`, exit 3 without a workbench, three `[ -x ]`-guarded call sites, decision `260810-2145`). Em-dash count unchanged at 126.
