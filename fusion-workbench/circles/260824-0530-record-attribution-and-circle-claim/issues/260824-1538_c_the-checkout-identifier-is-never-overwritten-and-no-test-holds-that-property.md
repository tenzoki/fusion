...
---
**Reconciliation 260824-1637** (reconciler, domain `code`, Phase 3 of session `260824-0539`, HEAD `cf7a5b0`; log `260824-1637-reconciliation.md`) — **STAYS `_o_`.** `grep -n 'malformed' hooks/lib/__tests__/fusion-identity.test.ts` returns nothing, so the branch is still untested. The property itself was confirmed live during this pass, by accident rather than by design: a probe that removed `tr` and `grep` from `PATH` drove the helper into the malformed branch, which reported "It was not overwritten; inspect it, and delete it only if no record cites it" and left `fusion-workbench/.checkout-id` byte-identical at `5e8248d7`. The behaviour holds and nothing in the suite would notice if it stopped.
