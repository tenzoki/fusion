...
**Why this is not academic for exit 4.** The whole point of `bin/fusion-identity`'s exit-4 design, and of the decision behind it (`260824-0613_*_does-a-filing-agent-halt-in-a-tree-that-is-not-a-git-work-tree-at-all.md`, option 2), is that a single user in a non-git directory keeps working. That user can activate a Circle. Today the two activation routes have nothing to write in the field.
