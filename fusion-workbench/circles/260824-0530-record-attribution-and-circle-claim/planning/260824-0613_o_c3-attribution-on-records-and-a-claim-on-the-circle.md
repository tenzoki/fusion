...
**The eight properties of `## Where this Circle stops`, checked against the tree.** Seven hold, four of those with a named exception, and one is false as written.
...
**Twelve open findings are the user's explicit decision and are not drift.** Review pass 1 filed eleven, the user chose to fix the two high-severity ones, and pass 2 filed three more. Each was re-checked against the tree by this pass and each is still open on its own evidence; the evidence is appended to each record.
