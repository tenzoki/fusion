...
**Date:** 2026-08-29
**Status:** Draft
**Spec:** none; the Directive is `## Directive` of `circles/260828-2342-citation-form-drops-store-segment/_t_circle.md`, written by the shaper into the record, and it is not restated here
...
2. **The grammar resolves storelessly and reports a store segment as a violation**
   - Executor: `coder`
...
3. **The sweep script, `hooks/scripts/citation-sweep.mjs`**
   - Executor: `coder`
...
4. **The rule text states the form, the violation, the scope of uniqueness, and the reach**
   - Executor: `coder`
...
5. **The 19 `$SCAN_*` self-citations say the record is fusion's own, and a lint keeps the key away from a stamp**
   - Executor: `coder`
...
7. **The archive safety filter matches the storeless form, and a probe pins it**
   - Executor: `coder`
...
9. **`/fusion:cleanup` prints the verdict line, paid for by one cut**
   - Executor: `coder`
...
10. **Run the sweep, hand-finish the residual, re-approve the pins, and commit B**
    - Executor: `coder`
...
12. **Bookkeeping of the records this Circle realises**
    - Executor: `coder`
...
- The five `260828-0904_*` decisions carry `Implemented:` and the `_i_` marker; issues `260828-0828_*`, `260828-0900_*`, `260828-0901_*`, `260828-0907_*` carry `Resolved:` and `_c_`.
...
| The hook-test budget is spent before step 8's tests land | Step 1 is measured first; if the freed room is under what steps 2 to 8 need, the coder rolls the pin re-approval prose of `reference-resolution-lint.test.ts:479` into a `shared/analyses/` entry (the mechanism decision `260822-1229` option 2 established) before adding a line |
...
| A consumer's cleanup prints `helper-missing` for one release | Stated in the docs note and the CLAUDE.md row; it is the standing cost of every `bin/` helper (`260825-1329_*`) |
