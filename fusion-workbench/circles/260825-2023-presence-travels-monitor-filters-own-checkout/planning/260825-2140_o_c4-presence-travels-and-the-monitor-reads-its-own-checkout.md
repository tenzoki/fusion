...
11. **The session identifier, if the measurement permits it**
    - Executor: `coder`
...
5. The Turn count has one definition and one implementation, and all four sites that print or define it read that one implementation.
6. The three defect records referred here carry a closing note citing the commit that discharged them, or state which part was not discharged and why.
