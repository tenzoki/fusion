...
---
Resolved: the first branch, by the commit that carries this line. The classifier's reading is the one that runs and `rules/workbench-tracking.md` already states it (class R1: the store sits inside `shared/`, one writer per file, tracked), so the cleanup body moves to it rather than the rule and the classifier moving to the body's. `skills/cleanup/SKILL.md` `## Step 3 — Push` now names `.fusion-setup` alone as the live state a session dirties and no split names, and `## Step 2 — Commit in meaningful splits` names this checkout's registry entry among the workbench records a split carries. The two sentences and `hooks/lib/staging-drift.ts` now say one thing about a modified `shared/checkouts/` entry: it is a record, and a split commits it. Seven bytes net on the skill surface, no path token added.
