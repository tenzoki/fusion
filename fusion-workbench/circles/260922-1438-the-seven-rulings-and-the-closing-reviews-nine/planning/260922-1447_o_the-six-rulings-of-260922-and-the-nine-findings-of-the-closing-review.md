...
8. [DONE] **Say in all four places which route writes an edge without a per-entry ruling**
   - Executor: `coder`
   ...
   - Growth: `agents/`, about +250 bytes; `rules/` is unbounded, about +150 bytes on an always-on file. **Landed: `agents/` +771 bytes**, 321 524 -> 322 295, head-room 6 272; the rule file +365, 68 859 -> 69 224.
   - Pin: paths and anchors may each rise by one or two (the orchestrator anchor cited from three files); re-approve on line 492. **Landed: paths 1708 -> 1711, anchors 317 -> 320**, one anchor citation per normative site; `docs/working-model.md` carries the change in prose and cites nothing.
   - **One acceptance reading differs from the plan's.** `grep -c 'edges you confirmed' docs/working-model.md` reaches `0` only after a second site is reworded: the helper paragraph at `:172` carried the same wording about confirmed edges, which the finding did not name. It now reads that the helper reports over the edges as the store carries them.
   - Gate: none
...
10. [DONE] **Hold the `writer=` and `skipped=` lines in the news body, and tell the user about a skipped path**
    - Executor: `coder`
    ...
    - Growth: `skills/`, about −157 net (about +240 against 397 of cuts). If step 2 took the `:16` bullet forward, the cut here is 216 and the net is about +24, stated in the commit message. **Landed: −162 net**, `news/SKILL.md` 8 373 -> 8 211, `skills/` 227 974 -> 227 812, head-room 54 -> 216. Step 2 landed inside its own bound and took no cut forward, so both cuts were available here. `fixtures/surface-growth.golden` was regenerated by its own failing-on-purpose run; no baseline moved.
    - Pin: unmoved. **Landed: unmoved**, no re-approval; the cut bullets and the added sentences carry no token of any pinned class.
    - Gate: none
    - Dependencies: none
...
15. [DONE] **Re-arm the eleven dispatch-path rows at the post-cut measurement, as a named event of that bound alone**
    - Executor: `coder`
    ...
    - Growth: hook tests, at most +2 lines in `rules-emission-golden.test.ts` (the fixture is not a `.ts` and `README-hooks.md` is covered by no bound), against about 55 after step 14. The dispatch-path bound itself goes to zero slack on every row by construction, which is the point of the step; no later step in this package touches a charged component. **Landed: +3 lines**, not 2: `DISPATCH_HEAD_ROOM`'s doc comment went from four body lines to seven, the event's one condition needing a sentence the estimate gave a clause. Hook tests 22 201 -> 22 204, head-room 57 -> 54. The golden was regenerated; the only baseline that moved is the one this step is for.
    - **Landed: all eleven rows at zero slack**, verified by a fresh measurement after the write, run the way `dispatchComponents()` runs it: analyst 198 789 -> 123 995, coder 188 256 -> 106 836, consultant 197 679 -> 122 046, curator 227 066 -> 187 121, editor 202 739 -> 127 524, ontocoder 191 869 -> 110 546, orchestrator 380 065 -> 222 589, planner 202 428 -> 127 593, reconciler 206 384 -> 120 813, reviewer 189 012 -> 114 309, shaper 250 768 -> 135 872. `helpers/growth-bound.ts` and its heading stand unedited, `DISPATCH_HEAD_ROOM` is still `0`, and no other baseline or head-room constant moved.
    - Pin: paths and anchors may rise by one or two in `README-hooks.md`; re-approve on line 492. **Landed: paths 1712 -> 1717, anchors unmoved at 319.** Measured by restoring `README-hooks.md` to HEAD in place, where the gate reads 1712/319 with the re-armed fixture and the edited test file standing, so neither of those carries a pinned token and the whole delta is the README log: three `CLAUDE.md` spellings plus the fixture and `helpers/growth-bound.ts`. Re-approved on line 492.
