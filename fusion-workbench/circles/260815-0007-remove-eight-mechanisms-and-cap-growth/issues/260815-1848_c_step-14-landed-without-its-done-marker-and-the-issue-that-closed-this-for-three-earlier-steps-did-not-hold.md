...
This is the second occurrence in this Circle. `260815-0804_*_three-plan-steps-have-landed-and-the-plan-carries-no-inline-state-marker-for-any-of-them.md` recorded the same defect for steps 1, 2 and 3 and was closed by adding the markers. Adding the missing markers fixes the instances; it does not fix whatever lets an executor commit a step without touching the plan, so the class recurred eleven steps later.
...
`260815-0029_*_…:351` reads `14. [DONE] **Take the after-measurement** *(closure criterion)*`, so the instance is gone. The record was explicit that it names the recurrence and not the instance, so that does not close it.
...
---
Resolved: referred (backlog) — a check that a commit carrying `Task: P-<n>` and a `Source:` plan implies step <n> of that plan is `[DONE]` is the idea; backlog entry to be filed by the user
