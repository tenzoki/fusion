...
---
**Resolved:** fixed in the commit that carries this line. `hooks/lib/review-coverage.ts` caller entry 1 now names `agents/orchestrator.md` `## Review coverage`, `## Closing a work item` step 2 and `## Ending the session`, in the CLI header's reading; `hooks/dist/lib/review-coverage.js` and `.d.ts` rebuilt. The same stale address stood at a fourth site, `hooks/tracker.ts` ("Phase 4 step 2a"), and at two present-tense comment lines in `review-coverage-mandate.test.ts` and `plan-stopping-section-lint.test.ts`; all three repointed in the same commit. Three past-tense lines that quote where the addresses *used to* sit stay as history. `grep -c` over the three named files prints 0 for each; `hooks/review-coverage.ts` untouched.
