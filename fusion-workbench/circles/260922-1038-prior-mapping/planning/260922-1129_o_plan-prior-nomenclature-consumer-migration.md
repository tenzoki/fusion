...
**Survey commit:** `ad5535df`, working tree of 2026-09-22 11:19. The tree carries uncommitted work of the item this checkout holds (`hooks/lib/work-graph.ts`, `hooks/order.ts`, `hooks/lib/__tests__/work-graph.test.ts` +11 lines, `bin/fusion-work-order`); `npx vitest run lib/__tests__/surface-growth-bound.test.ts` is red on the `hook-tests` golden for those 11 lines alone (`work-graph.test.ts 156` → `167`), and green on the two byte surfaces. Every figure below names the command that produced it.
...
What was re-verified at `ad5535df`, and where it differs from the spec's survey at `6420c14e`:

- **The skill body's byte figures hold.** `wc -c skills/migrate/SKILL.md` = 39 701. `awk '/^## Step 2/{f=1} /^## Guardrails/{f=0} f' skills/migrate/SKILL.md | wc -c` = **33 925**, the pre-v4 conversion text (Step 2 through Step 5). The rest: 3 513 bytes before `## Step 1` (535 of them the frontmatter, so 2 978 of preamble against the spec's 2 977), 366 for Step 1, 1 897 for the guardrails. `skills/setup/SKILL.md` = 19 424, of which the four probes' list and bullets (`:38-52`) are 3 209 bytes, the probe block (`:54`) 1 137, and the refusal text (`:56-64`) 1 346.
- **The skill surface's head-room is 54 bytes.** `cat skills/*/SKILL.md | wc -c` = 227 974 over 14 bodies; `SKILL_BASELINE` (`hooks/lib/__tests__/surface-growth-bound.test.ts:191-210`, nine entries, the five bodies with no entry counting in full) sums to 188 768, `SKILL_HEAD_ROOM` (`:278`) is 39 260, budget 228 028. `migrate/SKILL.md`'s own entry is 26 620, so it stands 13 081 above it, as the spec says.
- **The hook-test surface has 138 lines at HEAD and 127 in this working tree.** `TEST_LINE_BASELINE` sums to 19 228, `TEST_LINE_HEAD_ROOM` is 3 030, budget 22 258; `surface-growth.golden` records 22 120 at HEAD. `live-circle-record-detection.test.ts` (77 lines) has **no baseline entry**, so it counts as growth in full today and deleting it returns 77 lines of room.
- **The store-path carriers of C4 hold, with two additions that change no treatment.** `**Active spec/plan:**` over `fusion-workbench/circles/*/*.md`: 31 value lines, 0 carrying `planning/`, `shared/` or `circles/`. `.guard-state/`: `circles/` stands in `citation-form.json`, `staging-drift.json` **and `events.jsonl`** (821 lines); the directory is class L in full (`rules/workbench-tracking.md` `## The four classes`), so the third file takes the same treatment. `orchestrator-events.jsonl`: 4 567 rows, 82 with `circles/`, 43 with `planning/`. `shared/forum/`: 0 of 3 with a store path. `.gitignore`: 0 lines naming `circles`, `shared/planning` or `shared/consult`. `archive/`: four sweeps, and **two** of them (`260817-1907`, `260828-0043`) hold a `circles` subtree, not one as the spec reads; never opened either way.
- **This workbench's root carries entries the spec's table does not name.** `agentstate.yaml`, `orchestrator-live.md`, `portfolio.md` (retired root files, every one listed in `.gitignore:89-105` as class L; `portfolio.md` carries 0 `circles/` lines), `shared/backlog/` (`LEGACY_STORES` in `hooks/lib/stores.ts`, the pre-container backlog store the tree no longer names) and `shared/.DS_Store`. Each is a case for the classification table in `## Approach`; the pass would otherwise meet an entry it has no rule for on its first run.
- **Counts.** 37 containers, 34 with `planning/`, 18 empty directories under `circles/`, 8 files under `shared/planning/`, 2 under `shared/consult/`, 2 977 tracked files under `fusion-workbench/`, 14 untracked entries under `circles/` (this container among them), one item claimed by this checkout (`260922-0906-fix-package-over-every-open-issue`). `stashes/` and `.migration-v2-backup/` are absent here.
- **The existing tests that read the pre-v4 passes.** `hooks/lib/__tests__/live-circle-record-detection.test.ts` lifts setup's probe block and migrate's second survey block by heading anchor (`helpers/prompt-blocks.ts` `extractBashBlock`), builds a throwaway workbench under `mkdtempSync`, and runs them with a real `bash`: **that is the fixture-based shell-level pattern the dispatch asked for, and the repository already has it.** `path-literal-lint.test.ts:273-287` pins the bracket-probe `find` expression byte-identical across setup's probe, migrate's survey and migrate's reformat pass (three sites). `marker-format-lint.test.ts:43` exempts `setup` and `migrate` and `:94-105` asserts both still name the bracket form. `fenced-code-exemption.test.ts:190-194` reads `RETIRED_LAYOUT_FILES["skills/migrate/SKILL.md"]` (`hooks/lib/citation-scan.ts:564`), whose reason string describes "the pre-v4 -> v4 layout conversion". `derivable-enumerations-lint.test.ts` parses `const SEL = [...]` in setup and the skill-row table in `README-agents.md`; neither the probes nor migrate's prose are enumerations it holds.
- **The version-read idiom exists.** `skills/setup/SKILL.md:72`: `V="$(grep '"version"' "$FUSION_PLUGIN_ROOT/.claude-plugin/plugin.json" | head -1 | sed -E 's/.*"version": *"([^"]+)".*/\1/')"`. The installed copy here (`~/.fusion/.claude-plugin/plugin.json`) reads `11.10.0`, equal to HEAD; the newest tag is `v11.10.0`.
...
| left, no record owed | `shared/analyses`, `shared/investigations`, `shared/decisions`, `shared/reviews` (Retain), `.fusion-setup` (the marker), `shared/backlog` (a legacy store the tree no longer names, its records cited), `agentstate.yaml`, `orchestrator-live.md`, `portfolio.md`, `.active-circle` (retired root files `.gitignore` already names as class L; nothing reads them) | reported in one line each; the retired four named as deletable by hand |
| frozen | `archive`, `stashes`, `.migration-v2-backup` | never opened, listed byte-identical before and after (C2) |
...
- **Steps 1 to 4 here land before plan (1) step 7.** Plan (1) step 7 retargets `mkdir -p` and `printf` targets in migrate's Step 2 and Step 4 blocks (`:59`, `:118`), keeps the three-site pin at `path-literal-lint.test.ts:273-287` byte-identical, and re-fixtures `live-circle-record-detection.test.ts`; steps 1, 2 and 4 here delete all three. Landing first leaves plan (1) step 7 its setup half only (the `mkdir` retarget and the fifth probe with its window note) and gives that step some 26 000 bytes of skill room instead of the 54 it planned to cut for. Plan (1) step 3 also lists `live-circle-record-detection.test.ts:38` among the fixtures it moves; step 4 here deletes the file.
...
- **Shared budgets.** The skill surface: steps 1 to 3 free some 39 000 bytes and step 2 spends at most 14 000; what remains is what plan (1) steps 7 and 11 draw on. The hook-test surface: step 4 returns 93 lines (77 + 16) and spends at most 180 on the new test, so the surface ends step 4 at no more than 87 lines over today's figure, inside the 138 at HEAD; plan (1) step 4 then holds what is left, and a comment-prose cut (`260822-1154_*_does-the-hook-test-line-budget-cover-comment-prose.md`) is the remedy for whichever plan runs short, never a baseline move.
...
|---|---|---|---|---|---|
| HEAD `ad5535df` (measured) | 39 701 | 19 424 | 16 183 | 227 974 | 54 |
| 1 (cut, frees 33 925 plus the rewritten preamble and guardrails) | ≈ 5 800 | 19 424 | 16 183 | ≈ 194 000 | ≈ 34 000 |
| 2 (write, spends ≤ 14 000) | ≤ 19 800 | 19 424 | 16 183 | ≤ 208 100 | ≥ 19 900 |
| 3 (setup cut, frees ≈ 5 400 of the 5 692 measured) | ≤ 19 800 | ≈ 14 000 | 16 183 | ≤ 202 700 | ≥ 25 300 |
| 7 (help sentence, spends ≤ 300) | ≤ 19 800 | ≈ 14 000 | ≤ 16 500 | ≤ 203 000 | ≥ 25 000 |
...
2. **Write the store-name pass**
   ...
   - Changes, in the skill's existing shape and counter discipline:
     ...
     - `## Step 5 — Report`: the counters; one line per left-by-rule entry with its record; the `.gitignore` hits; every untracked entry moved by `mv` (what `git revert` will not restore); what to do next in this order: commit the migration as one commit and push it, tell the other checkouts to pull (C7), run `/fusion:setup`, optionally `bin/fusion-citation-sweep --dry-run` for the store-prefixed citations the pass did not touch (C4). In `plain` mode, that nothing here is in a diff.
     - `## Guardrails` as step 1 left them, plus: never rename inside `archive/`, `stashes/` or `.migration-v2-backup/`; never open a record.
   - Dependencies: step 1 (same commit)
   - Acceptance: the four C10 acceptance criteria of the spec that concern the skill body (first and second bullets); C2's acceptance over a fixture (step 4 proves it); every `find` in the body is anchored at `./fusion-workbench` and none reaches `archive/`; `wc -c skills/migrate/SKILL.md` ≤ 19 800 and the surface total recorded in Appendix A; `bash -n` on every extracted block exits 0.
...
U. **The run** (performed by the user, not an executor: `/fusion:migrate` is a slash command with its own confirmation, and the version guard makes it runnable only in a session whose installed plugin is `12.0.0`). The user runs it in this repository, reads the survey (expected: 37 containers of which 34 fold a `planning/`, 8 and 2 entries under the two shared stores, 18 empty directories, 0 collisions, 0 untracked entries, `LEFT` naming `shared/backlog`, the three retired root files and the Review-class stores with their records), and confirms. The confirmation gate is the skill's own `AskUserQuestion`; nothing in this plan pre-answers it.
...
- If the user rules option 2 or 3 on `260922-1125_*_does-the-store-name-migration-carry-its-mechanics-in-the-skill-body-or-in-a-bin-helper.md`, the work stops after step 1 and steps 2 and 4 are re-planned around the helper.
...
| The hook-test surface is shared with plan (1) step 4 and has 127 lines in this tree | The new test is capped at 180 lines with 93 returned, and the comment-prose cut is the named remedy for either plan; no baseline moves. |
...
- [ ] `260922-1125_*_does-the-store-name-migration-carry-its-mechanics-in-the-skill-body-or-in-a-bin-helper.md`: option 1 is what steps 2 and 4 realise; the user rules at the plan gate.
...
- [ ] Step U expects the user to run `/fusion:migrate` interactively after `fusion --update`. The spec allows a headless `claude --plugin-dir .` proof before the release; this plan does not plan one, because the fixture test already proves the blocks and a headless run cannot answer the skill's own confirmation. Say so if a headless run is wanted anyway.
...
|---|---|---|---|---|---|
| baseline `ad5535df` | `skills/*/SKILL.md` bytes | 227 974 | | 54 | |
| baseline `ad5535df` | hook tests lines (HEAD golden) | 22 120 | | 138 | |
| baseline `ad5535df` | `CLAUDE.md` bytes | (step 6 records) | | 0 on every dispatch path | |
| baseline `ad5535df` | `rules/fusion-workbench-conventions.md` bytes | (step 5 records) | | 0 on every dispatch path | |
