...
| 3 | Session histories | `$SCAN_HISTORY` = `circles/260801-1244-curator/history shared/history` | 217 | 1 in full (`shared/history/260812-1232-planner-remove-the-protected-path-half.md`), all 217 enumerated by name and date | The full corpus across every Circle is 415 |
...
| 5 | Reviews and analyses | `$SCAN_REVIEWS` = 25 files, `$SCAN_ANALYSES` = 15 files | 40 | 1 read in substance (`shared/analyses/260813-0828-documentation-staleness-survey.md`), 40 enumerated | The full corpus across every Circle is 62 reviews and 21 analyses |
...
| 7 | Archive store | `$WORKBENCH/archive` | **0 files** | — | Explicitly zero. `find archive -type f` returns nothing. Open defect `shared/issues/260801-1020_*_scan-keys-never-reach-the-archive-store.md` is the reason no resolver key points here |
...
**No live decision record in the editable surface supersedes another, across the 19 pairs the selection rule reached.** Four of the 19 say so in their own text: `shared/decisions/260719-2141_i_plane-rolle-push-only-vs-bounded-readback-martin.md` states "Refines D1 … does not supersede it", and `shared/decisions/260716-1847_i_plane-rolle-source-of-truth.md` records the same judgement from the other side. The corpus authors were explicit about the refine-versus-supersede distinction rather than silent on it.
...
- The branch policy. `shared/decisions/260809-2310_i_should-the-branch-policy-fall-the-way-the-write-classifier-fell.md` decided the deletion and records it in its own `Implemented:` line (`7598073`, verified: `git log --diff-filter=D` names it removing `hooks/lib/git-branch-guard.ts`, `shell-parse.ts`, `command-word.ts` and `rules/git-branch-discipline.md`). No earlier record established the policy, so there was nothing to supersede.
- The protected-path half, removed 2026-08-12 (`60c9cd8`, `3a83133`, `fa2f00b`). It was authorised **by the user directly at 260812-1230** and planned in `shared/planning/260812-1232_*_remove-the-protected-path-half-of-the-compliance-guard.md`. No decision record authorised it. Two records were filed as successor questions and both are still open.
- The write classifier (`ba7ccda`), whose own record `circles/260804-1205-shell-reachability-model/decisions/260807-0825_*_...` now carries a parenthetical saying the replacing measurement has since been removed too.

The consequence is a relation the marker vocabulary has no state for: **an implemented decision whose implementation was later deleted, with no superseding decision to cite.** At least two such records sit in the editable surface — `shared/decisions/260801-1020_i_may-any-fusion-writer-touch-rules.md`, whose whole answer was the `FUSION_ALLOW_RULES_WRITE` exemption and the project `protectedPaths` leaf, and `shared/decisions/260809-1527_i_should-the-revert-narrow-to-the-payload-path-for-the-four-write-tools.md`, whose subject was the revert. Roughly eleven more sit in closed Circles, outside this run's editable surface.
...
- **File:** `CLAUDE.md`
- **Tier:** 1
- **Citation:** `wc -c rules/design-diagrams.md rules/circle-records.md rules/workbench-stash-and-lock.md` → `30574 total`, against the claimed 30 588. The three files are exactly the indented `emit_if_exists` lines, `grep -n "^[[:space:]]\+emit_if_exists" bin/fusion-rules` → 422, 440, 455. The same paragraph says three sentences earlier that "any number written into this line is stale before it is committed", so this is also a surface-pair-6 contradiction. Corroborating, and named as a stop for anything beyond the derivation: open defect `circles/260813-0910-documentation-matches-shipped-plugin/issues/260813-2009_*_the-byte-budget-line-bans-a-number-and-then-states-one-two-sentences-later.md`, and open decision `circles/260813-0910-documentation-matches-shipped-plugin/decisions/260813-1820_*_how-fusion-s-own-documentation-treats-a-hand-measured-number-that-decays.md`, whose reconciliation note already names this exact re-introduction.
- **Derive-over-correct:** the entry proposes the derivation, not 30 574. The sentence needs the figure only to say the conditional set is roughly a third again on top of the floor, and the command that produces it is the same `wc -c` the preceding sentence already names. The precedent is the implemented decision `circles/260801-1244-curator/decisions/260814-0845_*_are-the-sixteen-agent-claims-corrected-or-derived-away.md`, option 2: remove the figure where the sentence does not need it.
- **Consequence group:** tier-1
- **Constraint removed:** none
- **Revert path:** `git checkout -- CLAUDE.md`
...
The repair form is not invented here. `rules/rule-file-provenance.md` defines the wildcarded marker `YYMMDD-HHMM_*_<slug>.md` and states the reason: "a record's marker moves with its life … so a citation carrying a literal marker dies at the record's first transition". The form was chosen in `circles/260805-2005-textschicht-gegen-code-nachziehen/decisions/260806-0015_*_zitierform-fuer-workbench-records.md`, and most of this corpus already uses it — several of the records below mix the two forms in one line.

**Why nobody caught these:** `hooks/lib/__tests__/reference-resolution-lint.test.ts` resolves workbench-record citations, and it passes at HEAD (33 tests green, run 2026-08-14). It does not scan the workbench itself, which is open defect `shared/issues/260812-1720_*_the-reference-resolution-lint-does-not-scan-the-workbench-where-citations-are-densest.md`. Every finding below is inside that blind spot, and none is inside a surface the lint does cover.
...
#### L05 — `shared/decisions/260706-1902_i_consultant-chat-longform-boundary.md:7`

- **Citation:** neither path resolves. `find . -name '*consultant-chat-misrouted*'` → `./shared/issues/260706-1902_c_consultant-chat-misrouted-to-longform-voice.md`; `find . -name '260706-1902*'` → `./shared/analyses/260706-1902-user-facing-agents-garbled-language-rootcause.md`. Both carry the pre-v4 `fusion-workbench/<type>/` prefix and the first carries a bracket marker.
...
#### L06 — `shared/decisions/260716-1847_i_offline-verhalten-bei-plane-ausfall.md:7`

- **Citation:** neither path resolves. The first is `shared/decisions/260716-1847_i_plane-rolle-source-of-truth.md`; the second is `circles/260716-1847-workbench-umbau/planning/260716-1847_c_spec-plane-integration-und-workbench-struktur.md`, verified by `find . -name '*spec-plane-integration*'`.
...
#### L07 — `shared/decisions/260716-1847_i_plane-rolle-source-of-truth.md:7` and `:49`

- **Citation:** the two header paths are L06's pair, reversed. The `Answered:` path `history/260716-1800-orchestrator-session.md` does not resolve either; `find . -name '260716-1800*'` → `./circles/260716-1847-workbench-umbau/history/260716-1800-orchestrator-session.md`.
...
#### L08 — `shared/decisions/260801-1020_i_may-any-fusion-writer-touch-rules.md:7`

- **Citation:** `shared/issues/260801-1020_o_guard-protects-rules-but-not-claude-rules.md` does not resolve; the record is now `_c_`. Note the same line already uses the wildcard form for its sibling decision, so the literal marker is the outlier within one sentence.
...
#### L09 — `shared/decisions/260807-2131_i_which-language-governs-a-customer-deliverable.md:10` and `:11`

- **Citation:** `260807-1515_a_…` is now `_i_`; `shared/planning/260807-2024_o_two-language-declarations.md` is now `_c_`.
...
#### L10 — `shared/decisions/260809-1527_i_should-the-revert-narrow-to-the-payload-path-for-the-four-write-tools.md:7`
...
#### L11 — `shared/decisions/260809-1731_i_how-should-the-domain-heuristic-count-a-projects-source-files.md:7`

- **Citation:** both `260807-1951_o_…` and `260807-1942_o_…` are now `_c_`. The third citation on the same line, `circles/260801-1244-guard-rules-write/issues/260805-1830_o_…`, **does** resolve and is left alone.
...
#### L12 — `shared/decisions/260809-2004_i_should-the-latching-churn-and-cross-file-criticals-be-bounded-or-dropped.md:7` and `:124`

- **Citation:** `circles/260801-1244-guard-rules-write/issues/260805-1859_o_…` is now `_c_`; `shared/issues/260809-2023_o_…` is now `_c_`. Four other citations on line 7 resolve and are left alone.
...
#### L13 — `shared/decisions/260809-2310_i_should-the-branch-policy-fall-the-way-the-write-classifier-fell.md:7`

- **Citation:** `shared/issues/260809-2300_o_…` is now `_c_`. The same line already uses the wildcard form for the shell-reachability decision, so the literal marker is the outlier within one sentence.
...
#### L14 — `shared/decisions/260810-0920_i_what-should-a-churn-key-be-anchored-to-and-what-happens-to-the-535-entries-already-recorded.md:7`

- **Citation:** `shared/issues/260809-2023_o_…` is now `_c_`. The sibling decision on the same line already uses the wildcard form.
...
#### L15 — `shared/decisions/260810-0921_i_how-should-a-prompt-call-a-bin-helper-that-the-installed-copy-may-not-have.md:7` and `:137`

- **Citation:** `shared/issues/260810-0352_o_…` is now `_c_`; `shared/decisions/260810-1544_o_…` is now `_a_`. The second is the mutual-citation half of L16 — the two records name each other and both citations died.
...
#### L16 — `shared/decisions/260810-1544_a_should-prompt-called-bin-helpers-get-one-guarded-call-convention-and-does-the-work-tree-preference-extend-to-them.md:7`

- **Citation:** `shared/decisions/260810-0921_a_…` is now `_i_`. The second citation on the same line, `shared/issues/260810-0352_c_…`, resolves and is left alone.
...
#### L17 — `shared/decisions/260810-1635_a_where-does-the-obligation-sit-to-update-the-artefact-that-explains-a-behaviour-when-the-behaviour-changes.md:7`

- **Citation:** `260809-2252_o_…` and `260810-0502_o_…` are now `_c_`. The other two issues on the same line, `260809-2258_o_…` and `260810-1632_o_…`, are genuinely still open and resolve as written — they are left alone.
...
#### L18 — `shared/decisions/260810-2032_a_should-the-drift-checks-four-sentences-be-pinned-to-an-approved-baseline-instead-of-screened-by-a-blacklist.md:114`

- **Citation:** `shared/issues/260801-2038_o_…` is now `_c_`, so the sentence's word "open" is false as well as the path.
...
#### L19 — `shared/decisions/260810-2145_a_should-a-repeated-skill-body-snippet-become-a-bin-helper-now-that-one-fact-lives-in-four-executable-copies.md:7`

- **Citation:** `shared/issues/260810-2030_o_…` is now `_c_`. The second issue, `260810-2110_o_…`, is genuinely open and resolves — left alone.
...
#### L20 — `shared/decisions/260812-1232_o_does-the-escalation-counter-survive-a-block-source-that-ships-inert.md:9`

- **Citation:** `shared/planning/260812-1232_o_remove-the-protected-path-half-of-the-compliance-guard.md` is now `_c_`.
...
#### L21 — `shared/decisions/260812-1232_o_does-the-write-guards-fusion-repo-stand-down-survive-the-loss-of-its-subject.md:8`
...
#### L22 — `shared/decisions/260812-2043_i_who-writes-the-recommended-marker-on-a-backlog-entry.md:7` and `:75`

- **Citation:** the plan is now `_c_`; `shared/decisions/260812-0254_a_does-fusion-need-a-backlog-store…` is now `_i_`; `circles/260813-0858-playmaker-maintains-backlog-store/issues/260813-1545_o_…` is now `_c_`.
...
#### L23 — `shared/decisions/260813-0027_o_should-the-orchestrator-be-able-to-dispatch-the-shapers-portfolio-activation-mode.md:7`

- **Citation:** `circles/260801-1244-curator/_a_circle.md` does not resolve; the record is now `_t_circle.md`, because the Circle was activated on 2026-08-14. The correct form for a Circle citation is the **directory name** — `rules/rule-file-provenance.md` citation form 2: "The directory name is used rather than the record filename, because the directory is stable across the Circle's whole lifecycle." The same line already uses the wildcard form for its two record citations.
...
`candidate` because **exclusion 1 applies**: deciding which of the two is right is ground-truth verification, and `agents/reconciler.md` owns that. Two open defects already carry the class: `shared/issues/260811-2146_*_half-the-decision-records-carry-a-status-that-disagrees-with-their-marker-and-twelve-keep-the-unfilled-template-stub.md` and `shared/issues/260812-1232_*_thirty-four-of-seventy-four-decision-records-carry-a-status-header-that-contradicts-their-filename-marker.md`. Both corroborate rather than contradict, so this is a queue-position finding, not a stop.
...
`shared/decisions/260813-0826_a_should-fusion-help-become-a-self-knowledge-skill-that-answers-from-the-live-installation.md:7` cites `shared/issues/260813-0825_o_the-user-facing-documentation-lags-two-releases-and-still-describes-a-removed-guard.md`. It does not resolve under any marker, and `git log --all --diff-filter=ADR --name-status -- 'fusion-workbench/shared/issues/260813-0825*'` shows commit `799fded` adding exactly two files at that stamp, neither with this slug. `candidate` because I cannot state the intended target — `260813-0825_o_the-v8-1-0-documentation-step-reached-three-files-and-the-feature-reached-seven-surfaces.md` is the closest, and that is `speculation:`, not a citation.

#### C03 — an answered decision's documentation instruction never landed

`shared/decisions/260806-1152_a_stash-manifest-dirname-and-pointer-content-duplicate.md` was answered on 260811 (option 1, keep both fields) and ends "Record the reason in the manifest schema documentation so the next reader does not re-file the duplication as redundancy." `rules/workbench-stash-and-lock.md` `### Manifest schema` does not record it — verified by reading lines 60-95. `candidate` because **neither of my two reasons applies**: silence is not a contradiction, and nothing was retired. The change is one added sentence to a rule file. Owner: `coder`, on the decision's own instruction, together with the golden regeneration the added bytes will require; and `agents/reconciler.md` for the `_a_`→`_i_` walk afterwards.
...
| L05 | `shared/decisions/260706-1902_i_consultant-chat-longform-boundary.md` | applied |
| L06 | `shared/decisions/260716-1847_i_offline-verhalten-bei-plane-ausfall.md` | applied |
| L07 | `shared/decisions/260716-1847_i_plane-rolle-source-of-truth.md` | applied (2 substitutions) |
| L08 | `shared/decisions/260801-1020_i_may-any-fusion-writer-touch-rules.md` | applied |
| L09 | `shared/decisions/260807-2131_i_which-language-governs-a-customer-deliverable.md` | applied (2) |
| L10 | `shared/decisions/260809-1527_i_should-the-revert-narrow-…md` | applied |
| L11 | `shared/decisions/260809-1731_i_how-should-the-domain-heuristic-…md` | applied (2) |
| L12 | `shared/decisions/260809-2004_i_should-the-latching-churn-…md` | applied (2) |
| L13 | `shared/decisions/260809-2310_i_should-the-branch-policy-…md` | applied |
| L14 | `shared/decisions/260810-0920_i_what-should-a-churn-key-…md` | applied |
| L15 | `shared/decisions/260810-0921_i_how-should-a-prompt-call-a-bin-helper-…md` | applied (2) |
| L16 | `shared/decisions/260810-1544_a_should-prompt-called-bin-helpers-…md` | applied |
| L17 | `shared/decisions/260810-1635_a_where-does-the-obligation-sit-…md` | applied (2) |
| L18 | `shared/decisions/260810-2032_a_should-the-drift-checks-…md` | applied |
| L19 | `shared/decisions/260810-2145_a_should-a-repeated-skill-body-snippet-…md` | applied |
| L20 | `shared/decisions/260812-1232_o_does-the-escalation-counter-survive-…md` | applied |
| L21 | `shared/decisions/260812-1232_o_does-the-write-guards-fusion-repo-…md` | applied |
| L22 | `shared/decisions/260812-2043_i_who-writes-the-recommended-marker-…md` | applied (2) |
| L23 | `shared/decisions/260813-0027_o_should-the-orchestrator-be-able-…md` | applied |
...
- **Derive-over-correct:** the entry does **not** re-measure the line numbers. A line-number citation into a source file is a measurement that decays on the next edit above it, and this project has that filed as `shared/issues/260808-0030_*_line-number-citations-into-rule-files-go-stale-and-no-gate-reads-them.md`, still open, with `shared/issues/260811-1755_*_stale-marker-citations-recur-and-the-lint-does-not-read-the-hook-entrypoints-where-one-was-hiding.md` beside it. Naming the consumer rather than the line survives every edit and still tells the reader where to look. This is also the shorter form, so it costs the growth bound nothing.
...
- **Tier:** 1
- **Citation:** `ls -a fusion-workbench/ | grep -i plane` → `.plane-map.json`, `.plane-outbox.jsonl`, `plane.config.yaml`. `grep -n "plane" rules/fusion-workbench-conventions.md` → no matches anywhere in the file. `grep -n "CONFIG_FILE=\|MAP=\|OUTBOX=" bin/fusion-plane` shows all three anchored to `$WORKBENCH`. Line 70 of the same file says "The list is exhaustive as written… When a `bin/` helper or a hook adds a root-anchored surface, it lands in this tree in the same commit", so the file contradicts its own exhaustiveness claim — a surface-pair-4-internal contradiction. Corroborating open defect: `shared/issues/260810-0410_*_the-layout-tree-calls-itself-exhaustive-and-omits-the-two-plane-runtime-files.md`, which names two of the three.
...
- **Filed alongside:** `circles/260801-1244-curator/issues/260814-1332_o_the-voice-profile-fallback-is-performed-by-the-helper-so-the-agent-cannot-record-it.md` — owner `coder`.
...
"This is the input to the reference conversion (a later Circle-B step) and to the docs pass (Circle E)." `ls fusion-workbench/circles/260718-1924-v5x-overhaul/planning/` shows `260719-1416_c_plan-circle-e-rest-docs-cleanup-v5-close.md`, closed, and the whole v5x Circle carries `_c_circle.md`. `candidate`: Tier 3 in principle, but the sentence is a lede describing why the file was written, not a normative claim, and no reader is misled about what to do. Rewriting it buys nothing and costs a golden regeneration.
...
### `shared/decisions/260706-1902_i_consultant-chat-longform-boundary.md`
...
### `shared/decisions/260716-1847_i_offline-verhalten-bei-plane-ausfall.md`
...
### `shared/decisions/260716-1847_i_plane-rolle-source-of-truth.md`
...
### `shared/decisions/260801-1020_i_may-any-fusion-writer-touch-rules.md`
...
### `shared/decisions/260807-2131_i_which-language-governs-a-customer-deliverable.md`
...
### `shared/decisions/260809-1527_i_should-the-revert-narrow-to-the-payload-path-for-the-four-write-tools.md`
...
### `shared/decisions/260809-1731_i_how-should-the-domain-heuristic-count-a-projects-source-files.md`
...
### `shared/decisions/260809-2004_i_should-the-latching-churn-and-cross-file-criticals-be-bounded-or-dropped.md`
...
### `shared/decisions/260809-2310_i_should-the-branch-policy-fall-the-way-the-write-classifier-fell.md`
...
### `shared/decisions/260810-0920_i_what-should-a-churn-key-be-anchored-to-and-what-happens-to-the-535-entries-already-recorded.md`
...
### `shared/decisions/260810-1544_a_should-prompt-called-bin-helpers-get-one-guarded-call-convention-and-does-the-work-tree-preference-extend-to-them.md`
...
### `shared/decisions/260810-2145_a_should-a-repeated-skill-body-snippet-become-a-bin-helper-now-that-one-fact-lives-in-four-executable-copies.md`
...
### `shared/decisions/260812-2043_i_who-writes-the-recommended-marker-on-a-backlog-entry.md`
...
### `shared/decisions/260813-0027_o_should-the-orchestrator-be-able-to-dispatch-the-shapers-portfolio-activation-mode.md`
