...
**Agent:** reconciler
**Domain:** `code`
**Session:** `shared/history/260813-2345-orchestrator-session.md`
...
**`planning/260814-0845_o_plan-curator.md` → `_c_`, Status Draft → Complete.** All five
...
**`planning/260814-0738_o_spec-curator.md` → `_c_`, Status Draft → Complete.** All seven
...
`issues/260814-0828_o_the-grounding-and-the-spec-still-call-the-growth-bound-decision-open-after-it-was-answered.md`;
...
- `decisions/260814-0738_i_how-is-the-always-on-growth-bound-armed-…` — `5c843e6` re-set exactly the
...
- `decisions/260814-0845_i_are-the-sixteen-agent-claims-corrected-or-derived-away.md` — `6ba9d77`.
...
- `decisions/260814-1332_o_what-marks-an-implemented-decision-…` — genuinely open. Filed by the
...
`shared/decisions/260801-1020_a_where-does-normative-consistency-live.md` → `_i_`. This is D1, the
...
(`circles/260801-1244-guard-bash-inspection/history/260801-2038-reconciliation.md:79`,
`circles/260801-1244-rule-provenance-header/history/260802-1413-reconciliation.md:62`). It does now.

**Two answered records were checked and deliberately left at `_a_`.**
`shared/decisions/260810-2145_a_should-a-repeated-skill-body-snippet-become-a-bin-helper-…` states in
...
capture is still open. `shared/decisions/260811-1522_a_should-the-readme-hooks-lib-table-pin-its-prose-…`
...
- `260814-0813_o` — the Circle record's title and `## Dependencies` still name the conventions file
  as the validation case. Unchanged, and now sharper: the validation case has since actually run
  against the decision corpus.
- `260814-0828_o` — half-resolved, as above.
- `260814-1200_o` — the instance is cleared (`~/.fusion` holds 17 prompts including `curator.md`,
...
**`shared/issues/260810-0410_o`** was annotated: ledger entry L24 added all three Plane files to the
layout tree at `1a36fe4`, so question 1 half-landed, while questions 2 and 3 are untouched. Its
successor record `260814-1419_o_three-plane-files-entered-…` says "do not refile it, this record is
the new state" — the two describe one job and should close together.

**Newly filed by this pass (3):**

1. `260814-1450_o_renaming-the-spec-and-plan-to-closed-broke-twenty-citations-that-spelled-the-open-marker.md`
...
2. `260814-1450_o_the-turn-3-review-states-five-findings-and-a-three-two-split-while-carrying-six-and-a-four-two-split.md`
   — a hand-written count wrong inside the review whose own cross-cutting observation is that
   hand-written counts need a gate.
3. `260814-1450_o_the-turn-3-bookkeeping-says-no-review-ran-in-the-commit-that-landed-the-review.md`
   — four bookkeeping surfaces, including a Turn-log citation to a stamp (`260814-1210`) that has
   never existed on disk.

**`shared/issues/260814-1430_o`, the net-negative breaker, was checked at the dispatcher's request
...
`reviews/260814-1419-coderev-curator-turn-3.md`: all six of its findings were re-checked against the
...
1. `circles/260801-1244-curator/decisions/260814-1332_o_what-marks-an-implemented-decision-whose-implementation-was-later-deleted.md`
...
2. `shared/decisions/260813-0027_o_should-the-orchestrator-be-able-to-dispatch-the-shapers-portfolio-activation-mode.md`
...
3. `shared/decisions/260812-1232_o_does-the-escalation-counter-survive-a-block-source-that-ships-inert.md`
4. `shared/decisions/260812-1232_o_does-the-write-guards-fusion-repo-stand-down-survive-the-loss-of-its-subject.md`
...
5. `shared/decisions/260812-0254_o_how-does-a-consuming-project-file-a-defect-against-the-plugin.md`

**LOW — open by choice or unscoped**

6. `shared/decisions/260811-2009_o_is-the-hooks-suite-meant-to-be-run-concurrently-with-itself-and-if-not-who-serialises-it.md`
7. `shared/decisions/260812-0254_o_should-a-cited-artifact-path-be-absolute-so-an-editor-can-open-it.md`
8. `shared/decisions/260812-0254_o_should-the-investigator-get-case-folders-with-a-status-per-case.md`
...
1. **Three live pointers broken by the two renames.** `agentstate.yaml`
   ...
   (`**Active spec/plan:**`) still spell `260814-0845_o_plan-curator.md`. Phase 4 already writes
   both surfaces.
2. **The Circle record's title, `## Dependencies` and `## Grounding snapshot`** carry the three
   contradictions named in `260814-0813_o`, `260814-0828_o` and the `260814-1210` stamp error.

`shared/planning/260801-1122_o_spec-normative-consolidation.md`, the earlier four-Circle spec, is a
