...
---
**Reconciliation 260824-1637** (reconciler, domain `code`, Phase 3 of session `260824-0539`, HEAD `cf7a5b0`; log `circles/260824-0530-record-attribution-and-circle-claim/history/260824-1637-reconciliation.md`) — **STAYS `_o_`.** Fifth instance, and this one is one link further along than the record's own case. Measured over Circle `260824-0530-record-attribution-and-circle-claim`: `commits=18 reviews=2 unusable=0 uncovered=1 verdict=uncovered`, and the uncovered commit is `cf7a5b0`, the **review file's own commit** rather than the commit discharging its findings. `git show --stat cf7a5b0` lists four files, all under `fusion-workbench/`, so nothing shipped is unopened. The record's option 3 — re-declaring a `**Reviewed-range:**` after the fix lands — would not reach this shape at all, because no fix follows: the review is last and covers itself only by covering its own commit, which it cannot. The record's option 1 is the one in force, per `shared/decisions/260815-2109_*_may-a-circle-close-over-an-uncovered-review-range-and-who-decides.md`.

Also seen: 260824-1637 by reconciler — the last review pass of a Circle is itself the uncovered commit, and the closing plan quoted `uncovered=0` as a met release precondition; filed as `circles/260824-0530-record-attribution-and-circle-claim/issues/260824-1637_*_the-circles-release-precondition-is-written-against-a-measurement-that-cannot-read-zero-at-closure.md`.

---
Resolved: referred (decision) — option 1 of the decision, closure tolerates a bounded uncovered tail, governs the cadence; shared/decisions/260815-2109_*_may-a-circle-close-over-an-uncovered-review-range-and-who-decides.md
