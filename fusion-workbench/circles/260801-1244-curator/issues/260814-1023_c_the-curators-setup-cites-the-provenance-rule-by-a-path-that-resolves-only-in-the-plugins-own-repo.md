...
---
Resolved: Setup step 5 in `agents/curator.md` now reads `$FUSION_PLUGIN_ROOT/rules/rule-file-provenance.md`, following the precedent at `agents/investigator.md:17`. The step also says why the prefix is load-bearing (a bare `rules/...` resolves against the consuming project's own rule directory, which is a surface the curator edits and never holds this file) and names the residual the record calls out: the variable points at the installed copy and is pinned for the session, so inside this repository it may read an install older than the work tree. The milder instance at `## Preserve list` was left as filed — it degrades gracefully because the same sentence inlines the five categories, so a curator that cannot open the file still holds the list. The two rule files that assert the citation is how the file reaches the curator (`rules/fusion-workbench-conventions.md` `## Rule-file provenance`, `rules/rule-file-provenance.md`) stay true under the new spelling and were not edited, which also leaves the golden fixture's byte pins untouched. `cd hooks && npm test` exit 0, so the reference-resolution lint accepts the `$FUSION_PLUGIN_ROOT` form as `ROOT_VARS` predicted.
