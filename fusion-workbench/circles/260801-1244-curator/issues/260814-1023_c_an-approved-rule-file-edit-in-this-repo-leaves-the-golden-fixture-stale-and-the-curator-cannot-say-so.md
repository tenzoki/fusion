...
---
Resolved: `agents/curator.md` `## Reporting work you may not do` gains a fourth bullet for exactly this case: where an applied edit moves the byte size, line count or content of a file that a test outside the curator's remit pins, the run report names the affected test, names the regeneration command, and marks it coder work; the curator does not run it. The bullet says in its own words why it is not either of the two cases above it — the edit was in remit and was approved, and only its consequence is somebody else's — and states the scope the record asked for precisely: it bites in the plugin's own repository, where `hooks/lib/__tests__/fixtures/rules-emission.golden` pins the rule files' sizes, while in a consuming project `./rules/` is that project's own directory and no fixture pins it. The failure stays loud (the suite goes red on the next run); what the report adds is the owner.
