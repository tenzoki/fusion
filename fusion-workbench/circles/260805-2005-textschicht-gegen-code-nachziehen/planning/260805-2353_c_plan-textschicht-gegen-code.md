...
**Date:** 2026-08-05
**Status:** Complete
**Spec:** `260805-2005-textschicht-gegen-code-nachziehen` — the Circle record's `## Directive` and `## Grounding snapshot` are the spec; no separate spec file exists.
...
1. [DONE] **File decision record D1: how a workbench record is cited** — filed as `260806-0015_*_zitierform-fuer-workbench-records.md`
   - Executor: coder
   ...
   - Falsifier: if `rules/rule-file-provenance.md`'s rationale already covers marker transitions and the 16 references broke for another reason, the decision is mis-framed — re-read the 8+8 citation findings (`260805-1839_*_ausgelieferte-texte-zitieren-acht-workbench-records-…` and `…acht-zitate-tragen-verfallene-decision-marker-…`) before filing.

2. [DONE] **File decision record D2: who owns the `_a_→_t_` transition, and the lock rule** — filed as `260806-0015_*_wem-gehoert-die-circle-aktivierung.md`
   - Executor: coder
...
3. [DONE] **File decision record D3: the self-detect staleness follow-up** — filed as `260806-0015_*_veraltete-regeln-im-eigenen-repo-melden-oder-umgehen.md`
   - Executor: coder
...
4. [DONE] **Fix `shared_of` losing all shared stores under zsh — the silent-data-loss item, first**
   ...
   - Falsifier: if zsh's emulation mode in the Bash tool actually word-splits (`sh` emulation), the measured `zsh → []` would not reproduce — reproduce the measurement first; the source review already did (`260805-1904_*_shared-of-im-archive-skill-…`).
...
9. [DONE] **Realise D2: one coherent activation story across the four files, plus the lock rule** *(option (b) + lock (i): conventions writer-set sentence rewritten, shaper mode honestly user-invoked-only, commit+cleanup skills lock-wrapped, lock rule's acquirer list and tags extended; `skills/next/SKILL.md` and `agents/orchestrator.md` needed no change — their current state is the (b) target state)*
   ...
   - Changes: exactly what the answered decision says, no more; the invariant sentence in the conventions must end up true against every writer that exists. Closes findings `260805-1839_*_die-circle-aktivierung-gehoert-drei-parteien-…`, `…der-shaper-portfolio-activation-modus-…`, `…die-lock-regel-sagt-always-…`, `260805-1840_*_konventionen-active-circle-nothing-else-touches-it.md`.
...
10. [DONE] **Batch A: mechanical corrections in `rules/` (all 15 files as needed)** *(eight rule files corrected; `agent-setup.md` needed no text change — step 5's code fix made its sentence true; the `konventionen-active-circle` finding was already resolved by step 9; golden deliberately regenerated, floors unchanged)*
    - Executor: coder
    - Files: `rules/agent-setup.md` (skip-silently sentence stays, now true after step 5), `rules/protected-path-discipline.md` (empty-list claim, `260805-1840_*_ppd-leere-liste-…`), `rules/fusion-workbench-conventions.md` (0/1/2-shape vs `fusion-rules` exit 3, `…konventionen-012-shape-…`; "auto-loaded by the plugin system", `260805-1841_*_konventionen-auto-loaded-…`), `rules/decision-record-examples.md` (heading `_a_→_s_` over an `_i_→_s_` body; `_i_→_s_` normed only in examples — align with the conventions' vocabulary), `rules/circle-records.md` (four of six citing skills do not cite; binding-decision path), `rules/workbench-path-resolution.md` ("cited directly by fusion-paths"; pre-v4 pointer-rejection mechanism; stale log-activity example), `rules/workbench-stash-and-lock.md` ("nine fields" vs ten, three dead record citations), pre-v4 example paths in the two always-on files (`260805-1840_*_beispielpfade-…`). Citation rewrites follow D1's decided form.
...
12. [DONE] **Batch C: skill bodies, bin headers, `install.sh`, templates** *(the `plane.config.yaml` item was executed by coder rather than ontocoder — the two corrections are comment lines only, no data value touched; `install.sh`'s LICENSE entry deliberately left for the user decision step 17 records; `rules/context-manifest.md`'s "stops" nuance left to the rules batch, whose files this step must not touch)*
    ...
    - Files (coder): `skills/*/SKILL.md` mechanical findings (`260805-1904_*_sechs-kleinbefunde-…` — text-level items only, including `/fusion:commit`'s missing `AskUserQuestion` in `allowed-tools`; `…help-verweist-…` — help pointing at a `CLAUDE.md` the installer never ships; `…log-activity-scannt-…` — backup-folder scan and setup's untenable store-equality claim; `260805-1842_*_circle-pop-nennt-manifest-feld-…`; `260805-1839_*_cleanup-hardcodet-einen-modellnamen-…`), bin header comments (`260805-1842_*_fusion-paths-header-…`, `…commit-lock-header-…`, `…header-kleinbefunde-…`, `260805-1839_*_kommentar-drift-…` — the bracket-notation relics the conventions themselves outlaw), `bin/fusion-rules`/`bin/fusion-paths` comment drift, `install.sh` header, `…context-manifest-stops-…` nuance. Files (ontocoder): `templates/plane.config.yaml` header (`260805-1842_*_plane-config-header-…`).
...
14. [DONE] **Lint 5: reference-resolution lint over the shipped text** *(landed as `hooks/lib/__tests__/reference-resolution-lint.test.ts`, 22 tests; falsifier run against the pre-fix tree caught all 17 remaining defects; seven mechanical HEAD leftovers fixed in the same step — playmaker's five dead 260511-1031 citations, four stale markers in README-hooks, one in protected-path-internals and one in a bin/fusion-plane comment, setup's pre-v4 issue path, next's wrong-file `## Marker globs` anchor, README-agents' six bare project-local `rules/` spellings, and a `skills/scripts` prose false positive in bin/fusion-workbench-root)*
    - Executor: coder
...
15. [DONE] **Lint 6: derivable enumerations checked against the tree** *(landed as `hooks/lib/__tests__/derivable-enumerations-lint.test.ts`, 17 tests: skill roster both directions, five digit agent-count claims, always-on list + order, conditional emission sets derived from the IS_* case arms incl. the own-repo-only condition, hooks/lib table, stash-manifest field count vs schema keys, DEFINITION_SITES echo in CLAUDE.md; live mutation check with a scratch skill dir verified; word-number counts in code comments documented as out of scope)*
    - Executor: coder
...
17. [DONE] **Bookkeeping: close the resolved findings where they live** *(51 records closed with `Resolved:` footer + commit citation, each spot-checked at HEAD before closing. Final count over the 66-record corpus: 51 closed here + 3 closed before this step (`…der-circle-datensatz-…`, `…fusion-guard-template-…` from the prior Circle's work, `…im-eigenen-repo-laden-…` in step 13) + 6 explicitly-left-open per this step's list (`alle-17-guard-blocks`, `coder-beschreibung-rust`, `domaenenheuristik`, `guard-event-log`, `tracker-cwd`, `install-sh-license`; the other three named names — `beim-filen`, `plane-testfixture`, `2323-emissionsmessung` — are outside the 18*/19* corpus) + 6 residuals the plan's partition did not anticipate: 3 half-resolved with progress notes appended (`acht-zitate` — hooks/lib TS sources outside the reference lint's file set still stale; `wpr-und-migrate` — migrate:16 half missed by Batch C; `fusion-paths-header` — migrate:14 two-places attribution remains) and 3 unresolved, left untouched (`ein-holderloses-lock-verzeichnis` — `is_stale_lock` still returns not-stale without a holder file; `playmakers-setup` — bullet 1 still cites "State Markers — circles" against the conventions file; `context-manifest-stops` — the stdout-partial-set nuance was deferred from Batch C to Batch A and executed by neither). 51+3+6+6 = 66.)*
    - Executor: coder
    - Files: `circles/260801-1244-guard-rules-write/issues/260805-18*`/`19*` records resolved by steps 4–16
    - Changes: for each record whose fix landed, append the `Resolved:` footer citing this Circle's commit hash(es) and rename `_o_` → `_c_` in place. Records are never moved. Explicitly left open: `…alle-17-guard-blocks-…` (routes to the reachability Circle's Grounding per this Circle's `## Dependencies`), `…der-circle-datensatz-dieses-circles-…` (belongs to the neighbour Circle's closure), `…die-domaenenheuristik-…`, `…die-coder-beschreibung-nennt-rust-…`, `…das-guard-event-log-waechst-…`, `…der-tracker-steht-im-plugin-repo-…` (pairs with guard issue `260804-2100`), `…install-sh-will-eine-license-…` (user decision), `…beim-filen-prueft-niemand-…`, `…der-plane-testfixture-…`, `260805-2323_*_die-emissionsmessung-…` (external machine). The marker renames are the loop-form `mv` the guard is known to fail-closed on when variables are used — write the paths literally.
...
**260806-1057-reconciliation.md — final reconciliation before Circle closure (reconciler).** All 17 steps verified `[DONE]` against ground truth; Status Complete and the `_c_` marker are consistent.
...
- One genuine residual stays open: `260806-0022_*_setup-klammer-probe-und-migrate-reformat-decken-verschiedene-baeume.md` (scope mismatch between setup's whole-tree probe and migrate's `shared/` + `circles/` depth-2 reformat pass — re-verified at HEAD against `skills/setup/SKILL.md:43` and `skills/migrate/SKILL.md:85`).
