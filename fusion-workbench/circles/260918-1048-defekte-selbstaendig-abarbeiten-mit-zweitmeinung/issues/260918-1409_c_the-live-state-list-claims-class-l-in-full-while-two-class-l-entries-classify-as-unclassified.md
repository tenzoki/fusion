...
---
Resolved: two rows added to `LIVE_STATE` (`.checkout-id`, `.cadence-anchors`); the comment above it now states the decided membership (exactly class L, R2 and R3 of `rules/workbench-tracking.md`, with `LIVE_PREFIXES`) and cites the decision by basename, `260920-2228_*_does-the-staging-classifiers-live-state-list-keep-rows-for-retired-surfaces.md`; a case in `staging-drift.test.ts` reads the class L row and probes every token through `classify()`, so the property is checked rather than claimed (on a scratch copy with the `.checkout-id` row deleted it names that token); the stale `ROOT_RECORDS` sentence in the same file, which still placed `portfolio.md` in class L, corrected beyond this record's acceptance; the test header's `## The properties under test` block cut to fund the case; `surface-growth.golden` regenerated; `hooks/dist/lib/staging-drift.js` rebuilt; the reverse inclusion (list ⊆ rule) filed as `260921-0807_*_the-class-l-case-pins-rule-into-list-and-nothing-pins-list-into-rule.md`; the conventions' same-commit sentence left unchanged, the case being the enforcement. Concept accepted by a consultant read; fixed in the commit that carries this line.
