...
- Both decisions the record scoped into this Circle were answered by the user at their own gates, and neither was answered by an agent; each carries whichever terminal marker its answer earned — `_a_`, `_d_`, or `_i_` where step 14's `_a_`→`_i_` transition realised a user answer that required code.
