...
11. [DONE] **The shipped text says what the guard now is**
    ...
    - **What the step's Files list did not carry, and where each landed.** Six defect records amend it and all six are closed here. `bin/fusion-turn-budget`'s header (`260816-2124`) now names `fusion.json` and states the two-layer merge instead of the plugin layer. `hooks/lib/guard-state-file.ts` (`260816-2125`) drops `escalation.ts` from its live-caller sentence, reads its `## What escalation.ts added` section as history, and re-justifies the optional `root` on what is true now — every caller in the tree passes one, and the parameter is kept the way `isFusionPluginRoot` is kept, explicitly without a current subject. `docs/upgrading-to-v9.md` (`260816-2126`) keeps its v9 history and corrects the two present-tense bullets in `## What needs no action`, pointing forward to the v10 note in prose rather than by path, because that file does not exist until step 12 and a citation of it would be the very dangling reference this step is closing. `skills/setup/SKILL.md` (`260816-2127`) attributes the legacy flag to no mechanism at all rather than to one of two, which is the formulation that cannot go stale again. `hooks-wiring.test.ts` (`260816-2315`) replaces the protected-path fingerprint with the reason that holds — the diagnostic loop, and that since `fab8a4b` the diagnostic *is* the v10 migration, so this matcher is how it reaches a project. `agents/orchestrator.md:122` and `bin/monitor:188` (`260816-2321`) are both corrected in full rather than at the one clause the line-scoped text named.
...
13. [DONE] **The decision records are annotated**
    ...
    - Executed 2026-08-17, under the user's approval of `circles/260816-1741-guard-becomes-observation-only/issues/260816-2316_*_step-13s-retired-line-list-is-wrong-in-three-of-its-four-entries-and-its-marker-scope-misses-a-fifth-record.md`, which **overrides this step's `Retired:` list**. Eleven records edited, ten renamed: six decisions `_a_` → `_i_`, one `Retired:` line with no rename, one decision `_a_` → `_s_`, and three issues `_o_` → `_c_`. `cd hooks && npm test` exits 1 at the unchanged red set of 2 files / 3 cases.
    - **The step's `Retired:` list was wrong in three of its four entries and its safeguard would have confirmed the worst one.** Only `260804-1631_*_may-a-project-file-set-guard-enabled-…` took a line. `260803-1419_*_…` and `260802-1912_*_…` already carry `Retired: 60c9cd8` and got no second. `260804-1630_*_what-does-a-project-guard-object-inherit-…` got none: its *setting* was deleted, its *answer* is live code, and `hooks/lib/config.ts` cites it in the present tense at `:46-49`, `:70-73` and `:363-367` with the per-leaf walk surviving at `:479-486` — all four read at HEAD before deciding. The re-derivation instruction was **not** run: a grep for deleted identifiers returns `260804-1630` first, because its `## Question` names five of them, so the safeguard points straight at the entry that must not carry a line.
