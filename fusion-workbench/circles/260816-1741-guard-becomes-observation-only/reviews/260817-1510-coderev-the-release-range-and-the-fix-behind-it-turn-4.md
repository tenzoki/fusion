...
**Plan under review:** `circles/260816-1741-guard-becomes-observation-only/planning/260816-1915_c_the-compliance-guard-becomes-observation-only.md`, steps 10 to 16, plus the post-release fix
...
the commit names and the reason it filed `260817-1032`.
...
`260816-2124`, whose fix landed at step 11 and did not reach these lines. Filed as `260817-1507`.
...
decision `260816-1916`, whose chosen option 1 carries the recorded Con *"the advisory reaches it
...
so behaviour is right and the missing example costs nothing today. Filed as `260817-1509`.
...
construction. That is the third distinct instance of the same limit in this Circle — `260816-2321`
...
move, and filed the gap (`260817-1032`). `c65e1cf` refused to add a `Retired:` line the plan
...
1. `260817-1506` — the four channel-reach corrections. The patch carries a fix that makes
...
2. `260817-1505` — the curator pair. Two clause deletions, and it is the Directive's own defect
   class in shipped agent text.
3. `260817-1507` — the helper header and the module docstring. Three surfaces, one contract.

**Cleanup, any later Turn:**

4. `260817-1508` — the archive skill's event-log sentence.
5. `260817-1509` — the two assertions in `turn-budget-lint.test.ts`. Note the hook-test surface has
...
|---|---|---|---|
| Theme 2 | `260817-1506` | `_c_` | Closed in `dcb0784`, shipped in v10.0.1. `docs/upgrading-to-v10.md:11-13` and `:74-77` and `README-hooks.md:315` now say the diagnostic has two channels; `hooks/lib/config.ts:103-114` keeps its claim with the compression narrowed. |
| Theme 1 (a) | `260817-1505` | `_o_` | Both sentences stand verbatim at `agents/curator.md:212` and `skills/curate/SKILL.md:110`. |
| Theme 1 (b) | `260817-1507` | `_o_` | `bin/fusion-turn-budget:14` still scopes its stderr to what the loader "had to drop". |
| Theme 1 (c) | `260817-1508` | `_o_` | `skills/archive/SKILL.md:130` and `:132` unchanged; neither live event type appears in the file. |
| Theme 3 | `260817-1509` | `_o_` | Suite still 35 files and 653 tests, the same count as before `01932d6`, so no test was added with the fix. |

**This review's own release judgement held.** It said "Nothing blocks the v10.0.1 patch" and that
one finding had to ride inside it; `260817-1506` did, as `dcb0784`, and the tag `v10.0.1` at
...
rest — are among the four the user chose to leave open. One of them, `260817-1505`, is the single
