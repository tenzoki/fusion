...
**Severity:** Medium. The clause states its rule correctly and an agent will act on it. The defect is that the file's most-read new sentence is an exhibit of the register the file forbids, which is the mechanism the Circle's own Directive rests on (`circles/260821-1042-reply-bounded-whole-question-answered/_t_circle.md:22-24`: a text that teaches by imitation).
...
**Fault 1, the fragment.** "Asked where the acceptance criteria are: the path and the section names, plus one line for each of the two defects you filed." There is no subject and no finite verb. It is an elliptical worked contrast, compressed from `shared/issues/260812-0253_*_agents-answer-a-question-the-user-did-not-ask-and-the-length-caps-do-not-hold.md`. The file's own gate, `rules/user-facing-output.md:133`:
...
The chat profiles carry the same prohibition as the verbless-fragment habit, and `shared/analyses/260816-0740-rhetorical-register-of-agent-output.md` inventories it as one of the thirteen figures this Circle's Directive names.
...
**What a fix has to respect.** The rule file's byte budget is net zero or less against HEAD `e764637` (`circles/260821-1042-reply-bounded-whole-question-answered/decisions/260821-1108_*_what-may-the-circles-own-new-clauses-cost.md`), and the file currently stands 84 bytes under. A rewrite has 84 bytes of room and no more. The executor already measured two wordings against each other for this clause and chose the shorter (`circles/260821-1042-reply-bounded-whole-question-answered/history/260821-2120-coder-the-reply-answers-the-question-that-was-put.md:37-39`), so the compression is deliberate and the fix is a third wording rather than an expansion. A sketch of one, at +9 bytes:
...
**Cross-references:** `circles/260821-1042-reply-bounded-whole-question-answered/planning/260821-1805_*_plan-reply-bounded-whole-question-answered.md:123` (step 3, which asked for "the shortest form that states the rule and its one worked contrast"); `circles/260821-1042-reply-bounded-whole-question-answered/decisions/260821-1108_*_what-may-the-circles-own-new-clauses-cost.md`.
