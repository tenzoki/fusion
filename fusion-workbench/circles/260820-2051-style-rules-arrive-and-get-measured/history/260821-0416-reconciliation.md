...
**Circle:** `circles/260820-2051-style-rules-arrive-and-get-measured` (`_t_`, the only active one)
...
3. **Step 16's `Closes:` over-reaches on one of its two records.** The record asks for the progress
   note on `shared/issues/260816-0740_*_…em-dash-ceiling…md` to be corrected. The correction was
   ...
   `shared/issues/260816-0740_*_…:82`. Record stays `_o_`.
...
5. **`## Where this Circle stops` clause 5 is not met.** The clause requires
   `shared/decisions/260816-0740_*_…` to gain "the protocol's path". `grep -c '260820-2354'` on that
   ...
   `260821-0413_o_the-decision-record-the-measurement-reports-on-does-not-cite-the-protocol-that-defines-it.md`.
...
7. **The final-measurement note means two different sets by "always-on".** Section 4 reports +2 138
   ...
   `260821-0413_o_the-final-measurement-note-means-two-different-sets-by-always-on-and-its-headline-delta-is-the-smaller-one.md`.
8. **The Turn 1 review's head-room figure is stale by seven lines.** `260821-0144` records 43 lines
...
10. **The Circle record's `## Turn log` section is empty.** Both Turns are written up in
    `shared/history/260820-2103-orchestrator-session.md` and neither reached the record. Not
    repaired here, same reason.
11. **The Circle record's `## Dependencies` calls `circles/260819-1645-four-constraints-on-deep-change`
...
- `shared/issues/260816-1330_*_the-foreclosure-clause-does-not-say-whether-it-costs-a-line-per-option-…md`
...
- `shared/issues/260816-1345_*_the-register-defects-corpus-table-is-labelled-always-on-…md` — the
  derivation landed on all four live carriers in `b8b8f42`, appended beneath each mislabelled claim
  rather than replacing it.
- `circles/260820-2051-…/issues/260820-2249_*_the-always-on-corpus-is-said-to-have-grown-…md` — both
...
- **`circles/260801-1244-curator/issues/260814-1332_*_the-voice-profile-fallback-…md`** (plan step 4).
  ...
  stands.
- **`shared/issues/260807-2154_*_the-writing-profile-carries-no-handle-…md`** (plan step 6, item 2).
  ...
  260821-0100 note left open is now closed too, by `ca83e79`, which the note could not have seen.
- **`shared/issues/260814-1419_*_the-shipped-chat-voice-profiles-changed-…md`** (plan step 3). The
...
- **Added here: `shared/issues/260816-1330_*_the-repunctuations-evidence-paragraph-…md`** (plan step
...
`shared/issues/260816-0740_*_…em-dash-ceiling…md` (the emitted set is clean, `CLAUDE.md` at 14.2 per
1000 is 94 per cent of what remains and was excluded by an unconfirmed decision);
`shared/issues/260816-1330_*_the-override-record-…md` (the annotation supersedes the premise instead
...
drifted again); and `shared/issues/260812-0253_*_agents-answer-a-question-…md`, which is **not**
...
- **`260821-0144_o_the-authoritative-prose-metric-has-no-test-…md`** — self-declares it: "a
...
- **`260821-0242_o_step-12s-two-acceptance-criteria-cannot-both-hold-…md`** — its content is a
...
`260821-0414_o_does-a-corpus-repair-circle-carry-a-budget-for-what-its-own-new-clauses-may-spend.md`.
...
`260821-0302` and part 1 of `260821-0148`, both write into `skills/setup/SKILL.md`. Either turns
...
(`shared/decisions/260815-2322_*_…`, and `shared/decisions/260815-2109_*_may-a-circle-close-over-an-uncovered-review-range-and-who-decides.md`
...
`260820-2324_c_plan-style-rules-arrive-and-get-measured.md`. Its `## Where this Circle stops` clauses
