...
Resolved: `agents/curator.md` `### Ledger entry schema` fixes the form for an edge entry's revert path as `git checkout -- "$(git ls-files ':(top)*<basename>')"`. One basename, so the citation gate stays green; no environment variable, so no shell has to have been set up by an agent; and `:(top)` resolves the pathspec against the repository root, so it runs from any directory in the work tree. Verified by running it with `env -u WORKBENCH -u FUSION_PLUGIN_ROOT` from `hooks/` against this item's own record: exit 0, the record restored. The untracked case is answered rather than assumed: where `git ls-files` returns nothing for that basename at survey time the workbench is not tracked, and the line reads "none — the file is not under version control", which is what `### The revert path` already requires of every entry. `bin/fusion-citation-check` exits 0 with none of the edited files named.
