...
**History-log step — the reviewers diverge.** 12 of 15 prompts name `$OUT_HISTORY` as a Process step. The three without: **coderev, ontorev, conceptrev** — the reviewers write a review file but no session-history log. This is already recorded as a prompt-gap in `260717-0031_*_p8-lint-gate-scope-open-questions-from-conversions.md` (cited in `fusion-paths:110-115`). It is a genuine inconsistency — every other agent logs a history entry — but it may be intentional (reviewers' output *is* their record). Circle D must decide: unify (add the step) or document the exception.
