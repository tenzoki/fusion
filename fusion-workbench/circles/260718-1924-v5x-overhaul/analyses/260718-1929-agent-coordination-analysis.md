...
**History-log step — the reviewers diverge.** 12 of 15 prompts name `$OUT_HISTORY` as a Process step. The three without: **coderev, ontorev, conceptrev** — the reviewers write a review file but no session-history log. This is already recorded as a prompt-gap in `issues/260717-0031[o]-p8-lint-gate-scope-open-questions-from-conversions.md` (cited in `fusion-paths:110-115`). It is a genuine inconsistency — every other agent logs a history entry — but it may be intentional (reviewers' output *is* their record). Circle D must decide: unify (add the step) or document the exception.
...
| **F5** | History-log step absent from the three reviewers (coderev, ontorev, conceptrev) while 12/15 agents have it. Already tracked in issue 260717-0031. | Medium | **re-wiring-for-D** (decide: unify or document the exception; do not leave it undecided) |
...
- `planning/260718-1001_o_master-plan-fusion-v5x-overhaul.md` §Circle A/B/C/D, resolved decisions.
