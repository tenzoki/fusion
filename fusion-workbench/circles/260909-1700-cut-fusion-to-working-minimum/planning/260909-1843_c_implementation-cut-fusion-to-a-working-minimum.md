...
18. **D1: run the migration**
    - **[Cancelled 260911 by the container ruling, not deferred.** `260910-2133_*_does-a-unit-of-work-keep-its-own-container-for-the-artifacts-it-produces.md` restores the per-work-item directory, so the move of `circles/**` into the shared stores that this step performs is the opposite of what the project now wants. What replaced it is step S10 of `260910-2145_*_restore-the-per-work-item-container.md`, an in-place conversion of the two live records that a forward commit reverses. The irreversibility that made this step the session's one-way change is gone with the flattening.]**
...
20. [DONE] **D3: re-baseline the four surface bounds, and only then retire the old one**
    - **[Re-baselining half superseded 260911 by the user's ruling.** `260822-1154_*_does-a-cut-only-circle-re-baseline-the-surfaces-it-cuts.md` was answered option 1 and implemented in `b90590fc`: a cut-only piece of work never re-baselines, whatever it cut, so the floor stays where it is and no size of cut absolves growth nobody removed. The four baselines therefore do not move and no entry naming this cut is written. The ruling's own measurement is why the literal reading was refused: three surfaces would have gained margin nobody earned while `agents/`, the surface that cut furthest, would have lost its shrink.]**
    - **Surviving half landed 260911 as `a5bb2a63`.** The universal-core growth bound is retired and nothing was re-baselined. The argument is a measurement rather than the plan's ordering claim: a core byte is charged to all eleven dispatch paths at zero head-room, and at `9ff123c5` the core held 2 625 bytes of margin against the tightest path's 170, so the per-path bound already refused every core addition this one refused. The defect that motivated the per-path bound, `260909-1346_*_the-rule-growth-bound-covers-the-core-while-the-hottest-path-grew-29-percent-back.md`, is closed in the same commit. One defect the change does not fix is filed: `260911-1120_*_claude-mds-growth-bound-bullet-names-four-failing-surfaces-and-three-remain.md`.
    - Executor: `coder`
...
21. [DONE] **D4: release**
    - **Landed 260911 as `v11.0.0`, tagged at `7662f881` and pushed.** The four version surfaces agree: the manifest, `README.md`, `install.sh` and the marketplace entry, whose description was brought byte-identical to the manifest's after both had gone on advertising a per-dispatch stopping time retired on 2026-09-10. `claude plugin validate .` passes with the standing `CLAUDE.md` warning, and all eleven roles answer a headless smoke dispatch. `CLAUDE.md` was brought to the new shape through the curator's gated pass (`a4c57133`, 23 candidates approved whole, net -307 bytes) rather than by hand, which is what this step required. `docs/upgrading-to-v11.md` states what a consuming project must do. The review-coverage read the release process demands was run and stated: `v10.26.0..HEAD` was 67 commits with all 67 uncovered, and the closing pass tiles it to `uncovered=0`. No marketplace cache clone exists at `~/.claude/plugins/marketplaces/`, so `/plugin install` cannot see the release locally; `install.sh` reads the GitHub tarball and is unaffected, which is the case `CLAUDE.md` already records from v6.0.0.]**
    - Executor: `coder`
