...
---
Resolved: the one live instance is corrected. The acceptance offered two routes, and the first one landed in full: `rules/fusion-workbench-conventions.md` `## Backlog entries — work items` now states that an entry asserts one relation and one only, the ordering prerequisite, with every other citation going to `**Cross-references:**`; the migration drops what it cannot express and names each drop; and this record's third requirement, that the live instance be corrected in the same change, is met at `260918-0706`. The general defect the record names is closed by the field definition rather than by a second field. Authorised by `260911-1747_*_may-a-done-work-items-head-field-be-edited-at-all-and-under-what-bound.md`.
