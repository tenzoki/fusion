...
**Head-room, measured rather than quoted.** `agents/*.md` stands at 414 534 bytes against a budget of 417 843, leaving **3 309 bytes**, of which `orchestrator.md` has already spent 10 444. `skills/*/SKILL.md` leaves **9 716 bytes**. The hook test surface stands at 18 498 lines against 20 375, leaving **1 877 lines**. `rules/circle-records.md` is a conditional emission (`bin/fusion-rules:432`, indented), so it sits outside the always-on rule bound, and `rules/` is not one of the three surfaces `surface-growth-bound.test.ts` measures. Writing there costs no budget.
...
| Declaration emit could in principle depend on the ambient type packages, so `@types/node` would become a second unpinned input. `inference:` verified only for the sources as they stand: no file in `hooks/dist` emits an `import(` type or references `node:`. | Left unpinned deliberately, with the measurement recorded here. Pinning four devDependencies on the strength of one that reaches the emit would be pinning by superstition, and the version assertion's failure message is where a future divergence will surface. |
