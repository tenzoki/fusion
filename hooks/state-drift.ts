...
function main(argv: string[]): number {
  ...
  if (root === null) {
    process.stderr.write(
      "fusion-state-drift: no fusion workbench above the working directory — nothing to check.\n",
    );
    return 2;
  }
  ...
  out.push(
    ...
    `drift=${report.drifted.length}`,
    `verdict=${report.drifted.length > 0 ? "drift" : "clean"}`,
  );
  for (const r of report.rows) out.push(renderRow(r));
  process.stdout.write(out.join("\n") + "\n");
  return 0;
}
