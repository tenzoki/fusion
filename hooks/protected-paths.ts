...
/** Why a half of the guard is not enforcing, or `null` when it is. */
type StandDown = "guard-disabled" | "fusion-plugin-repo" | null;

/** The one-word answer for a machine, across both halves. */
function enforcedToken(write: StandDown, measurement: StandDown): string {
  if (write === null && measurement === null) return "both";
  if (write === null) return "write-tools";
  if (measurement === null) return "measurement";
  return "none";
}

/** The one reason either half stands down, named for the directory it asked about. */
function pluginRepoBecause(dir: string): string {
 ...
 */
function standDownPhrase(inPluginRepo: boolean, half: string, dir: string): string {
  if (!inPluginRepo) return `${half} is active`;
...
function sourcePhrase(source: string, declared: number): string {
  if (source === "project") {
...
function main(argv: string[]): number {
  if (argv.length > 0) {
    process.stderr.write(
      `fusion-protected-paths: unknown argument ${JSON.stringify(argv[0])}\n${USAGE}\n`,
    );
    return 1;
  }
  ...
  if (root === null) {
    process.stderr.write(
      `fusion-protected-paths: no fusion workbench found above ${cwd} — run /fusion:setup at the project root.\n`,
    );
    return 2;
  }
  ...
  const writeStandDown: StandDown = disabled
    ? "guard-disabled"
    : writeInPluginRepo
      ? "fusion-plugin-repo"
      : null;
  const measurementStandDown: StandDown = disabled
    ? "guard-disabled"
  ...
  if (config.diagnostics.length > 0) {
    ...
    parts.push(
      `${n} configuration problem${n === 1 ? " was" : "s were"} reported and ${n === 1 ? "is" : "are"} listed below; the effective list may not be the one the configuration asks for.`,
    );
  }
  ...
  const lines = [
    ...
    ...paths.map(
      (p) => `path=${p} origin=${floor.has(p) ? "floor" : config.protectedPathsSource}`,
    ),
  ];
