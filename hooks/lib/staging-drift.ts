...
export function stagingSentence(report: StagingReport): string {
  ...
  if (messages.length > 0) {
    parts.push(
      `A commit-message file is sitting in the workbench: ${messages.map((r) => r.path).join(", ")}. ` +
  ...
  parts.push(
    "This is issue 260811-0114: a queue rebuild and its history file sat in the working tree for eighteen " +
