 ...
 * a stray workbench.
 */
export function guardStatePath(fileName: string): GuardStatePaths | null {
...
export function loadGuardState<T>(
  fileName: string,
  coerce: StateCoercion<T>,
): T {
  const paths = guardStatePath(fileName);
  if (!paths) return coerce(undefined);
 ...
 */
export function saveGuardState(fileName: string, state: unknown): void {
  const paths = guardStatePath(fileName);
  if (!paths) return;
 ...
 */
export function isStateObject(value: unknown): value is Record<string, unknown> {
