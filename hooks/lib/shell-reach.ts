...
import type { ParsedSegment, SegmentJoiner } from "./shell-parse.js";
...
export const SEGMENT_REACHES: readonly SegmentReach[] = [
  "start",
  "and",
  "seq",
  "or",
 ...
 * out of it.
 */
export const CARRYING_EDGES: ReadonlySet<SegmentReach> = new Set<SegmentReach>([
  "start",
  "and",
  "cond-true",
  "transparent",
...
/**
 * The compound heads this module models, and nothing else.
 *
 * `for`, `case`, `select` and `function` are absent BY DESIGN rather than by
 * oversight. None of them is in `GRAMMAR_PREFIXES` either, so a `for` body's
 * `do` finds no head of its own to claim and falls through to its raw operator
 * — today's answer. Adding one here is a decision about the shell, not a
 * refactor.
 */
type HeadWord = "if" | "elif" | "while" | "until";
 ...
 */
export const SPAN_OPENERS: ReadonlySet<string> = new Set([
  "{",
  "if",
  "while",
  "until",
 ...
 */
const WORD_HEAD_OPENERS: ReadonlySet<string> = new Set(["for", "case", "select"]);

/** One span boundary a segment's leading words cross, in source order. */
type SpanEvent = "open" | "close";

interface OpenHead {
  word: HeadWord;
  /**
   * Set by the ONE body word that claims this head. The claim is what keeps an
   * unrecognised compound from borrowing an enclosing guarantee: in
   * `while cd X; do A; for f in *; do B; done; done` the inner `do` finds the
   * `while` head already claimed and types `B` from its raw operator.
   */
  claimed: boolean;
}
...
interface ScopeState {
  heads: OpenHead[];
  /** The edge a grammar-only segment left for the next command position. */
  pending: SegmentReach | null;
  /**
   * Segment indices at which a compound command is still open, innermost last.
   *
   * Separate from `heads` on purpose. `heads` models a CONDITION and is popped
   * by the body word that claims it; this models an EXTENT and is popped by the
   * terminator that ends it. Entangling them would make `for`/`case` — which
   * have an extent and no condition this layer models — impossible to add to
   * one without adding them to the other.
   */
  spans: number[];
}
 ...
 * `|` answers today.
 */
function rawEdge(joiner: SegmentJoiner): SegmentReach {
  switch (joiner) {
    case "start":
      return "start";
    case "&&":
...
/** The leading grammar words of a segment, and whether a command follows them. */
interface SegmentHead {
  /** Grammar words in source order, terminators included. */
  grammar: string[];
  /** True when a terminator is among them — see the hand-on rule. */
  closes: boolean;
  hasCommand: boolean;
  /**
   * The compound extents this segment's leading words open and close, in source
   * order. Read by the span stack and by nothing else — `grammar` is left
   * exactly as it was, so `readCandidate` and `hasCommand` cannot shift.
   */
 ...
 */
function readHead(words: string[]): SegmentHead {
  ...
  while (i < words.length) {
    ...
    if (GRAMMAR_PREFIXES.has(w)) {
      grammar.push(w);
      if (SPAN_OPENERS.has(w)) spanEvents.push("open");
      i++;
      continue;
    }
    break;
  ...
  if (i < words.length && WORD_HEAD_OPENERS.has(words[i])) spanEvents.push("open");
...
function topUnclaimed(heads: readonly OpenHead[]): OpenHead | undefined {
  for (let i = heads.length - 1; i >= 0; i--) {
    if (!heads[i].claimed) return heads[i];
  }
  return undefined;
}

/** Pop heads down to and including the innermost one `word` closes. */
function popTo(heads: OpenHead[], closes: (h: OpenHead) => boolean): void {
  for (let i = heads.length - 1; i >= 0; i--) {
    if (closes(heads[i])) {
      heads.length = i;
      return;
 ...
 */
function readCandidate(grammar: readonly string[], heads: OpenHead[]): SegmentReach | null {
  let candidate: SegmentReach | null = null;
  ...
  for (const word of grammar) {
    ...
    let proposed: SegmentReach | null = null;
    switch (word) {
      case "if":
      case "while":
      case "until":
    ...
    if (candidate === null) candidate = proposed;
  }
 ...
 */
function prevAtDepth(segments: readonly ParsedSegment[], i: number): number {
  const depth = segments[i].depth;
  for (let k = i - 1; k >= 0; k--) {
    if (segments[k].depth === depth) return k;
    if (segments[k].depth < depth) return -1;
  }
  return -1;
}

/** Index of the next segment at the same depth, or -1. */
function nextAtDepth(segments: readonly ParsedSegment[], i: number): number {
  const depth = segments[i].depth;
 ...
 */
function inPipeline(segments: readonly ParsedSegment[], i: number): boolean {
  if (segments[i].joiner === "|") return true;
 ...
 */
function spanIsPipelineElement(
  segments: readonly ParsedSegment[],
  start: number,
  end: number,
): boolean {
  if (segments[start].joiner === "|") return true;
...
function pipelineHead(segments: readonly ParsedSegment[], i: number): number {
  let head = i;
  for (;;) {
    if (segments[head].joiner !== "|") return head;
    const prev = prevAtDepth(segments, head);
    if (prev === -1) return head;
    head = prev;
  }
}
 ...
 */
export function annotateReach(segments: readonly ParsedSegment[]): ReachedSegment[] {
   ...
   */
  const consumed: SegmentReach[] = [];
  const out: ReachedSegment[] = [];
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // out.
    if (scopes.length > depth + 1) scopes.length = depth + 1;
    while (scopes.length < depth + 1) scopes.push(freshScope());
    ...
    let reach: SegmentReach = exitsPipeline
      ? "pipe-exit"
      : head.hasCommand
     ...
     */
    const pipelineRow = (): SegmentReach => {
    ...
    if (head.hasCommand) {
      ...
      if (member && !exitsPipeline) reach = pipelineRow();
    } else {
      ...
      let handOn =
        candidate ?? (head.closes ? null : (scope.pending ?? rawEdge(segment.joiner)));
      ...
      if (handOn !== null && member) handOn = pipelineRow();
