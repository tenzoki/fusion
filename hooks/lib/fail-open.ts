 ...
 * which puts it there.
 */
export function bestEffort(tag: string, step: () => void): string | null {
  try {
    step();
    return null;
  } catch (err) {
...
export function answer(
  ...
  verdict();
  for (const report of reports) bestEffort(tag, report);
}
