...
/**
 * The session state file, at its fixed root-relative path.
 *
 * `rules/fusion-workbench-conventions.md` `## fusion-workbench Layout` puts it
 * there precisely because the hooks, the monitor and the `bin/` helpers read it
 * there and none of them has a fallback.
 */
const STATE_REL = "fusion-workbench/agentstate.yaml";
