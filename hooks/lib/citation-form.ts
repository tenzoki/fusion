...
import { GATE_KINDS, createScanner, type CitationHit, type CitationStatus } from "./citation-scan.js";
 ...
 */
export const REPORTED_STATUSES: CitationStatus[] = ["store-prefixed", "stale-marker", "spelled-marker"];
...
export function workbenchRecordPath(root: string, abs: string): string | null {
  ...
  if (!folded.startsWith(foldCase(store) + sep)) return null;
...
function everyLine(text: string): Set<number> {
 ...
 */
function linesCovering(text: string, fragments: string[]): Set<number> {
  ...
  for (const fragment of fragments) {
    if (fragment === "") continue; // `indexOf("")` never advances
    let at = text.indexOf(fragment);
    while (at !== -1) {
      const from = lineAt(at);
      const to = lineAt(at + fragment.length - 1);
      for (let line = from; line <= to; line++) out.add(line);
...
export function writtenLines(
  ...
  if (toolName === "Write") return everyLine(text);
...
const EMPTY = (root: string, file: string, why: string): CitationFormReport => ({
  root,
  file,
  why,
  violations: [],
 ...
 */
export function measureCitationForm(
  root: string,
  file: string,
  text: string,
  lines: Set<number> | null,
): CitationFormReport {
  if (lines === null) return EMPTY(root, file, "the tool payload names no written text to scope the report to");

  const scanner = createScanner(join(root, WB));
  if (!scanner.present) return EMPTY(root, file, "no workbench to resolve citations against");

  const hits = scanner.scanCitationTokens(
    file,
 ...
 * phrased twice here.
 */
export function citationFormSentence(report: CitationFormReport): string {
 ...
 * can parse.
 */
function coerceThrottle(value: unknown): string {
...
export function lastReportedCitationForm(root: string): string {
...
export function recordReportedCitationForm(root: string, signature: string): void {
 ...
 */
export function readRecord(abs: string): string | null {
  try {
