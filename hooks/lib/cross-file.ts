...
const DEFAULT_THRESHOLDS: CrossFileThresholds = {
  pingBackWarning: 3,
  pingBackCritical: 5,
};
...
export function analyzeCrossFile(
  state: CrossFileState,
  thresholds?: Partial<CrossFileThresholds>,
): CrossFileWarning[] {
  const t: CrossFileThresholds = { ...DEFAULT_THRESHOLDS, ...thresholds };
  const warnings: CrossFileWarning[] = [];
  ...
  if (criticalFiles.length > 0) {
    warnings.push({
      ...
      files: criticalFiles,
      recommendation:
        "Agent is cycling between these files without converging. Pause and review whether the current approach can complete, or revise the plan.",
    });
  }
  ...
  if (warningFiles.length > 0) {
    warnings.push({
      ...
      files: warningFiles,
      recommendation:
        "Watch these files for continued revisits. May indicate paired correction loops (impl ↔ test) or scope creep.",
    });
  }
