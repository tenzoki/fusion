 ...
 */
function recordTail(chars: string): { cls: string; stop: string } {
...
const CIRCLE_RE = new RegExp(
  LEFT_ANCHOR + ROOTING + `(${CONTAINER_ROOT_ALT})\\/(${CIRCLE_DIR})(?:\\/(?![A-Za-z0-9_.*<]))?` + NAME_END,
  "g",
);
...
export const RETIRED_LAYOUT_FILES: Record<string, string> = {
  "skills/migrate/SKILL.md":
    "the v11 -> v12 store-name migration, the transition between two layouts, both named literally",
};
...
function carriesForeignQualifier(before: string): boolean {
 ...
 */
export const SHAPE_DECIDED_KINDS: CitationKind[] = ["record", "package-record", "package-dir"];
...
export function createScanner(workbenchRoot: string, opts: { exhibits?: string[] } = {}): Scanner {
  ...
  function circleDirs(): Map<string, string[]> {
    ...
    for (const top of CONTAINER_ROOT_NAMES) add(top);
    ...
    if (existsSync(archive)) {
      for (const sweep of readdirSync(archive, { withFileTypes: true })) {
        if (sweep.isDirectory() && SWEEP_DIR_RE.test(sweep.name)) {
          ...
          for (const top of ARCHIVED_CONTAINER_ROOTS) add(`archive/${sweep.name}/${top}`);
        }
      }
    }
    circleDirIndex = dirs;
  ...
  function scanRecordCitations(
    ...
    for (const h of scanCitationTokens(rel, lines)) {
      ...
      if (h.status === "resolved" || h.status === "spelled-marker" || h.status === "ambiguous") resolved++;
...
export interface DeclaredCitationPaths {
  ...
  missing: { pattern: string; paths: string[]; named: number }[];
  unavailable: boolean;
}
...
export function declaredCitationFiles(
  ...
  const out: DeclaredCitationPaths = {
    files: [],
    unmatched: [],
    refused: [],
    missing: [],
  ...
  for (const pattern of patterns) {
    ...
    if (gone.length > 0) out.missing.push({ pattern, paths: gone, named: rels.length });
  }
  return out;
}
...
export function declaredCitationNotes(d: DeclaredCitationPaths): string[] {
  ...
  for (const m of d.missing) {
    notes.push(
      `declared pattern names ${m.paths.length} of ${m.named} file(s) the work tree does not hold, ` +
