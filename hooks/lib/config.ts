 ...
 */
function explainArrayOfNonEmptyStrings(value: unknown): string | null {
...
interface LeafRule {
  /**
   * `null` when the value is usable; otherwise the clause completing
   * `"<container>.<leaf>" ___.`, INCLUDING whatever it wants to say about what
   * it got. The message is produced by the check rather than stored beside it,
   * because a check with more than one way to fail has more than one thing to
   * say and a constant can say only the first of them.
   */
...
const CONTAINER_LEAF_RULES: Record<string, Record<string, LeafRule>> = {
  ...
  citations: {
    ...
    exhibits: { explain: explainArrayOfRecordBasenames },
  },
};
...
const RETIRED_PROJECT_FILES: Record<string, string> = {
  "fusion-guard.json":
    `fusion removed the guard settings this file configured, and every setting that outlived them now lives in ${PROJECT_CONFIG_FILENAME} at the project root. Nothing in this file is read. If it sets "orchestrator.maxTurns", that setting no longer exists anywhere — the Turn loop it bounded was removed — so there is nothing to copy across for it; any other setting belongs in ${PROJECT_CONFIG_FILENAME}. Then delete this file to stop this advisory.`,
};
...
const RETIRED_TOP_LEVEL_KEYS: Record<string, string> = {
  ...
  churn:
    "the per-file churn heatmap and its warning thresholds were removed on 2026-08-15.",
};
 ...
 * `260804-1630_*_what-does-a-project-guard-object-inherit-for-a-key-it-does-not-supply.md` requires that equivalence rather than merely permitting it: two
 ...
 * which is retired inside its container (`260816-1915_*_how-much-of-the-configuration-loader-survives-when-its-only-leaf-is-the-turn-budget.md`, option 1).
 */
  ...
  for (const [key, value] of Object.entries(parsed)) {
    ...
    for (const [leafKey, leafValue] of Object.entries(value)) {
      ...
      if (retiredLeaf !== undefined) {
        diagnostics.push(
          `fusion configuration at ${source}: "${key}.${leafKey}" no longer exists — ${retiredLeaf} The key was ignored; the rest of this file is unaffected. Delete it to stop this advisory.`,
        );
        continue;
      }
      ...
      if (failure === null) {
        ...
        diagnostics.push(
          `fusion configuration at ${source}: "${key}.${leafKey}" ${failure}. The key was ignored and inherits as if it were absent.`,
        );
      }
    }
    raw[key] = kept;
...
export function loadConfig(sources?: ConfigSources): GuardConfig {
  ...
  const value: GuardConfig = {
    citations: {
      ...
      exhibits: pickCitations("exhibits"),
    },
    diagnostics,
  };
