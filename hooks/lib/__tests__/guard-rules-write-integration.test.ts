...
// The other three cases of this block — the `files` merge, the pre-seeded
// halt and the `fusion-guard.json` writer — moved to
// `guard-project-config-integration.test.ts` with the describes that needed
// them. What is left is the one capability that belongs to the exemption:
// the environment strip that makes the flag-unset half of every criterion
// below mean anything.
...
// ---------------------------------------------------------------------------
// The project-configuration describes that stood here have MOVED to
// `guard-project-config-integration.test.ts`: an unparseable project file
// reported rather than swallowed, what a project configuration can and cannot
// reach, and the plugin-repo case. Their subject is the loader, which is
// staying; this file's subject is the exemption and the protected list, which
// is not. Their probes were re-pointed at the decision-governed deny on the way
// across.
// ---------------------------------------------------------------------------
