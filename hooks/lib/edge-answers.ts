...
export interface AnswerRow {
  /** The work package the edge would be written into, by storeless basename. */
  dependent: string;
  /** The work package the edge points at, by storeless basename. */
  target: string;
...
export function isRunFile(base: string): boolean {
...
export function runFiles(root: string): string[] {
  ...
  const walk = (dir: string): void => {
    ...
    for (const e of entries) {
      ...
      else if (e.isFile() && isRunFile(e.name)) found.push(abs);
    }
  };
  try {
    if (statSync(wb).isDirectory()) walk(wb);
  } catch {
    return [];
  }
...
export function parseEdgeLine(line: string): Omit<AnswerRow, "answer"> | null {
...
function outcomesSection(lines: string[]): string[] {
...
export function answerFor(section: string[], id: string): Answer {
  ...
  for (const line of section) {
    ...
    let best: { at: number; value: Answer } | null = null;
    for (const value of OUTCOMES) {
      const at = new RegExp(`(?<![A-Za-z-])${value}(?![A-Za-z-])`).exec(after)?.index;
      if (at !== undefined && (best === null || at < best.at)) best = { at, value };
    }
...
export function readRunFile(path: string): AnswerRow[] {
  let lines: string[];
  try {
  ...
  for (const line of lines) {
    ...
    if (id === null || !/^\s*-\s*\*\*Edge:\*\*/.test(line)) continue;
...
export function computeEdgeAnswers(root: string): EdgeAnswersReport {
  ...
  const byPair = new Map<string, AnswerRow>();
  let edgeEntries = 0;
...
export function renderAnswerRow(row: AnswerRow): string {
