 ...
 * handle it.
 */
export const GIT_TIMED_OUT: unique symbol = Symbol("git timed out");
...
function attempt(root: string, args: string[], timeoutMs: number): GitResult {
  try {
...
export function git(
  ...
  if (first !== GIT_TIMED_OUT) return first;
