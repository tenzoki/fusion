...
function main(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        process.stderr.write(`fusion-review-coverage: unknown argument ${JSON.stringify(arg)}\n${USAGE}\n`);
        return 1;
    }
    const root = findWorkbenchRoot();
    if (root === null) {
        process.stderr.write("fusion-review-coverage: no fusion workbench above the working directory — nothing to check.\n");
        return 2;
    }
    const report = measureReviewCoverage(root, { since, head });
    const out = ["anchor=workbench-root"];
    ...
    if (report.why !== "") {
        out.push(`since=`, `head=${head ?? "HEAD"}`, "commits=0", "reviews=0", "unusable=0", "uncovered=0", "verdict=unchecked", `why=${report.why}`);
        process.stdout.write(out.join("\n") + "\n");
        return 0;
    }
    const unusable = report.reviews.filter((r) => r.why !== "").length;
    out.push(`since=${report.since}`, `head=${report.head}`, `commits=${report.commits.length}`, `reviews=${report.reviews.length}`, `unusable=${unusable}`, `uncovered=${report.uncovered.length}`, `verdict=${report.uncovered.length > 0 ? "uncovered" : "covered"}`);
    ...
    for (const c of report.uncovered)
        out.push(renderUncovered(c));
    for (const r of report.reviews)
        out.push(renderReview(r));
    out.push(`carried=${report.carriedFrom === null
        ? "(not recorded)"
        : report.carried.length === 0
            ? "none"
            : report.carried.join(", ")}`, `carried-from=${report.carriedFrom ?? ""}`);
    process.stdout.write(out.join("\n") + "\n");
    return 0;
}
