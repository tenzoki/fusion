...
/**
 * Whether an `execFileSync` failure is the budget running out. Node sets
 * `code` to `ETIMEDOUT` on that path and nowhere else; a git killed from
 * outside carries the signal but not the code, and is a decline.
 */
function isTimeout(err) {
    return typeof err === "object" && err !== null && err.code === "ETIMEDOUT";
}
function attempt(root, args, timeoutMs) {
    try {
        ...
        return isTimeout(err) ? GIT_TIMED_OUT : null;
    }
}
/**
 ...
 */
export function git(root, args, timeoutMs = GIT_TIMEOUT_MS) {
    const first = attempt(root, args, timeoutMs);
    if (first !== GIT_TIMED_OUT)
        return first;
    return attempt(root, args, timeoutMs);
}
