...
export function sessionAnchor(root) {
    ...
    if (row?.git_head_at_start)
        return { since: row.git_head_at_start, why: "" };
    return {
        since: "",
...
export function measureReviewCoverage(root, opts = {}) {
    ...
    const floorMs = anchorDate === null || anchorDate === GIT_TIMED_OUT ? 0 : Number.parseInt(anchorDate.trim(), 10) * 1000;
