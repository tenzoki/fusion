...
export const LIVE_STATE = [
    ...
    { path: ".check-stamps", why: "when each periodic check last ran here — written by /fusion:check" },
 ...
 */
const STORES = [...RECORD_STORES, ...WINDOW_LEGACY_RECORD_STORES, ...LEGACY_STORES];
/**
...
export function classify(rel, sessionHistory) {
    ...
    if (hasCommitMessageName(rel)) {
        return {
            ...
            why: "a commit-message-shaped name that no artefact store owns — the orchestrator's commit step prescribes " +
 ...
 */
function coerceStagingState(value) {
...
export function stagingSentence(report) {
    ...
    if (messages.length > 0) {
        parts.push(`A commit-message-shaped file that no artefact store owns is sitting in the workbench: ` +
