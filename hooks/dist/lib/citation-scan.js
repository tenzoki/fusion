                    ...
                    if (markerM) {
                        ...
                        if (wild.length > 0) {
                            return {
                                ...
                                problem: `stale marker '_${markerM.letter}_': the record now exists as ` +
                    ...
                    if (named.length === 0) {
                        ...
                        if (dashed && !md && headField) {
                            return {
                                status: "undecidable",
                                matches: [],
                                problem: "a head-field value naming no record: an identifier, or a citation whose record moved; " +
...
export function partition(hits) {
    ...
    const undecidable = (h) => !unjudged(h) && (h.kind === "stamp-bare" || h.status === "ambiguous" || h.status === "undecidable");
    return {
