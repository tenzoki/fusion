...
export const DISPATCH_BYTES_ADVISORY = "dispatch-bytes: the rule emission could not be measured";
/**
 ...
 */
export function resolveRulesHelper() {
    const candidates = [];
 ...
 */
export function emissionStamp(root, pluginRoot) {
    const dirs = [
...
const runRulesHelper = (helper, agent, cwd) => execFileSync(helper, [agent], {
    cwd,
    encoding: "utf-8",
 ...
 * against.
 */
function sumEmission(root, stdout) {
    let total = 0;
    for (const raw of stdout.split("\n")) {
        const line = raw.trim();
        if (line === "" || line.startsWith("skill:"))
            continue;
...
function coerceRuleSizes(value) {
    if (!isStateObject(value))
        return {};
    const out = {};
    for (const [agent, entry] of Object.entries(value)) {
        if (!isStateObject(entry))
            continue;
        const { pluginRoot, stamp, bytes } = entry;
 ...
 */
export function ruleBytes(root, agent, run = runRulesHelper) {
 ...
 */
export function measureDispatchBytes(root, agent, run = runRulesHelper) {
    const pluginRoot = resolveRulesHelper()?.root;
    const promptBytes = pluginRoot === undefined ? undefined : sizeOf(resolve(pluginRoot, "agents", `${agent}.md`));
    ...
    if (rulesBytes === undefined) {
        return {
            fields,
            advisory: `${DISPATCH_BYTES_ADVISORY}; bytes_rules and bytes_total are absent from the ` +
 ...
 */
export function workItemFromPrompt(toolInput) {
