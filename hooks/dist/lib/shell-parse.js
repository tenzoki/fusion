...
function capture(literal, literals) {
 ...
 * missed switch.
 */
function stripData(command, mode, literals) {
    const n = command.length;
    let out = "";
    let i = 0;
 ...
 */
function stripSubshellParens(token) {
...
function findBalancedParen(text, openIdx) {
    let depth = 0;
...
export function parseCommand(command, options) {
    ...
    const source = options.quoted === "capture"
...
export function resolveWord(token, literals) {
    ...
    for (;;) {
        ...
        if (m === null)
            break;
        if (m.index > last)
            parts.push({ code: true, text: token.slice(last, m.index) });
        parts.push({ code: false, text: literals.get(m[0]) ?? "" });
        last = m.index + m[0].length;
    }
    if (last < token.length)
        parts.push({ code: true, text: token.slice(last) });
    const first = parts[0];
