...
export const DOMAINS = ["code", "data"];
// ---------------------------------------------------------------------------
...
export const COUNT_NAMES = ["code_files", "data_files", "counted_by"];
...
function tokenize(src) {
    ...
    while (i < src.length) {
        ...
        if (c === '"' || c === "'") {
            ...
            if (close < 0)
                throw new CascadeError(`unterminated string literal in condition: ${src}`);
...
class Parser {
    ...
    parseCondition() {
        ...
        if (this.pos !== this.tokens.length) {
            throw new CascadeError(`trailing tokens after the condition in: ${this.src} (at ${JSON.stringify(this.peek()?.v)})`);
        }
    ...
    parsePrimary() {
        const tok = this.peek();
        if (!tok)
            throw new CascadeError(`condition ends early: ${this.src}`);
        if (tok.t === "int") {
            this.pos++;
        ...
        if (tok.t === "ident") {
            this.pos++;
            if (!COUNT_NAMES.includes(tok.v)) {
                const retired = RETIRED_COUNT_NAMES.includes(tok.v);
                throw new CascadeError(retired
        ...
        if (tok.t === "op" && tok.v === "(") {
            ...
            if (!this.eat("op", ")"))
                throw new CascadeError(`unclosed parenthesis in: ${this.src}`);
            return inner;
        }
...
function assertBoolean(e, src) {
    ...
    throw new CascadeError(`this condition is a value, not a comparison, so whether it fires depends on ` +
...
function unquotedIndexOf(line, ch) {
    let quote = null;
...
export function extractCascadeBlock(markdown) {
    ...
    if (found.length !== 1) {
        throw new CascadeError(`expected exactly one fenced block assigning both the \`${DOMAINS.join("` and `")}\` ` +
 ...
 * read as a branch.
 */
export function parseCascade(markdown) {
    ...
    const branches = lines.map((line) => {
        ...
        if (colon < 0)
            throw new CascadeError(`branch has no \`:\` separating test from result: ${line}`);
        ...
        if (!DOMAINS.includes(assign[1])) {
            if (RETIRED_DOMAINS.includes(assign[1])) {
                throw new CascadeError(`branch assigns domain "${assign[1]}", which was retired: the cascade has two ` +
            ...
            throw new CascadeError(`branch assigns domain "${assign[1]}", which is not one of ${DOMAINS.join(" | ")}: ${line}`);
        }
        const domain = assign[1];
        if (head === "else") {
            return { kind: "else", domain, source: line, condition: null };
        }
        const m = /^(if|elif)\b\s*(.+)$/.exec(head);
        if (!m)
            throw new CascadeError(`branch head is not readable: ${line}`);
        return {
            kind: m[1],
            domain,
            source: line,
...
function assertWellFormed(branches) {
    ...
    if (last.kind !== "else") {
        throw new CascadeError(`the cascade has no final \`else\`, so it is not total: some input reaches the end ` +
...
function lookup(counts, name, src) {
    const v = counts[name];
    if (v === undefined)
        throw new CascadeError(`no value supplied for \`${name}\`, needed by: ${src}`);
    return v;
}
function describe(v) {
    return typeof v === "string" ? JSON.stringify(v) : String(v);
}
function numeric(v, expr, src) {
    if (typeof v === "number")
        return v;
    const who = expr.kind === "var" ? `\`${expr.name}\`` : "a value";
    throw new CascadeError(`${who} is ${describe(v)}, not a number, and this branch does arithmetic on it: ${src}. ` +
...
function evaluate(e, counts, src) {
    switch (e.kind) {
        case "num":
            return e.value;
        case "str":
        ...
        case "compare": {
            ...
            if (e.op === "==" || e.op === "!=") {
                if (typeof l !== typeof r) {
                    throw new CascadeError(`this branch compares ${describe(l)} with ${describe(r)}, which are not the same ` +
                        `kind of value: ${src}`);
                }
                return e.op === "==" ? l === r : l !== r;
            }
        ...
        case "logical": {
            const l = evaluate(e.left, counts, src);
            if (typeof l !== "boolean")
                throw new CascadeError(`non-boolean operand to \`${e.op}\`: ${src}`);
...
export function evaluateCascade(branches, counts) {
    for (let i = 0; i < branches.length; i++) {
        const b = branches[i];
        if (b.condition === null)
            return { domain: b.domain, index: i, branch: b };
        const hit = evaluate(b.condition, counts, b.source);
        if (typeof hit !== "boolean")
            throw new CascadeError(`condition did not yield a test: ${b.source}`);
        if (hit)
            return { domain: b.domain, index: i, branch: b };
    }
 ...
 */
export function countsFromHelperOutput(stdout) {
    const read = (key) => {
        const m = new RegExp(`^${key}=(.*)$`, "m").exec(stdout);
        if (!m)
            throw new CascadeError(`bin/fusion-count-sources printed no \`${key}\` line`);
        return m[1].trim();
    };
    const num = (key) => {
        const raw = read(key);
...
const DOMAIN_LITERAL_RE = new RegExp(`(\`|"|')(${DOMAIN_NAMES})\\1|\\*\\*(${DOMAIN_NAMES})\\*\\*`, "g");
...
export const REACH = {
    ...
    covered: [
        ...
        {
            claim: "A paraphrase naming the counts in prose — code files, source files, data files, and " +
        ...
        {
            claim: "A stale copy restating the retired four-outcome cascade. Its two surviving outcomes " +
        ...
        {
            claim: "A stale copy written only in the two retired outcomes, `strategic` and `knowledge`. " +
    ...
    holes: [
        ...
        {
            claim: "A paraphrase naming its inputs in words the prose list does not carry. The list is a " +
    ...
    excluded: [
        ...
        },
        {
            ...
            measured: "fires",
            note: "Where the agent-roster line went, and the entry above is the only reason this one " +
