 ...
 * carrying it.
 */
const RETIRED_CONTAINER_LEAVES = {
    guard: {
        protectedPaths: "fusion removed the protected-path mechanism it configured, so declaring the list protects nothing and nothing reads it.",
    },
};
    ...
    for (const [key, value] of Object.entries(parsed)) {
        ...
        for (const [leafKey, leafValue] of Object.entries(value)) {
            ...
            if (kind === "project" && key === "guard" && leafKey === "enabled") {
                diagnostics.push(`Guard configuration at ${source}: "guard.enabled" cannot be set by a project — a project does not switch off the guard that governs it. The key was ignored.`);
                continue;
            }
            ...
            if (retired !== undefined) {
                diagnostics.push(`Guard configuration at ${source}: "${key}.${leafKey}" no longer exists — ${retired} The key was ignored; the rest of this file is unaffected. Delete the line to stop this advisory.`);
                continue;
            }
