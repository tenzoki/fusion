 ...
 */
export function churnKey(rawFilePath, cwd, root) {
    if (!root)
        return null;
 ...
 */
export function migrateChurnKeys(state, root, exists = existsSync) {
    const files = {};
...
function reanchor(key, root, exists) {
    const anchored = projectRelative(key, root);
    if (isAbsolute(anchored) || anchored.length === 0)
        return null;
    if (isAbsolute(key))
        return anchored;
    if (exists(resolve(root, anchored)))
        return anchored;
    const underWorkbench = `${WORKBENCH_DIR}/${anchored}`;
    if (exists(resolve(root, underWorkbench)))
...
export function rankThrashing(state, root, limit = 0, exists = existsSync) {
    ...
    ranked.sort((a, b) => b.stats.thrashingScore - a.stats.thrashingScore ||
