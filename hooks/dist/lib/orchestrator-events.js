 ...
 */
export const ABSENT_SESSION_ID_ADVISORY = "orchestrator-events: the hook payload carried no session identifier";
/**
 ...
 * this keeps it true.
 */
function adviseAbsentSessionId(rowKind, written) {
...
export function recordDispatchLaunch(input) {
    ...
    if (root === null)
        return;
    if (!eventRowsAdmitted(root, payloadSessionId(input)))
        return;
...
export function emitSubagentStop(input) {
    ...
    if (entry === undefined)
        return;
    // The admission check sits AFTER the mapping lookup on purpose. A sync dispatch's
    ...
    if (sessionId === undefined)
        adviseAbsentSessionId("task_done", admitted);
    if (!admitted)
        return;
    delete map[agentId];
    writeMap(root, map);
...
export function emitDispatchEvent(event, input) {
    ...
    const admitted = eventRowsAdmitted(root, sessionId);
    if (sessionId === undefined)
        adviseAbsentSessionId(event, admitted);
    if (!admitted)
        return;
    const identity = resolveIdentity(root);
 ...
 * session's brief and mode — a judgement no hook holds. It carried a
 ...
 * that wants the brief filters on its absence. Whether the model's row
 ...
 */
export function readCheckoutId(root) {
    try {
 ...
 */
export function newestHookSessionStart(root) {
    let text;
    try {
        text = readFileSync(eventLogPath(root), "utf-8");
    }
    ...
    let best = null;
    for (const line of text.split("\n")) {
        if (!line.includes(SESSION_START_WRITER))
            continue;
        let row;
        try {
            row = JSON.parse(line);
        ...
        if (row.event !== "session_start" || row.writer !== SESSION_START_WRITER)
            continue;
        if (mine !== undefined && row.checkout !== undefined && row.checkout !== mine)
            continue;
