...
function caveat(n) {
    ...
    return (`note=${rows} an outcome row this program could not resolve to one value, ` +
...
function main(argv) {
    if (argv.length > 0) {
        process.stderr.write(`fusion-edge-answers: unknown argument ${JSON.stringify(argv[0])}\n${USAGE}\n`);
        return 1;
    }
    const root = findWorkbenchRoot();
    if (root === null) {
        process.stderr.write("fusion-edge-answers: no fusion workbench above the working directory — nothing to read.\n");
        return 2;
    }
    ...
    if (report.unreadable > 0)
        out.push(caveat(report.unreadable));
    for (const row of report.rows)
        out.push(renderAnswerRow(row));
