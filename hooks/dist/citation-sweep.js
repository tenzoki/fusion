...
function candidateFor(hit) {
    ...
    switch (hit.kind) {
        case "record": {
            const m = new RegExp(`(${STAMP})((?:_[a-zA-Z*]_|${BRACKET_SLOT})?[^]*)$`).exec(t.slice(t.lastIndexOf("/") + 1));
            if (m === null)
                return null;
...
const SPLICED_RE = new RegExp(`(?<![A-Za-z0-9._\\/-])([A-Za-z]+)(${STAMP}(?:${MARKER_SLOT}|-)[A-Za-z0-9.…*-]*)`, "g");
...
function repairsOn(scanner, lineText, ownBase) {
    ...
    while ((m = SPLICED_RE.exec(lineText)) !== null) {
        ...
        if (!indexed(scanner, rest) && !scanner.circleDirs().has(rest))
            continue;
        if (!readsBackWhole(scanner, rest))
            continue;
...
function main(argv) {
    ...
    if (repair) {
        ...
        for (const abs of files) {
            ...
            for (const [, , r] of here.sort((a, b) => a[0] - b[0] || a[1] - b[1]))
                residual.push(r);
            if (n === 0)
                continue;
            touched++;
