 ...
 */
import { currentHead, measureStagingDrift, renderStagingRow, } from "./lib/staging-drift.js";
import { findWorkbenchRoot } from "./lib/workbench-root.js";
const USAGE = "usage: fusion-staging-drift";
function main(argv) {
    if (argv.length > 0) {
        process.stderr.write(`fusion-staging-drift: unknown argument ${JSON.stringify(argv[0])}\n${USAGE}\n`);
        return 1;
    }
    const root = findWorkbenchRoot();
    if (root === null) {
        process.stderr.write("fusion-staging-drift: no fusion workbench above the working directory — nothing to check.\n");
        return 2;
    }
    ...
    out.push(`head=${head === "" ? "(none)" : head.slice(0, 7)}`, `rows=${report.rows.length}`, `unstaged=${report.faults.length}`, `verdict=${report.faults.length > 0 ? "unstaged" : "clean"}`);
    for (const r of report.rows)
        out.push(renderStagingRow(r));
    process.stdout.write(out.join("\n") + "\n");
    return 0;
}
