...
/**
 * The mandated caveat. It states the count, that an absent field asserts
 * nothing, and that the doubt is unmeasurable from here — all three, because
 * all three are what the gate accepted.
 */
function caveat(n) {
    const items = n === 1 ? "1 item carries" : `${n} items carry`;
    return (`note=${items} no \`**Depends-on:**\` field, and an absent field is not a claim of ` +
        "independence: the grammar says the field is absent when there is nothing to say. " +
        "`ready=` is optimistic by that count, and nothing here can tell by how much.");
}
function main(argv) {
    if (argv.length > 0) {
    ...
    if (report.noDependsOnField > 0)
        out.push(caveat(report.noDependsOnField));
