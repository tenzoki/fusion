 ...
 */
import { measureStateDrift, renderRow, } from "./lib/state-drift.js";
import { findWorkbenchRoot } from "./lib/workbench-root.js";
const USAGE = "usage: fusion-state-drift";
function main(argv) {
    if (argv.length > 0) {
        process.stderr.write(`fusion-state-drift: unknown argument ${JSON.stringify(argv[0])}\n${USAGE}\n`);
        return 1;
    }
    const root = findWorkbenchRoot();
    if (root === null) {
        process.stderr.write("fusion-state-drift: no fusion workbench above the working directory — nothing to check.\n");
        return 2;
    }
    const report = measureStateDrift(root);
    const out = ["anchor=workbench-root"];
    if (!report.statePresent) {
        out.push("state=absent", "rows=0", "drift=0", "verdict=no-session-state");
        process.stdout.write(out.join("\n") + "\n");
        return 0;
    }
    out.push("state=present", `rows=${report.rows.length}`, `drift=${report.drifted.length}`, `verdict=${report.drifted.length > 0 ? "drift" : "clean"}`);
