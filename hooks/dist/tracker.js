...
import { citationFormSentence, lastReportedCitationForm, measureCitationForm, readRecord, recordReportedCitationForm, workbenchRecordPath, writtenLines, } from "./lib/citation-form.js";
/**
 ...
 * `## Closing a work item` step 2) — so an every-call
 ...
 * sat — would report the commonest correct state as a fault. A code-implementer writes an issue
 ...
 */
function measureCitationFormForModel(input) {
    if (!WRITE_TOOLS.includes(input.tool_name))
        return null;
    const written = extractFilePath(input.tool_input);
    if (written === null)
    ...
    if (text === null)
        return null;
    const report = measureCitationForm(root, rel, text, writtenLines(input.tool_name, input.tool_input, text));
    if (report.why !== "" || report.signature === "")
        return null;
...
async function main() {
    ...
    // reason.
    const parts = [coverage, staging, citations];
