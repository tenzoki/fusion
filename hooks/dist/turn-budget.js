...
function main(argv) {
    if (argv.length > 0) {
        process.stderr.write(`fusion-turn-budget: unknown argument ${JSON.stringify(argv[0])}\n${USAGE}\n`);
        return 1;
    }
    const root = findWorkbenchRoot();
    if (root === null) {
        process.stderr.write(`fusion-turn-budget: no fusion workbench found above ${process.cwd()} — run /fusion:setup at the project root.\n`);
        return 2;
    }
