...
function parseArgs(argv: string[]): Options {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--limit") {
      const value = argv[++i];
      if (value === undefined) throw new Error("--limit needs a number");
...
function main(argv: string[]): number {
  ...
  try {
    ...
    process.stderr.write(`fusion-churn-rank: ${(err as Error).message}\n${USAGE}\n`);
    return 1;
  }
  ...
  if (root === null) {
    process.stderr.write(
      `fusion-churn-rank: no fusion workbench found above ${process.cwd()} — run /fusion:setup at the project root.\n`,
    );
    return 2;
  }
  ...
  const lines = [
    ...
    ...ranking.ranked.map(
      ({ path, stats }) =>
        `score=${stats.thrashingScore} total=${stats.totalChanges} ` +
