 ...
 */
function eventKey(e: EscalationEvent): string {
 ...
 */
function unseenHaltEvents(
  seen: EscalationEvent[],
  now: EscalationEvent[],
): EscalationEvent[] {
  const budget = new Map<string, number>();
  for (const e of seen) {
    if (e?.level !== "halt") continue;
    const key = eventKey(e);
    budget.set(key, (budget.get(key) ?? 0) + 1);
  }

  const arrived: EscalationEvent[] = [];
  for (const e of now) {
    if (e?.level !== "halt") continue;
    const key = eventKey(e);
...
try {
  ...
  console.error(
    `Note: the halt was cleared, but the state could not be re-read to check for a halt raised meanwhile: ${String(error)}`,
  );
}
...
if (arrived.length > 0 || stillHalted) {
  ...
  if (arrived.length > 0) {
    ...
    console.error(
      `But ${arrived.length} halt${arrived.length === 1 ? " was" : "s were"} raised while this ran, which you were not shown:`,
    );
    ...
    if (stillHalted) {
      console.error(
        "The guard is still halted. Read what is named above, then run this again to clear it.",
      );
    } else {
      console.error(
        "That halt is not in effect either: the merge cannot tell a halt raised meanwhile from the one you were clearing, so it was written away with it.",
      );
      console.error(
        "Nothing is blocked right now — but the violation above happened. Read what it names; the next measured change to that path halts again.",
      );
    }
  } else {
    ...
    const stateFile =
      guardStatePath("escalation.json")?.filePath ??
      "the .guard-state/escalation.json file under the workbench named above";

    console.error("\nThe clear was written, but the guard is halted again.");
    console.error(
      "Nothing arrived to explain it. No halt event is on disk that you were not already shown, so this run cannot name a second violation: something put the halt back without recording one.",
    );
    console.error(
      "That is what a writer still holding the state from before the clear does, and what a hand edit of the state file does:",
    );
    console.error(`  ${stateFile}`);
    console.error(
      "The guard is still halted. Run this again once nothing else is writing that file. If it comes back halted with nothing named, read the file yourself.",
    );
  }
  process.exit(2);
}
