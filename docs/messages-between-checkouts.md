...
**Before the first use, update and restart.** A session reads its command roster from the installed copy of fusion when it starts, and never re-reads it. On an install that predates this feature, neither half exists: run `fusion --update`, then restart the session, once. Both commands then resolve.
...
## Writing: the `post` command

The writing half is `/fusion:post`, a command of its own that you type when a session has something to hand over. `/fusion:cleanup` commits and pushes and does nothing else, so it no longer offers a message; before v11 it did, inside its one stop, and the `--skip` and `--only` selectors that governed that are gone with the pipeline.

`/fusion:post` composes a draft of at most twenty lines, shows it to you, and asks once: write it, change it, or cancel. On a yes it writes exactly one file into the message store and commits nothing, so the entry travels with your next commit and push. It takes one optional argument, `--since <commit>`, the start of the commit range the entry names.
...
Run it before `/fusion:cleanup`, so the entry is committed and pushed with the work it describes; written afterwards, it waits for your next push, and whoever pulls in between gets the work without its message.
...
Fourteen days. An aged entry is moved out of the live store by `/fusion:archive` at tier 1, which you run when you choose and which asks before it moves anything. It is archived, not deleted.
...
**A project that does not track its workbench in git gets nothing, and is told which state it is in.** The delta is taken from two git tree listings. Where `fusion-workbench/` is untracked, the store appears in no tree and there is nothing to read against, so `bin/fusion-forum new` exits 5 printing `state=workbench-untracked` instead of a successful `new=0`, and the news body names the state that came back rather than reporting an empty result. Measured against a scratch project with the workbench in `.gitignore`, with an entry sitting in the author's store; the same project tracked returns the entries. Whether a project commits its workbench is that project's own decision and fusion ships no rule either way, so the untracked configuration is supported and the feature stays inert inside it, though no longer silent. Filed as `260908-0848_*_an-untracked-workbench-answers-new-equals-zero-forever-and-no-state-names-it.md`, which the naming closed.
...
The flow on each side lives in `skills/news/SKILL.md` and in `skills/post/SKILL.md`; the retention rule and its accepted cost are in `skills/archive/SKILL.md` under tier 1.
