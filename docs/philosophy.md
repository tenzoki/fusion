...
**1. Specialization beats generalists.** Fusion ships **a fleet of narrow agents** — an orchestrator that dispatches the rest, plus coder, ontocoder, planner, shaper, taskplanner, playmaker, reconciler, coderev, ontorev, conceptrev, analyst, investigator, bugfixer, consultant, editor, and curator. Each has a tight scope enforced in its prompt: a `coder` cannot edit ontology YAML, an `ontocoder` cannot edit Go, a reviewer never edits what it reviews. The cost is dispatch overhead — one task may pass through three agents before it lands. The payoff is reasoning quality per pass, because no agent strays into a layer it shouldn't touch.
