...
3. **From a story in Plane**, if you run the Plane bridge. `/fusion:seed-from-plane <id>` reads that story's title and description once and hands them to the shaper as the draft. Same mode, same anticipated Circle, and activation stays your explicit choice ([`skills/seed-from-plane/SKILL.md`](../skills/seed-from-plane/SKILL.md)).

**A request you hand the orchestrator creates no Circle.** It runs against the Circle that is already active, and when none is active it runs without one — its plans, issues and decisions land in `shared/`, which is the ordinary case and the one "implicitly" above describes. A Circle becomes active by being activated, with `/fusion:next <circle-dir>`, never by a session starting.
...
Nothing in steps 1 to 4 writes what an active Turn loop writes, so this path is safe to walk in the middle of an active session. The playmaker's writes are four — the sections it appends to Circle records, the portfolio, its own history log, and the backlog store it maintains — and the Turn loop writes none of them ([`skills/next/SKILL.md`](../skills/next/SKILL.md) `## Boundaries`). A new anticipated Circle does not disturb the active one.
