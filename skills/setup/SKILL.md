...
- Effective protected paths: run the `bin/fusion-protected-paths` block from `$FUSION_SRC/agents/orchestrator.md` Setup Step 5 — the `[ -x ]` guard is part of it — and relay its `summary=` line and `path=… origin=…` lines in the Setup-complete summary. That section is the canonical implementation; do not restate its output shape or its exits here. A run that took no report says so and is reported as such, never as a project with nothing protected.
