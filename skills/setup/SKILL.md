...
**`UNRESOLVED` is not a path, and no step below reads through it.** `bin/fusion-source-root`'s own header carries the branch, the guard, `UNRESOLVED` and the read-versus-run split; what is Setup's own is the consequence. When the print says `UNRESOLVED`, name it in the Setup-complete summary, say which steps citing a plugin file were not run, and tell the user to restart the session so the SessionStart hook exports the variable. The check is this print, once: Step 0e re-resolves the root itself, and it is the **one named exception** to the split, reading and refreshing the four stylometric profiles at the source root, decided for that comparison alone (`260820-2324_*_is-the-work-tree-the-refresh-source-when-setup-runs-in-the-plugins-own-repository.md`).
...
This is the Circle-container layout defined in `rules/fusion-workbench-conventions.md` `## fusion-workbench Layout`, which enumerates every store and every root-anchored surface. Two facts about it are Setup's own:

- **A Circle is a directory, not a file.** `circles/` starts empty; each unit of work later gets `circles/<YYMMDD-HHMM>-<slug>/` with its own stores. Setup does not create any Circle.
...
**The Turn budget and the dispatch bound are resolved here too, both out of one block.** Run the `bin/fusion-turn-budget` block from `$FUSION_SRC/agents/orchestrator.md` Setup Step 2 — the `[ -x ]` guard is part of it — and hold both answers for the session the same way the `fusion-paths` values are held. That section is the canonical implementation and carries the unresolved branch for each: the budget's four consequences, and the dispatch bound's harmless one, where no stopping time reaches any dispatch prompt and every dispatch runs to its natural end as it does today. Do not restate them here, and do not substitute a number for a value that did not resolve. Report both values, or the fact that one did not resolve and why, in the Setup-complete summary — and with it every diagnostic line the helper put on stderr, which that section requires and which arrives even when the budget resolves.
