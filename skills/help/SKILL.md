...
**Coming from a v10.14 install:** v10.20 changes the citation form: a record is cited by its storeless basename, `YYMMDD-HHMM_*_<topic>.md`, and a store segment in a citation is a violation. `bin/fusion-citation-check` is new and prints a `verdict=` line over the project's own corpus; `/fusion:cleanup` Step 8 repeats it, reading `helper-missing` until `fusion --update`. fusion rewrites nothing; a project sweeps its own corpus with `node "$FUSION_SRC/hooks/scripts/citation-sweep.mjs" --dry-run` first. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-20.md`.
