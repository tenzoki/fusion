...
**Coming from an 11.4.1 install:** 11.5.0 adds an eleventh periodic check and asks nothing of the user. `claude-md` reports by heading where a project's `CLAUDE.md` carries its weight, and renders no verdict about what a section is *about*; that judgement is the reader's. It comes due on the ordinary 30-day cadence; `/fusion:check --only claude-md` runs it now. Second, the curator may now **move** a passage it is forbidden to delete, so a non-obvious failure mode or a hidden-coupling note can go behind a pointer instead of staying in place or being lost; a relocation is opt-in and `/fusion:curate` does not ask for one, so an ordinary run behaves exactly as before. And `$FUSION_SRC/rules/context-lean-claude-md.md` now states a test a reader can apply to one heading and get the same answer twice. No setting moves, no command changes, no file in a consuming project is rewritten, and there is no upgrade note for 11.5.0.

**Coming from an 11.4.0 install:** 11.4.1 corrects two sentences of shipped text and changes nothing else — no setting, no file, no command, no behaviour, and no upgrade note. Both named the work-order helper without its root, so the path pointed into fusion's own work tree and resolved to nothing in a consuming project. One of the two was in this topic, in the 11.3.0 paragraph that has since dropped off the end of it; the other is the `**Depends-on:**` paragraph of `$FUSION_SRC/docs/working-model.md`, which now reads `$FUSION_PLUGIN_ROOT/bin/fusion-work-order`.

**Coming from an 11.3.0 install:** 11.4.0 removes one command. **The one thing to act on:** the `log-activity` command no longer resolves — type `/fusion:cadence` instead, which now writes this checkout's activity log out of git and the whole workbench tree and then digests it, so the log the digest reads is never stale and there is nothing to run first. Two things move in the digest itself: recurring themes rank by the number of distinct **days** a theme appears in rather than sessions, and the `**Covers:**` line is gone, because the merged body reads the record it wrote rather than opening each session history, so the writer half is no longer an input — the digest names no writers, in any window. And `/fusion:cadence` now halts when no workbench sits above the working directory, instead of writing a log into whatever directory the user happened to stand in. An existing `activity-log-<checkout>.md` is refreshed in place, nothing else is rewritten, no setting moves. `$FUSION_SRC/docs/upgrading-to-v11-4.md` is the note.
