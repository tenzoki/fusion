...
2. **Pick the right entry point** depending on what you're doing:
   ...
   - A goal to capture without starting work on it → `/fusion:memo` files it as its own work package in the project backlog, where it waits at `open` until somebody claims it. Its path is then a valid input to **requirements-designer**, which turns it into a spec without touching the item
...
3. **The workbench is the project's cross-session memory.** `fusion-workbench/` holds one store per artefact kind (plans for the approach, issues for "go fix it", decisions for "decide and record", plus reviews, analyses and the frozen history corpus) in two places: each work package's own container in the `work-packages` store, and `shared/` for work belonging to no package. **The Origin Rule makes the placement decision**: an artefact belongs to the package whose brief caused it, and to `shared/` when no item is in scope.
