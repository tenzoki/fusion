...
**Coming from a v10.26 install:** v11 is a cut. Five agent names stop resolving: `taskplanner`, `playmaker`, `bugfixer`, and `coderev`/`ontorev`, now one `reviewer` taking a `**Review domain:**` line. The Turn loop is gone, and the Circle becomes the work item, with a `**Status:**` and a `**Claim:**` head field. **A workbench still holding a live Circle record must run `/fusion:migrate`**, or every new record lands in the shared store instead of inside its item; `/fusion:setup` refuses until it is converted. `/fusion:cleanup` is commit and push, its pipeline now `/fusion:reconcile`, `/fusion:archive`, `/fusion:log-activity`, `/fusion:curate` and `/fusion:post`. Point the user at `$FUSION_SRC/docs/upgrading-to-v11.md`.
