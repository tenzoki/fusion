...
**Coming from an 11.0.0 install:** 11.0.1 asks nothing of the user. No setting, no file and no command changes, and no shipped program behaves differently. It corrects documentation that described a workbench layout this version does not have, removes twelve statements that named the five agents v11 deleted as though they still resolve, repairs a file header in the hooks, and adds one regression test. There is no upgrade note for 11.0.1 and none is needed; a user who has not yet reached v11 still reads `$FUSION_SRC/docs/upgrading-to-v11.md` first.
