...
**Coming from a v10.4 install:** v10.5 adds one Setup step that speaks to the user — `/fusion:setup` Step 0e compares the four voice profiles in the workbench against the ones this version ships and asks **once**, in a single question covering every file that differs, whether to replace them or keep theirs. "Keep mine" is the answer for a profile the project edited; either answer stamps the file in `fusion-workbench/.asset-provenance`, which is what stops the question coming back every session. Everything else is text the agents read: `rules/user-facing-output.md` now bounds a reply as a whole rather than letting the excess move into a trailing block, and `bin/fusion-prose-metric` is a new measurement that reports and never gates. Two fixes land with it — the monitor's "Dismiss All" works again, and the config template no longer documents an example that is not valid JSON. Nothing is rewritten and nothing has to be migrated. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-5.md`.

**Coming from a v10.3 install:** v10.4 arms three blocking gates — over the citations between workbench records, over a live plan's `## Where this Circle stops` section, and over the committed `hooks/dist/` — and **all three run in fusion's own test suite, not in the user's project**: they read the plugin's own workbench and git objects, and `install.sh` copies neither. What can reach a user is one halt: the shaper's `**Initiated by:**` audit line is now required on **every** portfolio-activation run, top-level ones included, because the self-test that used to waive it read a signal that separates nothing. `analyst` also gains a project-local rule pattern, so a short `./rules/analyst-capture-layout.md` now loads on every `analyst` run — which is the cheap fix for an orphaned `investigator-capture-layout.md` carried across from v9. Nothing is rewritten and nothing has to be migrated. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-4.md`, and at the v10.5 note above as well.

**Coming from a v10.2 install:** v10.3 takes the `**Status:**` head field off the decision-record template as well, and adds one question before a Circle closes — the orchestrator reads the plan's `## Where this Circle stops` clauses back to the user and asks whether each holds. Nothing is rewritten and nothing has to be migrated; existing records keep the field, including when they are transitioned. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-3.md`, and at the v10.4 and v10.5 notes above as well.

**Older than that:** this section carries the last three releases and no more, because it is a per-release list on a surface with a fixed byte ceiling. Every release since v9 has its own note under `$FUSION_SRC/docs/`, named `upgrading-to-<version>.md`; a project further behind reads them in version order, starting at its own. One of them carries an action that fails silently when it is skipped: v10 retired the project-root `fusion-guard.json`, and a Turn budget left inside it is not read, so it has to be copied into `fusion.json` before the old file is deleted.
