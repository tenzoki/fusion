...
**Coming from an 11.7.0 install:** 11.8.0 changes two things in the curator's edge survey and asks nothing of the user. An entry the gate never put to anybody now carries the outcome `not-offered` rather than the false `skipped`, so in all four subjects an outcome line means that gate was answered and no line means none was. And the suppression read, which pulled every prior run file into context — 971 958 bytes over fourteen — is now `bin/fusion-edge-answers`, which prints the answered pairs. No setting moves and there is no upgrade note for 11.8.0.

**Coming from an 11.6.0 install:** 11.7.0 gives the curator a fourth subject and asks nothing of the user. `/fusion:curate --edges` reads the live work items and the records they cite, classifies each relation it finds, and proposes `**Depends-on:**` or `**Cross-references:**` entries — each naming both endpoints, how each was identified, whether the direction was fixed by the words or inferred from the work, and the sentence it was read from. Nothing is written before the gate, and a rejection leaves every work item byte-identical, so a run you decline costs nothing. It closes the half of the September planning work that was specified and never built: `bin/fusion-work-order` has computed order, depth, blocked counts, readiness and cycles since then over edges only a hand-written entry could supply. A `/fusion:curate` run without the flag is unchanged, no setting moves, and there is no upgrade note for 11.7.0.
