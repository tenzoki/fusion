...
**Coming from an 11.3.0 install:** 11.4.0 removes one command. **The one thing to act on:** the `log-activity` command no longer resolves — type `/fusion:cadence` instead, which now writes this checkout's activity log out of git and the whole workbench tree and then digests it, so the log the digest reads is never stale and there is nothing to run first. Two things move in the digest itself: recurring themes rank by the number of distinct **days** a theme appears in rather than sessions, and the `**Covers:**` line is gone, because it named the writers of the session histories and that store has been closed to writes since v11. And `/fusion:cadence` now halts when no workbench sits above the working directory, instead of writing a log into whatever directory the user happened to stand in. An existing `activity-log-<checkout>.md` is refreshed in place, nothing else is rewritten, no setting moves. `$FUSION_SRC/docs/upgrading-to-v11-4.md` is the note.
