...
**Coming from a v10.22 install:** v10.23 gives every checkout an entry of its own. `/fusion:setup` asks once for the person and a name and writes one file into the workbench's shared checkout store, and four surfaces render that name where they rendered the eight hex characters; the name is an attribute and the hex stays the key, so no record and no event line is rewritten. Four personal log files (memos, tasks, cadence, and the activity log at the project root) are keyed by that identifier instead of the OS login, and the skill that writes each renames an existing file onto the new name on its next run. A resolution line cites a heading rather than `path:line`, an answered or deferred decision names who ruled, and only the orchestrator moves a decision from open to answered. Setup also reports how far behind its upstream the checkout stands, read locally and without fetching. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-23.md`.
