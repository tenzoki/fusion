...
If no specific topic is identifiable, open with **the daily loop**: morning `/fusion:cadence`; work — start the orchestrator and say what you want; ideas `/fusion:memo`; done `/fusion:cleanup` and walk away (one question waits). Then list the five topics with one-line summaries and ask which to drill into.
...
**Coming from an 11.2.0 install:** 11.3.0 widens the work item and asks nothing of the user. **The one thing to act on, and it is an option rather than a step:** `**Status:**` takes a fifth value, `paused` — set aside deliberately, expected back. It is entered from `open` or `claimed` and left to `open`, `claimed` or `dropped`, the `**Claim:**` is cleared, and the body says *what* the item waits for and never a date. A `**Depends-on:**` entry naming a paused item stays live and blocks, so `bin/fusion-work-order` prints it with readiness `paused` and its dependents `blocked`. Second, `**Active spec/plan:**` lets an item name the artifact the work runs on; `/fusion:migrate` carries it across. Existing items keep the status they have, no setting moves, no command changes, and there is no upgrade note for 11.3.0.

**Coming from an 11.1.0 install:** 11.2.0 changes one behaviour and asks nothing of the user. A commit whose only path is the event log no longer gets a row in that log. The row used to dirty the tree it had just been committed from, and committing that dirt wrote another row, so no run of commits ever ended clean; one commit now settles it. Two consequences to know rather than act on: the log can no longer say when the log itself was committed, and "every landed commit under the commit lock has a row" now carries an exception, so anything counting commits out of that log reads low. Merge and empty commits keep their row. No setting, no file and no command changes, and there is no upgrade note for 11.2.0.

**Coming from an 11.0.1 install:** 11.1.0 adds and widens; nothing a project depends on was removed. **The one thing to act on:** `agents/orchestrator.md` no longer declares a `tools:` line, so the orchestrator inherits the session's tools. MCP servers, `ToolSearch`, `WebSearch` and whatever a project adds are reachable from it, and a hand patch to an installed copy is now unnecessary: update, then delete the patch. Three changes reach the work-item store. A `**Depends-on:**` entry asserts one relation and one only, that the named item reach `done` or `dropped` before this item may start; every other citation an item carries goes in `**Cross-references:**`, a new head field that orders nothing. `/fusion:migrate` writes no `**Depends-on:**` at all, because a conversion cannot confirm a prerequisite, and puts the names it can still resolve in `**Cross-references:**`. And `bin/fusion-work-order` is new: it reports depth, transitive blocking count, topological order, readiness and cycles over the store, computing per run, storing nothing and gating nothing. Beside the store, an agent may now dispatch another operative agent; a nested dispatch that could meet a human gate halts rather than answering it, and no agent originates a work item, which is unchanged. Existing records are not rewritten, no setting moves, and there is no upgrade note for 11.1.0.

**Older than that:** this section carries the last three releases and no more, because it is a per-release list on a surface with a fixed byte ceiling. For an older install, run `ls $FUSION_SRC/docs/` and read every `upgrading-to` note above the user's own version, in version order; not every release has one, and no filename is derivable from a version string. One of them still carries an action that fails silently when it is skipped: v9 retired the `strategic` and `knowledge` domain values, and a record still carrying one runs as `code` without saying so. The v10 note's own copy-the-budget step no longer applies — the setting it rescued was retired outright on 2026-09-10, so `fusion-guard.json` is deleted and nothing is copied out of it.
