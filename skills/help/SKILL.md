...
If no specific topic is identifiable, open with **the daily loop**: morning `/fusion:cadence`; work — start the orchestrator and say what you want, or `/fusion:next`; ideas `/fusion:memo`; done `/fusion:cleanup` and walk away (one question waits). Then list the five topics with one-line summaries and ask which to drill into.
...
5. **Recovering after a crash:** just start the orchestrator again — its Setup finds `fusion-workbench/agentstate.yaml`, leads with one line of where the session stood, and recommends **Continue**.

6. **Three commands frame the work, and there are only three to remember.** `/fusion:setup` at the start of a session, `/fusion:cleanup` at the end, `/fusion:cadence` whenever you want to see what you have actually been doing. Cleanup is a pipeline: it files issues for what is unfinished, commits and pushes in meaningful splits, reconciles, archives, regenerates the activity log, then reconciles `CLAUDE.md` **at the one gate — deliberately last**, and commits the housekeeping after your answer. `--only <step>` runs one of those steps alone (`--only archive`, `--only claude-md`, `--only log-activity`, `--only forum`), `--skip <step>` runs everything else, `--dry-run` shows what would happen, `--no-push` keeps the commits local. Tell a user who asks about archiving, the activity log, or `CLAUDE.md` maintenance to reach for cleanup with `--only`, not for a separate command.

   **You can type cleanup and walk away**: everything up to the gate completes unattended, the one `CLAUDE.md` question waits for your return, and apply plus the housekeeping commits follow your answer. `--skip claude-md` runs gateless end to end.
...
**Coming from a v10.25 install:** v10.26 hands a dispatched agent a wall-clock stopping time. Seven agents — coder, ontocoder, bugfixer, reconciler, coderev, ontorev, curator — receive one line saying when to stop where they stand, hand back what they finished together with what they did not, and the orchestrator continues that work in a fresh dispatch at the same place. Nothing is killed when the clock runs out: the bound is requested and never enforced, so an agent that ignores it behaves as before. `orchestrator.dispatchMinutes` (default 20) was the number that fed it. **All of that was retired again after v10.26**: no dispatch carries a stopping time, and a project still declaring the setting gets one advisory naming the leaf. `bin/fusion-events dispatches` is new and stays, reporting how long each dispatch ran against a threshold the reading itself sets. One v10.25 defect closes: the message half of `/fusion:cleanup` did not write on the pipeline path, only under `--only forum`. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-26.md`.

**Coming from a v10.24 install:** v10.25 puts a message between two checkouts of one project. A session that ends offers to leave one short note in the repository, and the side about to pull reads it with `/fusion:news` out of a fetched ref, with nothing merged and the working tree untouched. The writing half rides the end of `/fusion:cleanup` as a second question inside the one gate, so a run you walk away from still completes; `--only forum` runs it alone, `--skip claude-md` drops it with the step, and `--dry-run` writes nothing. `orchestrator.dispatchMinutes` joins the configuration loader (default 20) and nothing reads it yet. One defect ships open: on a project that does not track `fusion-workbench/` in git, the reading half reports nothing new whatever arrives. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-25.md`.

**Coming from a v10.23 install:** v10.24 checks the citation form of a record at the moment it is written and reports back to the writer; a PostToolUse hook has no verdict to give, so it blocks nothing and rewrites nothing. `bin/fusion-citation-check` gains a column separating the violations a sweep may repair from the ones no exemption lets it touch, which is what a build gate can be wired on. The monitor takes port 0 and can write the port it got to a file named by `MONITOR_URL_FILE`, and the orchestrator's commit-message file is per session instead of one shared path. One defect ships open: under load `git` can exceed its deadline and read as no git here. Point the user at `$FUSION_SRC/docs/upgrading-to-v10-24.md`.
