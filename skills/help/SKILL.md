...
**Read the source files this workflow points at; do not paste them whole.** Synthesize a focused answer in your own voice and cite file paths so the user can read the originals if they want.

If a question goes beyond what the docs cover, **say so** — do not invent.

**A path into a file the module ships carries the `$FUSION_SRC` root.** Every topic below sends you to a shipped doc, README or rule file; nothing the module ships exists at a consuming project's root, so a bare `docs/…` or `rules/…` path resolves to nothing there. Resolve the root once, before reading the first one:
...
Read `$FUSION_SRC/docs/philosophy.md`. It covers the five "Why it's built this way" pillars of the fusion module — specialization beats generalists, coordination through files (not shared memory), traceability as a first-class output, compliance over speed, and one framework across many project shapes — the last being the domain-parameter design that lets the same plumbing serve `code` and `data` projects.

For *how the machinery actually runs* — a work package's life from its brief to its closure, the spec-driven flow, the approvals, and the compliance guard end to end — point the user at `$FUSION_SRC/docs/working-model.md` (the operational companion to this "why" doc).
...
2. **Pick the right entry point** depending on what you're doing:
   ...
   - A goal to capture without starting work on it → `/fusion:wp` files it as its own work package in the work-package store, where it waits at `open` until somebody claims it. Its path is then a valid input to **requirements-designer**, which turns it into a spec without touching the package
   - "What should I work on next?" → read the work-package store yourself; the portfolio briefing and the session work queue both went at v11 with the agents that built them
...
3. **The workbench is the project's cross-session memory.** `fusion-workbench/` holds one store per artefact kind (plans for the approach, issues for "go fix it", decisions for "decide and record", plus reviews, analyses and the frozen history corpus) in two places: each work package's own container in the `work-packages` store, and `shared/` for work belonging to no package. **The Origin Rule makes the placement decision**: an artefact belongs to the package whose brief caused it, and to `shared/` when no package is in scope.

   The layout is defined once, in `$FUSION_SRC/rules/fusion-workbench-conventions.md` (`## fusion-workbench Layout`). Read it there and cite it rather than reciting paths from memory — agents themselves do not hard-code these paths either; they resolve them at run time via `$FUSION_PLUGIN_ROOT/bin/fusion-paths <name>`. If the user wants to know where a given artefact will land in *their* project, run that resolver and show them, rather than guessing from the layout.
...
**Coming from an 11.11.2 install:** 12.0.0 renames, and the old names keep working until 13.0.0 wherever they are persisted. Seven agents take their PRIOR/Fusion names: `shaper` → `requirements-designer`, `planner` → `implementation-planner`, `coder` → `code-implementer`, `ontocoder` → `data-implementer`, `reconciler` → `state-auditor`, `editor` → `document-editor`, `curator` → `policy-curator`. `fusion <old name>` and `Agent(fusion:<old name>)` no longer start one; a plan step whose `Executor:` names one is dispatched under the new name. Three stores are renamed, the container store `circles` to `work-packages`, `planning` to `plans`, `consult` to `consultations`; the old directories are read beside the new ones, new records land under the new names, and `/fusion:setup` names each old one it finds. `/fusion:migrate` renames them; commit that as one commit and push it. The dispatch parameter `**Item:**` is now `**Work package:**`, and `OUT_BACKLOG`/`SCAN_BACKLOG` are `OUT_PACKAGES`/`SCAN_PACKAGES`. The prose says work package, brief, evidence base and approval where it said Circle, Directive, Grounding and gate. A pre-v11 workbench converts first: `/fusion:migrate` under `claude --plugin-dir` on a `v11.11.1` checkout, then update. `$FUSION_SRC/docs/upgrading-to-v12.md` is the note.

**Coming from an 11.11.1 install:** 11.11.2 adds one command and moves two kinds of file. `/fusion:wp` files a new work package; `/fusion:memo` no longer does, and answers an `idea:` prefix by pointing there. Memo keeps one `tasks-<person>.md` and one `notes-<person>.md`, keyed by your git e-mail, and writes nothing without one; its next run appends this checkout's `memos-`/`tasks-` files to them and removes them. The periodic-check stamps move to a local, gitignored `fusion-workbench/.check-stamps`, so every check falls due once more on each checkout, and `/fusion:setup` drops the `checks` object from `.fusion-setup` on its next write. No setting moves; `docs/upgrading-to-v11.md` says the same.
