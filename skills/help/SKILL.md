...
**Coming from an 11.8.0 install:** 11.8.1 closes seven text defects in the hook headers, the READMEs and the archive body and asks nothing of the user. 11.9.0 adds one head field on a work item, `**Mode:** autonomous`, written on the user's word: by hand, through `/fusion:memo` from the user's own text, or by the orchestrator when the user asks for it at filing or claim. With it the orchestrator answers the stops about that item's solution itself (the plan review, the item's claim and finish, its stop-conditions read) and logs each as answered by the field; ontology, structural change, destruction and ambiguity still file an open decision and skip. Without it nothing changes, no setting moves, and there is no upgrade note for either release.
