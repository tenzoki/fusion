...
**Coming from an 11.5.1 install:** 11.6.0 adds one command and asks nothing of the user. `/fusion:discuss` runs a bounded discussion between the agent you are talking to and the consultant, round by round in the chat, and writes its claim register into the workbench's new discussion store from the first round onward, so a discussion broken off mid-way reads as interrupted rather than as pending. It begins, inspects and closes a discussion and decides nothing: `--infer` prints the register's current conclusions and leaves the discussion open, `--close` writes the final recommendation and ends it, and one discussion is open per session. The consultant also becomes dispatchable by the orchestrator, which was decided on 2026-09-13 and not carried out until now. No setting moves, no existing command changes, no file in a consuming project is rewritten, and there is no upgrade note for 11.6.0.

**Coming from an 11.5.0 install:** 11.5.1 fixes one defect in `/fusion:news` and changes nothing else — no setting, no file, no command, no behaviour, and no upgrade note. Its fourth step took the message body from the fetched ref and the writer's name from the working tree, so a checkout that had registered in a commit you fetched and had not merged was rendered as one that never registered, false at the moment it was printed: the registration sat in the very commit the body was read from. Both halves now read at the same commit.
