...
**Coming from an 11.4.0 install:** 11.4.1 corrects two sentences of shipped text and changes nothing else — no setting, no file, no command, no behaviour, and no upgrade note. Both named the work-order helper without its root, so the path pointed into fusion's own work tree and resolved to nothing in a consuming project. One of the two is in this topic, two paragraphs down, in the one describing 11.3.0 — if you typed that path on an 11.3.0 install and got no such file, the text was wrong, not what you typed. Both now read `$FUSION_PLUGIN_ROOT/bin/fusion-work-order`: here, and the `**Depends-on:**` paragraph of `$FUSION_SRC/docs/working-model.md`.
