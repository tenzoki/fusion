...
**Coming from an 11.9.0 install:** 11.9.1 closes one defect in the orchestrator's ending step and asks nothing of the user. A session ended in chat wrote its session-end row to the event log after the last commit and nothing committed it, so every such session left the log dirty in the tree; the ending step now commits that row on its own, under the commit lock. No setting moves and there is no upgrade note for 11.9.1.
