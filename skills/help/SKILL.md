...
2. **Pick the right entry point** depending on what you're doing:
   ...
   - A goal to capture without starting work on it → `/fusion:memo` files it as its own work package in the work-package store, where it waits at `open` until somebody claims it. Its path is then a valid input to **requirements-designer**, which turns it into a spec without touching the package
