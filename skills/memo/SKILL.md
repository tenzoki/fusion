...
- Never edit, rename, close or defer an existing backlog entry. This skill creates entries at `_o_` and does nothing else to the store. A marker moves when the user moves it by hand, or when the shaper closes an entry a Circle took whole.
