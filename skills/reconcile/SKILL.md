...
The user invoked `/fusion:reconcile`. This body performs one procedure: it dispatches `fusion:state-auditor` once, reads what comes back, and reports it. **It runs no other pass, reads no other skill body, and dispatches nothing else.**
...
**No store path is resolved.** This body names no workbench record, and the state-auditor resolves its own write and search targets at its own Setup.
...
## Step 4 — Dispatch the state-auditor

Use the `Agent` tool with target `fusion:state-auditor`. The parameter lines come first, in this order, each on its own line, and a value this run does not hold is **omitted rather than emptied**:
