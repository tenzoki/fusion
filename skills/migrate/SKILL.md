...
`CONFLICTS` restarts at zero here: this is a second shell and it inherits nothing. Add it to the count the first block printed rather than reading it as the total, and set `FOUND=1` yourself whenever `LIVE` or `CONFLICTS` is above zero.
