...
- **Never touch the root-anchored surfaces.** `orchestrator-events.jsonl`, `.guard-state/`, `.commit-lock/`, `.session-marker`, `.checkout-id`, `.cadence-anchors`, `.check-stamps`, `monitor`, `stilwerk/`, `.fusion-setup` stay where they are (`.active-circle` is the one exception, and it is deleted rather than moved — Step 4b says why); their consumers read them at fixed root-relative paths and none has a fallback (`rules/fusion-workbench-conventions.md` `## fusion-workbench Layout`).
