...
**Where `$MARKER` is `t`, the refusal is in the claim's terms and not the marker's.** Read the record's `**Claim:**` and this checkout's identity from `bin/fusion-identity`, guarded with `[ -x ]` as every helper call site is. Where the field opens with `Claimed ` and names another identity, say who holds the Circle and when the claim was written, then offer one override at an `AskUserQuestion`: *Take it over here* / *Leave it alone*. Taking it appends the `Overridden ` sentence per `$FUSION_SRC/agents/orchestrator.md` `## Circle head fields`, so the first sentence stays and both identities stand in the record, then writes `.active-circle` (Step 6.3) and nothing more: the record already carries `_t_`, so nothing is renamed. Leaving it writes nothing. `Unclaimed`, an absent field, or this checkout's own identity is the mismatch above and is reported as one (`$FUSION_SRC/rules/circle-records.md` `### The claim field`).
