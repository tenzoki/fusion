...
Overrides: `FUSION_REF` (git ref, e.g. `FUSION_REF=tags/v8.0.0` to pin a release — every release since v5.5.0 is tagged), `FUSION_HOME` (install dir, default `~/.fusion`), `FUSION_BIN` (launcher dir, default `~/.local/bin`).
...
- **Compliance guard** — the plugin's `hooks/config.json` defines the defaults: decision-to-path mappings and sensitivity levels, escalation thresholds (block → halt) and per-session churn thresholds. The per-project way to change any of it is `fusion-guard.json` at the project root — `/fusion:setup` seeds it (declaring nothing, inheriting everything), and every key you declare in it wins over the plugin's value, merged per leaf. It is git-tracked on purpose: changes to how strictly the guard reads your decisions should show in a diff. ([`hooks/config.example.json`](hooks/config.example.json) documents the full key shape.) Runtime state lives per-project in `fusion-workbench/.guard-state/` (gitignored). See [`README-hooks.md`](README-hooks.md) for the full guard model.

### Tuning or disabling the compliance guard

The guard isn't all-or-nothing. It runs on a spectrum from full enforcement to advisory to off, assembled from the fields already in `hooks/config.json`. Pick the row that matches how much friction you want:

| Goal | Change |
|---|---|
| Off entirely | `guard.enabled: false` |
