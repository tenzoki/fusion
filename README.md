...
Overrides: `FUSION_REF` (git ref, e.g. `FUSION_REF=tags/v10.22.0` to pin a release — every release since v5.5.0 is tagged), `FUSION_HOME` (install dir, default `~/.fusion`), `FUSION_BIN` (launcher dir, default `~/.local/bin`).

**Upgrading from v10.20?** v10.21 adds the checkout registry: on your next `/fusion:setup` the checkout registers itself as one file under `fusion-workbench/shared/checkouts/`, and four display sites render that name where they rendered eight hex characters. The alias is an attribute and the hex stays the key everywhere, so no record and no event line is rewritten; `bin/fusion-events presence` now counts two git identities one person has registered as one person, and deleting the store restores v10.20 behaviour exactly. `docs/upgrading-to-v10-21.md` is the note.
