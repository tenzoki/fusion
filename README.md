...
A multi-agent orchestration framework for Claude Code. Fusion runs a work session as a team of **15 specialized agents** — an orchestrator that dispatches the rest, plus coders, reviewers, planners, and analysts — coordinating through files on disk, with a compliance guard that catches drift at write time and a human at the decisions that matter.
...
- **Tune the guard to the work.** The compliance guard shines on high-trust work — ontology, schemas, regulated code, multi-domain knowledge — where a bad edit costs more than a slow one. For hot-loop prototyping where you want to thrash a file freely, writes to high-sensitivity decision-governed paths get blocked. That is not a bug; it just means you want the guard dialed down. See [Tuning or disabling the compliance guard](#tuning-or-disabling-the-compliance-guard) below.

## Configuration

- **Compliance guard** — the settings resolve through **three layers**, merged per leaf key: your project's `fusion-guard.json` at the project root, then the plugin's `hooks/config.json`, then the built-in defaults in `hooks/lib/config.ts`. A key you declare wins outright; a key you leave out, even inside an object you did declare, inherits from the next layer down. What the plugin's `hooks/config.json` actually ships is the escalation threshold (block → halt); its decision-to-path mappings, per-category sensitivities and decision list are shipped **empty** (`categoryPaths: {}`, `categorySensitivity: {}`, `decisions: []`), so a governed path exists only once a project declares one. The file a project edits is `fusion-guard.json` — `/fusion:setup` seeds it from [`templates/fusion-guard.json`](templates/fusion-guard.json), which declares nothing, inherits everything, and documents each key you can set in its own notes. It is git-tracked on purpose: changes to how strictly the guard reads your decisions should show in a diff. ([`hooks/config.example.json`](hooks/config.example.json) shows what filled-in `categoryPaths` and `decisions` look like; it predates both `orchestrator.maxTurns` and the retired key below, so read the template for those.) Runtime state lives per-project in `fusion-workbench/.guard-state/` (gitignored). See [`README-hooks.md`](README-hooks.md) for the full guard model.
...
| Goal | Change |
|---|---|
| Off entirely | `guard.enabled: false` in the plugin's `hooks/config.json`. It is the one key a project's `fusion-guard.json` may not set: a declared value is ignored and named in an advisory ([`README-hooks.md`](README-hooks.md#per-project-configuration-fusion-guardjson)) |
| Advisory-only (warns, never blocks) | keep `enabled: true`; leave every decision sensitivity at `medium`/`low`, since only `high` blocks |
...
| More (or fewer) Turns per session | `orchestrator.maxTurns`, declared as `{"orchestrator": {"maxTurns": <n>}}`. This one is not a guard setting and no hook reads it: `bin/fusion-turn-budget` resolves it once at the orchestrator's Setup, and it lives in this file because it is the per-project configuration surface a project already has. The default is defined in exactly one place, `DEFAULTS` in `hooks/lib/config.ts`, and is deliberately not restated in the plugin's `hooks/config.json`. A value that is not a whole number of 1 or more is dropped, named in an advisory, and inherits as if absent |
| Stop the `guard.protectedPaths` advisory | Delete the line from `fusion-guard.json`. The key is **retired**: fusion removed the protected-path mechanism it configured, so the loader drops the value and names the key in an advisory on every guarded tool call until the line comes out. The rest of the file keeps working while it is there. Full account in [`README-hooks.md`](README-hooks.md#per-project-configuration-fusion-guardjson) |

  Only two things ever block a write: a write to a **decision-governed path at `high` sensitivity**, and an active **halt** (which trips after `escalation.blocksBeforeHalt` consecutive blocks, default 3). There was a third until 2026-08-12 — a write to a **protected path**, at any sensitivity — and the whole protected-path half of the guard was removed with it. Churn detection sat beside all of this until 2026-08-15, counting per-file edits and emitting warning events for the monitor; it never blocked a write or halted on its own, and it is gone. `guard.enabled: false` stands the whole guard down. See [`README-hooks.md`](README-hooks.md) for the full model.
