...
Overrides: `FUSION_REF` (git ref, e.g. `FUSION_REF=tags/v11.11.1` to pin a release — every release since v5.5.0 is tagged), `FUSION_HOME` (install dir, default `~/.fusion`), `FUSION_BIN` (launcher dir, default `~/.local/bin`).
...
**The daily loop, in five lines:** morning — `/fusion:cadence` shows what you have actually been doing; work — start the orchestrator and say what you want; ideas on the way — `/fusion:wp` files them without breaking stride; done — `/fusion:cleanup`, and you may walk away: one question waits for your return. Everything below is that loop in detail.
