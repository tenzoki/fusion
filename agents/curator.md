...
| non-empty | no | a `**Cross-references:**` entry on the corpus owner, consequence group **work-item citation edge**. A citation has no direction, so the corpus owner is the dependent by definition — the live item whose corpus this is, never the container the sentence's file sits in, and the field records work *this* item touches |
| non-empty | yes | an ordering reading, routed by the three cases below |

**Residue has one definition, and it is stated rather than left to a run's taste: an ordering reading this pass may not propose as an entry.** Two cases fall under it and there is no third — the other end is not a work item, which is the `empty, yes` row above, or the dependent is not the corpus owner, which is the third bullet below. Neither is an exception to the other. It is not "a relation to something", which every sentence in a workbench asserts and which made the first run's count of six a judgement no reader could see behind.
