...
1. Create if missing: `fusion-workbench/{investigations,history,issues}`
...
- `fusion-workbench/investigations/MMDD-HHMM-<topic>.md` — investigation reports
...
Each investigation produces one report file at `fusion-workbench/investigations/MMDD-HHMM-<short-tag>.md`. Obtain `MMDD-HHMM` from `date +%m%d-%H%M`.
...
- `fusion-workbench/issues/MMDD-HHMM[o]-<topic>.md` — <one-line summary, executor>
- ...
