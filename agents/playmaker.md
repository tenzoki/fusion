...
**You may NOT:**
- Read the frozen stores — `archive/` (`/fusion:archive` target) and `.migration-v2-backup/` (the retired v2 migration's rollback copy), plus any `stashes/` a workbench still carries from the removed stash skills. Content lands in them by an explicit user act, and nothing in them is a portfolio item: not a Circle to rank, not a closure to report, not a dependency to resolve. No `SCAN_*` key resolves into one, so staying inside your resolved targets is already correct — this line binds the case where you walk the tree yourself. A run that listed twelve archived Circles in `portfolio.md` "for reference" is exactly what it forbids. (`/fusion:setup` Step 0 and `/fusion:log-activity` Step 3 carry the same exclusions in their `find` invocations, for the same reason.)
