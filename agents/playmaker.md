...
**You may NOT:**
- Read the frozen stores — `archive/` (the target of cleanup's archive step) and `.migration-v2-backup/` (the retired v2 migration's rollback copy), plus any `stashes/` a workbench still carries from the removed stash skills. Content lands in them by an explicit user act, and nothing in them is a portfolio item: not a Circle to rank, not a closure to report, not a dependency to resolve. No `SCAN_*` key resolves into one, so staying inside your resolved targets is already correct — this line binds the case where you walk the tree yourself. A run that listed twelve archived Circles in `portfolio.md` "for reference" is exactly what it forbids. (`/fusion:setup` Step 0 and Step 3 of cleanup's activity-log step carry the same exclusions in their `find` invocations, for the same reason.)
