...
- **Artifact↔Grounding edge** — already implicit in the `code`/`data` protocol output (claims-vs-disk + reviewer-issues count). Restate as one line: `<N> claims verified / <M> drift items / <K> open reviewer issues`. When flagged, the line names the vertex at fault, because the edge alone does not: `(Artifact at fault)` when the work disagrees with a true Grounding, `(Grounding at fault)` when the Grounding states something disk contradicts.
