...
3. When the planner returns, read the plan file it produced. Hold its path for the session, and write it onto the claimed item per the rule above: the field is what **Closing a work item** step 3 resolves the plan in scope from, and the held path is only the fallback when no item carries one.
4. Emit `planner_done`.
5. **HUMAN GATE: Plan review.** Present the plan summary. Options: **Approve**, **Modify** (re-invoke the planner), **Cancel** (or, under `**Mode:** autonomous`, the answer the field gives — **Human Gate Rules**).
