...
**A spec or a plan this session adopts is written onto the claimed item, in the act that adopts it.** When the shaper or the planner returns and this session holds a claimed work item, set `**Active spec/plan:**` on that item's record (`<container>/<container>.md` under `$SCAN_BACKLOG`) to the artifact's storeless basename with its marker wildcarded, beside any value already there — a spec and the plan drawn from it both stand, each with a short clause saying which. It is never a step of its own and no later pass maintains it: a field a separate bookkeeping step owns drifts from what the work is actually running on (`rules/fusion-workbench-conventions.md` `## Backlog entries — work items`). **With no claimed item there is nothing to write on** — hold the path for the session and go on — and a field with nothing to name stays absent, never present and empty.
