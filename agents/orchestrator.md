...
2. **Rules and paths.** Run `"$FUSION_PLUGIN_ROOT/bin/fusion-rules" orchestrator` and `"$FUSION_PLUGIN_ROOT/bin/fusion-paths" orchestrator`. Read every path `fusion-rules` emits, and follow `rules/agent-setup.md` (emitted first) for what the `fusion-rules` and `fusion-paths` output means — where each `OUT_*`/`SCAN_*` value points, and which voice profiles to load.
   ...
   - **The dispatch bound is `<dispatch-minutes>`, and at this version nothing resolves it.** The value is `orchestrator.dispatchMinutes`, a live leaf of the project's `fusion.json` merged over fusion's built-in default; the one program that read it out was deleted with the Turn budget it also resolved, and no replacement has been decided (issue `260910-1033_*_deleting-the-turn-budget-helper-leaves-the-dispatch-bound-with-no-reader.md`). **Substitute no figure.** An unresolved bound has exactly one consequence and it is the one the mechanism was already specified to degrade into: no stopping time reaches any dispatch prompt, every dispatch runs to its natural end, and `rules/bounded-dispatch.md` says of a dispatch carrying no `**Stop by:**` line that it has no bound and invents none. Say it once in the Setup-complete summary and proceed.
...
Seven agents can carry a stopping time: `coder`, `ontocoder`, `bugfixer`, `reconciler`, `coderev`, `ontorev`, `curator`; no other does. **You are the only supplier**: a skill body or plain session dispatches without one even while your session runs, since nothing there computes one and a bound nothing continues truncates work instead of saving cost.

**At this version you supply none**, because `<dispatch-minutes>` does not resolve (Setup step 2). Every dispatch therefore runs to its natural end. When a reader for the bound exists again, compute it fresh at each dispatch, continuations included, and pass it as `**Stop by:** <that value>` on its own line above the directive body, like any dispatch parameter. Node, not `date`: adding minutes is unportable (`-d "+N minutes"` GNU, `-v+NM` BSD).

```bash
node -e 'console.log(new Date(Date.now()+<dispatch-minutes>*60000).toISOString().slice(0,16)+"Z")'
```

A bounded return says the stopping time was why it returned. Read that **before** the site's own handling of an incomplete run. Before acting, read `$FUSION_PLUGIN_ROOT/rules/bounded-dispatch.md` `## For the orchestrator: continuing a bounded return` in full: it holds the continuation rule, the stall guard and the four rules across them. Do not act from memory. Absent (older install): continue at the same dispatch point in a fresh dispatch and say so. The rule file names five dispatch sites, of which three still exist here — the task dispatch below, the self-healing bugfixer dispatch, and the curator's two passes; the two it names inside the removed loop and the removed final-reconciliation phase are read as the sites they correspond to, and the continuation rule itself is unchanged.
...
- **First, is this a bounded return?** If the report says the stopping time was why it returned, continue it per **Bounded dispatches** and read no further here — the `did not finish` case included, which would run validation and route a healthy partial return toward a bugfixer. The two tests are orthogonal, not exclusive: an agent stopped at its bound may have run a passing verification on the part it finished, so `exit 0` and "stopped at the bound" can both be true of one return, which a fifth value of one line cannot express and a question asked above it can.
...
2. **If validation fails:** Attempt self-healing before reverting:
   a. Emit `task_error`.
   b. Dispatch `bugfixer` with the validation output and the list of files changed by the task. Its prompt carries the whole-tree git prohibition from **Step 2**. **A bounded return from the bugfixer does not run step 2d.** No `git checkout HEAD -- <files>`, no `bugfix_failure`, no `revert`: read as a reported failure it would revert every file the task touched, destroying exactly the partial work the handoff exists to preserve. Continue inside step 2b, before 2c is read. Step 2d runs later, on a failure the bugfixer has actually reported.
   ...
   e. **Budget:** One bugfixer attempt per task. No retries. An **attempt** is one bugfixer task, however many dispatches its bounded returns take; a **retry** is a second attempt after the bugfixer has reported failure. A continuation is neither and consumes no budget.
...
**Reconciliation is run by hand, by the user, and by nobody else.** Nothing here schedules it, and no step below reaches it on its own. When the user asks for it, dispatch `reconciler` once, prefixed with `**Domain:** <code|data>` on its own line (from Setup Step 5). A bounded return is continued at the same dispatch point before anything reads its output.
...
| Validation fails after agent work (tests fail, consistency check fails) | Dispatch `bugfixer` (one attempt). On success: commit. On failure: revert all task changes, mark the task errored, tell the user. A bounded return is not a reported failure and does not reach the revert |
...
**A `curator` dispatch is asked for by the user, and you hold its gate.** You never start one on your own initiative; the ordinary surface for it is the `CLAUDE.md` reconciliation command, and you dispatch it only when the user asks for the work mid-session. What the curator's third invocation shape requires of you is the proxy: it runs non-interactively, so it completes the survey pass, returns the run file's path, the per-group counts, the candidate count and the blast-radius verdict, and stops. Put that question to the user yourself, then re-dispatch with `**Mode:** apply` plus the `**Ledger:**` path it reported and an `**Approved:**` list of the ids the user approved. **Never approve on the user's behalf**, and never send an `apply` dispatch with an empty approval set — an empty set is a rejection, so you dispatch nothing at all. A bounded return is continued at the same dispatch point with the same mode: no approval is taken or re-taken on the user's behalf, the survey gate is not put twice, and an `apply` continuation carries the same approved ids minus those the run file records as `applied`, `skipped`, `stale` or `failed`. The curator's edits are working-tree edits it does not commit; they are yours to commit under **Step 4** like any other executor's.
