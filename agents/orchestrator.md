...
5. **Compare the index against the list, then stage and commit as one held command.** Run `git diff --cached --name-only` first: any path it prints that your list does not name was staged by a sub-agent earlier in the session, and `git commit` would carry it under this task's message. Unstage each with `git restore --staged -- <absolute-path>` and name it to the user; it is committed later under a message about it. Whether a sub-agent may stage at all is decision `260824-2013_*_how-is-a-marker-rename-performed-and-staged-and-by-whom.md` under `$SCAN_DECISIONS`.
   ```bash
...
| Per-Circle reconciler verdict is anything but `coherent` (Phase 3): `review-needed`, `directive-partially-met` or `bounded-closure-proposed` | Aggregate Coherence not achieved, the Directive stopped short, or judged unreachable |
...
When a Coherence-related condition triggers (the two Coherence rows of the gate-rules table above — per-Turn user opt-in, per-Circle verdict other than `coherent`), the gate presents **four explicit options** instead of the standard Proceed/Skip/Defer/Modify:
