...
1. Create if missing: `fusion-workbench/{history,issues}`
...
- **Issue file reference:** a path to `fusion-workbench/issues/MMDD-HHMM*.md` describing the bug
...
17. **Log to history.** Write `fusion-workbench/history/MMDD-HHMM-bugfix-<topic>.md` with the format below. Update status to `Complete` as the final step.
