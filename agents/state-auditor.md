...
6. **Read the session anchor off your dispatch prompt.** No file carries it: there is no session state file and no session history file, so the dispatcher hands you both values or you do not have them.
   - `**Directive:** <text>` — the session brief, the canonical input for Step 2.5's Artefact↔Brief and Evidence base↔Brief edges.
   - `**Since:** <commit>` — the `<session-start-HEAD>` anchor for the `git log <session-start-HEAD>..HEAD` walk in Step 2.5's Artefact↔Brief edge. Absent, fall back to the stamp on the work package this checkout has claimed and say in your report that you did; with nothing claimed, say that no anchor was available and walk no range.

   **A missing brief is not one you invent.** With no `**Directive:**` line, the two Brief edges read `not evaluable: no brief stated` and the recommendation is `state brief`. Improvising one is the failure this branch exists to prevent.

**Step 2.5's three-edge audit result depends on the brief and the session-start git anchor from step 6.** They are mandatory and precede Step 2.5: without them the result improvises a brief or stalls, and both are wrong outcomes.
...
**Layered on top of every domain:** a three-edge Coherence audit result (Artefact↔Evidence base, Artefact↔Brief, Evidence base↔Brief) computed from the workbench and returned as a `## Coherence` section of your report. See Step 2.5 below.
...
**Compute the aggregate result.** One of four, disjoint and complete:
...
- `directive-partially-met` — the brief is reachable, at least one clause of it is unmet in the artefact, and the shortfall is filed. This is work stopped short on purpose: nothing drifted and nothing is unreachable, so neither neighbour fits.
- `bounded-closure-proposed` — the brief is judged definitively unreachable.

The result is computed deterministically from the edge flags, not from LLM-judgement-from-vibes. Each edge's evidence is cited.
...
**And the three-edge Coherence audit result**, as its own section of the report, in this exact format — the orchestrator parses it there:

```markdown
## Coherence

**Audit result:** coherent | review-needed | directive-partially-met | bounded-closure-proposed

**Edges:**
- Artefact↔Evidence base: <one line>
- Artefact↔Brief: <one line, or `not evaluable: <reason>`>
- Evidence base↔Brief: <one line, or `not evaluable: <reason>`>

**Rebalance recommendation:** <none | state brief | revise artefact | revise evidence base | revise brief | accept Bounded Closure>
```

The recommendation maps from the result and the vertex the flagged edge faults:
- either Brief edge `not evaluable` because no brief was stated → `state brief`, whatever the result; no Rebalance option addresses a brief that does not exist
...
- `review-needed` with `Artefact↔Evidence base` flagged `(artefact at fault)` → `revise artefact`
- `review-needed` with `Artefact↔Evidence base` flagged `(evidence base at fault)` or `Evidence base↔Brief` flagged → `revise evidence base`
- `review-needed` with `Artefact↔Brief` flagged (commits orthogonal/away from brief) → `revise brief`
- `directive-partially-met` or `bounded-closure-proposed` → `accept Bounded Closure`

If multiple edges are flagged, list the recommendation that resolves the highest-leverage one (brief first, then evidence base, then artefact). The orchestrator presents the four-option Rebalance approval regardless, and fires it on `coherent` too when the recommendation is `state brief`, where Revise Brief is the option that states one; the recommendation is advisory.

**Rationale for the priority order.** A wrong brief forces a wrong artefact and a wrong evidence base, so revising the brief resolves more drift than revising the artefact: **Revise Brief** (the destination is wrong) → **Revise Evidence base** (the basis is wrong) → **Revise Artefact** (the work is wrong, destination and basis right) → **Accept Bounded Closure** (the destination is unreachable). The user always chooses; the order surfaces the most-likely-fundamental option first (Foundation V3 §2.1).
...
User-facing output (reconciliation summaries reported to the user, Coherence prose) follows `rules/user-facing-output.md`.
