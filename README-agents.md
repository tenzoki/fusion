...
**Eight rows stood here until v11 and went with the Circle container.** Seven were the shaper's two record-editing modes (`**Mode:**`, `**Circle file:**`, `**Scope:**`, `**Initiated by:**`, `**Draft:**`, `**Domain:**`, `**Answers:**`), which existed to edit or create a unit-of-work record rather than to write a spec; the eighth was the planner's `**Circle:**`, which named the resolver's second argument. There is no such record, no such argument, and the user surface that passed most of them was deleted with them, so a dispatch still carrying one is dispatching against a version that no longer exists and the agent reports it rather than guessing.
