...
- **Conditional:** `design-diagrams.md` for the design-diagram agents (`planner`, `analyst`, `taskplanner`, `shaper`); the long-form `default-voice-<lang>.yaml` for the prose agents; `circle-records.md` for the Circle-transitioning agents (`orchestrator`, `playmaker`, `shaper`); `decision-record-examples.md` for the decision-transition agents (`orchestrator`, `shaper`, `playmaker`, `planner`, `reconciler` — moved off the always-on floor at the `260827-0830` user gate); `user-facing-output.md` for the agents the user reads directly (`orchestrator`, `consultant`, `playmaker`, `shaper`, `editor`, `curator` — gate `260827-0910`; everyone else keeps the chat profile's anti-patterns via `agent-setup.md`); `commit-lock.md` for the `orchestrator`; `project-language.md` for the `editor` and `backlog-entries.md` for the `playmaker` (decision `260827-1056` — the operative cores stay always-on in the conventions); `review-contract.md` for the two review-writing agents (`coderev`, `ontorev`); and per-agent domain patterns (below).
