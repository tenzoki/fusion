...
- **Conditional:** `design-diagrams.md` for the design-diagram agents (the five producers + `conceptrev`); the long-form `default-voice-<lang>.yaml` for the prose agents; `protected-path-internals.md` for the guard-internals agents (`coder`, `coderev`, `bugfixer`) — in the fusion plugin's own repo only, where the classifier sources are editable (the repo check is at cwd with no upward walk, so it fires only when the session's cwd is the repo root itself; from a subdirectory the helper behaves as in a consuming project); `circle-records.md` for the Circle-transitioning agents (`orchestrator`, `playmaker`, `shaper`); `workbench-stash-and-lock.md` for the `orchestrator`; and per-agent domain patterns (below).
