...
|-------|----------------|-----------------|-----------|-----------|-------------|
| `taskplanner` | `**Domain:**` | `code` \| `data` \| `strategic` \| `knowledge` | defaults to `code`; selects the Priority Axis 1 logic | orchestrator at Phase 1 (`agents/orchestrator.md:449`) | `agents/taskplanner.md:19`, `:34-36` |
| `reconciler` | `**Domain:**` | the same four | defaults to `code`; selects the ground-truth verification protocol | orchestrator at Phase 3 (`agents/orchestrator.md:706`); `/fusion:cleanup` Step 3 (`skills/cleanup/SKILL.md:147`), which obtains the domain from `agentstate.yaml` and never decides one | `agents/reconciler.md:28-30`, `:45-47` |
| `playmaker` | `**Domain:**` | the same four | defaults to `code`; biases the ranking heuristics | orchestrator at Phase 4 (`agents/orchestrator.md:907`), `/fusion:next` Step 3 (`skills/next/SKILL.md:103`) and again on Step 5b's second dispatch (`skills/next/SKILL.md:170`), or a direct user dispatch | `agents/playmaker.md:25-27`, `:36-38` |
...
| `planner` | `**Executors:**` | comma-separated, each one of `coder` \| `ontocoder` \| `analyst` | defaults to `[coder, ontocoder]`; unrecognised names are ignored | orchestrator, when the detected domain is `strategic` or `knowledge` (`agents/orchestrator.md:434`) | `agents/planner.md:47-51` |
...
| `shaper` | `**Domain:**` | the same four | defaults to `code` | `/fusion:direct` (`skills/direct/SKILL.md:72`). Anticipated-circle mode only, and a pass-through: it fills the new Circle record's `**Domain:**` frontmatter and changes nothing about how the shaper itself works | `agents/shaper.md:59`, `:82` |
...
| `editor` | `**Deliverable language:**` | `de` \| `en` | **halts and produces nothing.** This is the one parameter with no default and no fallback | orchestrator (`agents/orchestrator.md:495`, `:1454`), or the user | `agents/editor.md:18-30` |
