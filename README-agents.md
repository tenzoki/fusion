...
| `playmaker` | `**Domain:**` | the same two | defaults to `code`; biases the ranking heuristics | orchestrator at Phase 4 (`agents/orchestrator.md:875`), `/fusion:next` Step 3 (`skills/next/SKILL.md:103`) and again on Step 5b's second dispatch (`skills/next/SKILL.md:170`), or a direct user dispatch | `agents/playmaker.md:25-27`, `:34-36` |
...
| `shaper` | `**Initiated by:**` | the question the user was asked, the option they chose, and the date — quoted, not paraphrased | **halts** whenever the run was dispatched and this line is missing or empty; optional when the user runs shaper top-level. Which of the two a run is, the shaper decides for itself by testing whether it holds `AskUserQuestion` (`agents/shaper.md:66`) | orchestrator, on the portfolio-activation dispatch and on every re-dispatch of it (`agents/orchestrator.md:345`); a top-level user run may carry it, and no skill passes it | `agents/shaper.md:66`, `:68` |
| `shaper` | `**Draft:**` | the user's raw draft text (may span lines), or a path to a backlog entry | **halts** whenever `**Mode:** anticipated-circle` was given and this line is missing or empty | `/fusion:direct <draft>` (`skills/direct/SKILL.md:71`) | `agents/shaper.md:70`, `:73`, `:117` |
| `shaper` | `**Domain:**` | the same two | defaults to `code` | `/fusion:direct` (`skills/direct/SKILL.md:72`). Anticipated-circle mode only, and a pass-through: it fills the new Circle record's `**Domain:**` frontmatter and changes nothing about how the shaper itself works | `agents/shaper.md:70`, `:93` |
