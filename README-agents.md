...
**"Anything" in the Reads column is the project tree minus `.secret`.** Ten of the sixteen prompts state that exclusion in their own Scope section — `analyst`, `bugfixer`, `conceptrev`, `coderev`, `consultant`, `investigator`, `ontorev`, `orchestrator`, `planner`, `shaper`. The other six (`coder`, `ontocoder`, `reconciler`, `taskplanner`, `playmaker`, `editor`) say nothing about `.secret` either way.
...
Six agents read run-time parameters off the dispatch prompt: plain markdown lines of the form `**<Keyword>:** <value>`, one per line, ahead of the directive body. **This table is the roster's single authoring home** — the agent rows above name no parameters, and `CLAUDE.md` cites this section rather than restating it. The last column names the prompt lines each row was read against.

| Agent | Parameter line | Accepted values | If absent | Passed by | Declared at |
|-------|----------------|-----------------|-----------|-----------|-------------|
| `taskplanner` | `**Domain:**` | `code` \| `data` \| `strategic` \| `knowledge` | defaults to `code`; selects the Priority Axis 1 logic | orchestrator at Phase 1 | `agents/taskplanner.md:19`, `:34-36` |
| `reconciler` | `**Domain:**` | the same four | defaults to `code`; selects the ground-truth verification protocol | orchestrator at Phase 3 | `agents/reconciler.md:28-30`, `:45-47` |
| `playmaker` | `**Domain:**` | the same four | defaults to `code`; biases the ranking heuristics | orchestrator at Phase 4, `/fusion:next`, or a direct user dispatch | `agents/playmaker.md:25-27`, `:36-38` |
| `planner` | `**Executors:**` | comma-separated, each one of `coder` \| `ontocoder` \| `analyst` | defaults to `[coder, ontocoder]`; unrecognised names are ignored | orchestrator, when the detected domain is `strategic` or `knowledge` | `agents/planner.md:47-51` |
| `planner` | `**Circle:**` | a Circle directory name — no marker, no `.md`, no prefix | resolves as usual: the active Circle when one is active, `shared/` when none is | orchestrator or user, to plan an anticipated Circle before it is activated. A name matching no directory exits 1 from the resolver and the planner halts rather than re-resolving | `agents/planner.md:53-55` |
| `shaper` | `**Mode:**` | `portfolio-activation` \| `anticipated-circle` | falls back to the existing mode-detection heuristic, i.e. user-direct | the user directly for `portfolio-activation`; `/fusion:direct` for `anticipated-circle` | `agents/shaper.md:39-47`, `:57` |
| `shaper` | `**Circle file:**` | workbench-relative path to a Circle's `_a_circle.md` record | **halts** whenever `**Mode:** portfolio-activation` was given and this line is missing or unreadable | the user running shaper top-level | `agents/shaper.md:47`, `:55` |
| `shaper` | `**Draft:**` | the user's raw draft text (may span lines), or a path to a backlog entry | **halts** whenever `**Mode:** anticipated-circle` was given and this line is missing or empty | `/fusion:direct <draft>` | `agents/shaper.md:57`, `:60`, `:104` |
| `shaper` | `**Domain:**` | the same four | defaults to `code` | `/fusion:direct`. Anticipated-circle mode only, and a pass-through: it fills the new Circle record's `**Domain:**` frontmatter and changes nothing about how the shaper itself works | `agents/shaper.md:57`, `:80` |
| `shaper` | `**Parent task:**` | path to the active task file | no parent-task context; the spec output is the same either way | orchestrator, in in-Circle clarification mode | `agents/shaper.md:45` |
| `editor` | `**Deliverable language:**` | `de` \| `en` | **halts and produces nothing.** This is the one parameter with no default and no fallback | orchestrator, or the user | `agents/editor.md:18-30` |
