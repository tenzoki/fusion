...
Six agents read run-time parameters off the dispatch prompt: plain markdown lines of the form `**<Keyword>:** <value>`, one per line, ahead of the directive body. A value may run past its own line — `**Draft:**` and `**Confirmed operations:**` both end at the next `**<Keyword>:**` line or at the end of the prompt. **This table is the roster's single authoring home** — the agent rows above name no parameters, and `CLAUDE.md` cites this section rather than restating it. `Declared at` names the prompt lines each row was read against. `Passed by` has its own ground truth, and it is not the agent prompts: a skill can pass a parameter no agent prompt mentions, so every cell in that column was read against `agents/*.md` **and** all sixteen `skills/*/SKILL.md` bodies, and names the line it was read against wherever the passer is a skill.
...
| `playmaker` | `**Domain:**` | the same four | defaults to `code`; biases the ranking heuristics | orchestrator at Phase 4 (`agents/orchestrator.md:850`), `/fusion:next` Step 3 (`skills/next/SKILL.md:103`), or a direct user dispatch | `agents/playmaker.md:25-27`, `:36-38` |
| `playmaker` | `**Confirmed operations:**` | a block of `- split …` / `- merge …` / `- close …` / `- defer …` lines, one backlog operation per line, copied verbatim from the first run's own proposals | no confirmation travels on the prompt, so the run performs none of the four operations. It may still perform one it confirmed the other way, by asking the user itself — which a dispatched sub-agent cannot do | `/fusion:next` Step 3's second dispatch (`skills/next/SKILL.md:167-176`) | `agents/playmaker.md:207-215` |
| `playmaker` | `**Proposal source:**` | the portfolio path, plus the `**Generated:**` stamp that portfolio carried when the operations were proposed | the stamp check has nothing to read. `agents/playmaker.md:219` defines the refusal for a stamp that differs, a portfolio that is gone, and a portfolio whose header carries no `**Generated:**` value; an omitted line is not among the three | `/fusion:next` Step 3's second dispatch (`skills/next/SKILL.md:176`, contract restated at `:179`) | `agents/playmaker.md:207`, `:216`, `:219` |
